...
function Qn({ scrRef: e, handleSubmit: r, id: n }) {
  const [t, o] = ce.useState(Ar(e.bookNum)),
    ...
    s = (i, f) => {
      const l = { bookNum: le.bookIdToNumber(f.bookId), chapterNum: 1, verseNum: 1 };
      a(l);
    },
    c = (i) => {
...
function rt({
  ...
  const u = {
    action: c || (s == null ? void 0 : s.action),
...
function ot({
  ...
  const fe = ce.useMemo(() => {
    ...
    return f ? [{ ...Or.SelectColumn, minWidth: p }, ...ee] : ee;
  }, [e, f, p]);
  return N.jsx(Or, {
    columns: fe,
      ...
      const W = fr(s, A({ shouldForwardProp: S, label: d }, v)),
        j = ($, ...m) => {
          ...
          u &&
            ...
            h || X.push(a);
          const ue = X.length - m.length;
          if (Array.isArray($) && ue > 0) {
            const B = new Array(ue).fill('');
            (te = [...$, ...B]), (te.raw = [...$.raw, ...B]);
          } else
            typeof $ == 'function' &&
              $.__emotion_real !== $ &&
