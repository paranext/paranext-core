...
function ln({
  ...
  let p;
  if (n) {
    const u = t === Ee.Before || t === Ee.Above,
      y = N.jsx('span', {
      ...
      t.map((a, s) =>
        N.jsx(
          dn,
          {
            ...
            onClick: () => {
              e({ name: a.name, command: a.command });
            },
            ...a,
          },
          s,
...
function Zn({ scrRef: e, handleSubmit: r, id: n }) {
  const t = (i) => {
    ...
    o = (i, l) => {
      const p = { bookNum: he.bookIdToNumber(l.bookId), chapterNum: 1, verseNum: 1 };
      t(p);
    },
    a = (i) => {
    ...
    c = ce.useMemo(() => sr()[e.bookNum - 1], [e.bookNum]);
  return N.jsxs('span', {
    id: n,
    children: [
      N.jsx(un, {
...
function Ot(e) {
  ...
  return A({ keys: s, values: a, up: c, down: i, between: l, only: f, not: p, unit: n }, o);
}
          ...
          if (v != null)
            if (typeof v == 'object')
              if (s[y]) u = Le(u, e(y, v, a, s));
                ...
                yo(h, v) ? (u[y] = r({ sx: v, theme: a })) : (u = Le(u, h));
              }
  ...
  return (
    ...
    (s, c = {}) => {
      ...
      const { name: i, slot: l, skipVariantsResolver: f, skipSx: p, overridesResolver: u } = c,
        ...
        h = p || !1;
      let d;
      process.env.NODE_ENV !== 'production' && i && (d = `${i}-${Mo(l || 'Root')}`);
      let S = Ke;
      l === 'Root' ? (S = t) : l ? (S = o) : _o(s) && (S = void 0);
...
function zo(e) {
  ...
  return e.type === 'hsla' && ((c += 'a'), i.push(r[3])), Cr({ type: c, values: i });
}
function en(e) {
  e = je(e);
...
`),
        ...
        return (Array.isArray(o) ? o : [o])
          .map(
            (f) =>
              `${f} ${typeof s == 'string' ? s : sn(s)} ${c} ${typeof i == 'string' ? i : sn(i)}`,
          )
          .join(',');
      },
    },
  ...
  const [s, c] = ce.useState(!1),
    ...
    p = ce.useCallback((S) => {
      S.stopPropagation(),
        c((W) => {
          const j = !W;
          return j && S.shiftKey ? l(!0) : j || l(!1), j;
        });
    }, []),
    u = ce.useRef(null),
    [y, v] = ce.useState(0);
