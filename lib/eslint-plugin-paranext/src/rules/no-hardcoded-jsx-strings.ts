...
function isTechnicalString(str: string): boolean {
  ...
  if (/^[\d.]+(%|px|em|rem|vh|vw|ch)?$/.test(str)) return true;
...
export default createRule({
  ...
  meta: {
    ...
    messages: {
      hardcodedString:
        "Hardcoded string '{{value}}' should be localized. Use localizedStrings['%key%'] instead.",
    },
  },
  defaultOptions: [],
        ...
        if (node.value?.type === 'Literal' && typeof node.value.value === 'string') {
          if (shouldReport(node.value.value)) {
            context.report({
              ...
              data: {
                value:
                  node.value.value.substring(0, 30) + (node.value.value.length > 30 ? '...' : ''),
              },
            });
          }
        }
