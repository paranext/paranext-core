...
function isTechnicalString(str) {
    ...
    // Explicitly allowed
    if (ALLOWED_COMPARISON_STRINGS.has(lower))
        return true;
    // File extensions
    if (/^\.[a-z0-9]+$/.test(str))
        return true;
...
exports.default = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Disallow comparisons against hardcoded English strings that should be localized',
            recommended: 'error',
        },
        schema: [],
        messages: {
            hardcodedComparison: "Comparison against hardcoded string '{{value}}'. Use constants or localized strings instead.",
        },
    },
    defaultOptions: [],
    create(context) {
        function checkLiteral(node, operator) {
