...
const createRule = utils_1.ESLintUtils.RuleCreator((name) => `https://github.com/paranext/paranext-core/blob/ai/main/.context/standards/Entry-Point-Guide.md#${name}`);
...
exports.default = createRule({
    ...
    meta: {
        ...
        schema: [],
        messages: {
            missingCleanup: "Registration result '{{name}}' should be added to context.registrations.add() for proper cleanup.",
            unusedRegistration: 'Registration call result is not captured. Store in a variable and add to context.registrations.add().',
        },
    },
    defaultOptions: [],
    create(context) {
        ...
        const registrationVariables = new Set();
        // Track variables that have been added to context.registrations
        const cleanedUpVariables = new Set();
        ...
        function isRegistrationCall(node) {
            ...
            if (node.callee.type === 'MemberExpression') {
                ...
                if (callee.property.type === 'Identifier') {
                    ...
                    if (!REGISTRATION_METHODS.includes(methodName))
                        return false;
