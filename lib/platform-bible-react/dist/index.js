...
function sh({ scrRef: t, handleSubmit: e }) {
  const n = xt(), [r, o] = ut(""), [a, i] = ut(
    ...
    (I) => hc(I).filter((M) => {
      const Y = ot.bookIdToEnglishName(M).toLowerCase(), G = r.replace(/[^a-zA-Z]/g, "").toLowerCase();
              ...
              pc,
              {
                ...
                children: /* @__PURE__ */ s(
                  uc,
                  {
                    ...
                    endChapter: on(Y),
                    activeChapter: t.bookNum === ot.bookIdToNumber(Y) ? t.chapterNum : 0,
...
function Sd() {
  return ma || (ma = 1, process.env.NODE_ENV !== "production" && function() {
    ...
    st.AsyncMode = b, st.ConcurrentMode = _, st.ContextConsumer = F, st.ContextProvider = R, st.Element = $, st.ForwardRef = L, st.Fragment = D, st.Lazy = I, st.Memo = M, st.Portal = Y, st.Profiler = G, st.StrictMode = B, st.Suspense = Z, st.isAsyncMode = rt, st.isConcurrentMode = y, st.isContextConsumer = O, st.isContextProvider = U, st.isElement = X, st.isForwardRef = z, st.isFragment = K, st.isLazy = q, st.isMemo = W, st.isPortal = H, st.isProfiler = J, st.isStrictMode = Q, st.isSuspense = dt, st.isValidElementType = C, st.typeOf = E;
  }()), st;
}
var ha;
function Zi() {
...
function jd() {
  return Ea || (Ea = 1, process.env.NODE_ENV !== "production" && function() {
    ...
    var _ = i, F = a, R = t, $ = c, L = n, D = w, I = f, M = e, Y = o, G = r, B = d, Z = p, at = !1, rt = !1;
    function y(V) {
...
function Tf(t) {
  ...
  if (_) {
    if (a) {
      var M, Y = E === "y" ? Ot : Rt, G = E === "y" ? Ft : Lt, B = E === "y" ? "height" : "width", Z = _[E], at = Z + h[Y], rt = Z - h[G], y = g ? -R[B] / 2 : 0, O = T === Ue ? F[B] : R[B], U = T === Ue ? -R[B] : -F[B], X = e.elements.arrow, z = g && X ? Fo(X) : {
        width: 0,
        height: 0
      }, K = e.modifiersData["arrow#persistent"] ? e.modifiersData["arrow#persistent"].padding : Ts(), q = K[Y], W = K[G], H = hn(0, F[B], z[B]), J = C ? F[B] / 2 - y - H - q - L.mainAxis : O - H - q - L.mainAxis, Q = C ? -F[B] / 2 + y + H + W + L.mainAxis : U + H + W + L.mainAxis, dt = e.elements.arrow && $n(e.elements.arrow), A = dt ? E === "y" ? dt.clientTop || 0 : dt.clientLeft || 0 : 0, Nt = (M = D == null ? void 0 : D[E]) != null ? M : 0, V = Z + J - Nt - A, yt = Z + Q - Nt, Ut = hn(g ? Zn(at, V) : at, Z, g ? ye(rt, yt) : rt);
      _[E] = Ut, I[E] = Ut - Z;
    }
    if (l) {
...
const js = /* @__PURE__ */ j.forwardRef(function(e, n) {
  ...
  const _ = jo({
    ...
    children: R
  }), Ut = Is(), de = Ut.direction === "rtl", [St, An] = j.useState(), [Xt, Ce] = j.useState(null), pe = j.useRef(!1), Oe = Y || rt, Re = dn(), _e = dn(), ue = dn(), Ze = dn(), [Mn, Ho] = is({
    controlled: K,
    default: !1,
    name: "Tooltip",
    state: "open"
