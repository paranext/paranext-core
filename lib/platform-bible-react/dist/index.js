...
import $t, { forwardRef as Oi, useCallback as Pe, useState as xe, useRef as ft, useEffect as Lt, useLayoutEffect as Eo, useMemo as fn } from "react";
...
function jh({
  ...
  const [o, i] = xe(1), [s, l] = xe(r), [c, u] = xe(
    Array.from({ length: r }, (d, b) => b + 1)
  );
  Lt(() => {
    i(1), e(1), l(r), t(r), u(Array.from({ length: r }, (d, b) => b + 1));
  }, [r, t, e]);
  const p = (d, b) => {
    i(b), e(b), b > s && (l(b), t(b));
  }, f = (d, b) => {
...
var Ne = "top", Fe = "bottom", Ve = "right", Re = "left", zr = "auto", gn = [Ne, Fe, Ve, Re], At = "start", un = "end", Ac = "clippingParents", pa = "viewport", Xt = "popper", Dc = "reference", Xo = /* @__PURE__ */ gn.reduce(function(e, t) {
...
function eu(e) {
  ...
  if (!(!i || !s)) {
    var f = Qc(o.padding, n), d = Hr(i), b = c === "y" ? Ne : Re, v = c === "y" ? Fe : Ve, g = n.rects.reference[p] + n.rects.reference[c] - s[c] - n.rects.popper[p], h = s[c] - n.rects.reference[c], T = bn(i), I = T ? c === "y" ? T.clientHeight || 0 : T.clientWidth || 0 : 0, y = g / 2 - h / 2, w = f[b], m = I - d[p] - f[v], S = I / 2 - d[p] / 2 + y, P = on(w, S, m), L = c;
...
function Jo(e) {
  ...
  if (u) {
    ...
    if (S === Ae(n) && (S = at(n), Ze(S).position !== "static" && l === "absolute" && (P = "scrollHeight", L = "scrollWidth")), S = S, o === Ne || (o === Re || o === Ve) && i === un) {
      w = Fe;
...
function vu(e) {
  ...
  if (!t.modifiersData[r]._skip) {
    for (var o = n.mainAxis, i = o === void 0 ? !0 : o, s = n.altAxis, l = s === void 0 ? !0 : s, c = n.fallbackPlacements, u = n.padding, p = n.boundary, f = n.rootBoundary, d = n.altBoundary, b = n.flipVariations, v = b === void 0 ? !0 : b, g = n.allowedAutoPlacements, h = t.options.placement, T = Ue(h), I = T === h, y = c || (I || !v ? [Rn(h)] : bu(h)), w = [h].concat(y).reduce(function(z, H) {
      ...
      var C = w[F], R = Ue(C), $ = Bt(C) === At, j = [Ne, Fe].indexOf(R) >= 0, D = j ? "width" : "height", M = pn(t, {
        placement: C,
        boundary: p,
        rootBoundary: f,
        altBoundary: d,
  ...
  if (S) {
    if (i) {
      var $, j = w === "y" ? Ne : Re, D = w === "y" ? Fe : Ve, M = w === "y" ? "height" : "width", V = S[w], Q = V + h[j], Z = V - h[D], O = b ? -L[M] / 2 : 0, A = I === At ? P[M] : L[M], U = I === At ? -L[M] : -P[M], K = t.elements.arrow, z = b && K ? Hr(K) : {
        width: 0,
        height: 0
      }, H = t.modifiersData["arrow#persistent"] ? t.modifiersData["arrow#persistent"].padding : ma(), q = H[j], G = H[D], W = on(0, P[M], z[M]), X = y ? P[M] / 2 - O - W - q - F.mainAxis : A - W - q - F.mainAxis, Y = y ? -P[M] / 2 + O + W + G + F.mainAxis : U + W + G + F.mainAxis, ne = t.elements.arrow && bn(t.elements.arrow), _ = ne ? w === "y" ? ne.clientTop || 0 : ne.clientLeft || 0 : 0, J = ($ = C == null ? void 0 : C[w]) != null ? $ : 0, N = V + X - J - _, re = V + Y - J, ve = on(b ? jn(Q, N) : Q, V, b ? ht(Z, re) : Z);
      S[w] = ve, R[w] = ve - V;
    }
    if (l) {
      var Oe, me = w === "x" ? Ne : Re, lt = w === "x" ? Fe : Ve, Se = S[m], Ge = m === "y" ? "height" : "width", $e = Se + h[me], Ke = Se - h[lt], we = [Ne, Re].indexOf(T) !== -1, wt = (Oe = C == null ? void 0 : C[m]) != null ? Oe : 0, ct = we ? $e : Se - P[Ge] - L[Ge] - wt + F.altAxis, zt = we ? Se + P[Ge] + L[Ge] - wt - F.altAxis : Ke, xn = b && we ? Zc(ct, Se, zt) : on(b ? ct : $e, Se, b ? zt : Ke);
      S[m] = xn, R[m] = xn - Se;
    }
    t.modifiersData[r] = R;
  }
...
const Ia = /* @__PURE__ */ x.forwardRef(function(t, n) {
  ...
  const S = tt({
    ...
    children: L
  }), ve = wn(), Oe = ve.direction === "rtl", [me, lt] = x.useState(), [Se, Ge] = x.useState(null), $e = x.useRef(!1), Ke = j || Z, we = Qt(), wt = Qt(), ct = Qt(), zt = Qt(), [xn, so] = Zi({
    controlled: H,
    default: !1,
    name: "Tooltip",
    state: "open"
...
`));
  ...
  const or = x.useRef(), rs = (ee) => {
    const Me = re.props;
    Me.onMouseMove && Me.onMouseMove(ee), Jt = {
      x: ee.clientX,
      y: ee.clientY
...
function ao(e) {
  const { menuDefinition: t, onClick: n, commandHandler: r, includedGroups: o } = e, { items: i, allowForLeadingIcons: s } = fn(() => {
