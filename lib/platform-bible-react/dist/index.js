...
var ta = Object.defineProperty, ea = (t, e, n) => e in t ? ta(t, e, { enumerable: !0, configurable: !0, writable: !0, value: n }) : t[e] = n, V = (t, e, n) => ea(t, typeof e != "symbol" ? e + "" : e, n);
const yt = [
  "GEN",
  "EXO",
  "LEV",
...
V(tt, "Original", new tt(it.Original)), V(tt, "Septuagint", new tt(it.Septuagint)), V(tt, "Vulgate", new tt(it.Vulgate)), V(tt, "English", new tt(it.English)), V(tt, "RussianProtestant", new tt(it.RussianProtestant)), V(tt, "RussianOrthodox", new tt(it.RussianOrthodox));
let vt = tt;
function tn(t, e) {
  const n = e[0];
...
V(W, "defaultVersification", vt.English), V(W, "verseRangeSeparator", "-"), V(W, "verseSequenceIndicator", ","), V(W, "verseRangeSeparators", [W.verseRangeSeparator]), V(W, "verseSequenceIndicators", [W.verseSequenceIndicator]), V(W, "chapterDigitShifter", 1e3), V(W, "bookDigitShifter", W.chapterDigitShifter * W.chapterDigitShifter), V(W, "bcvMaxValue", W.chapterDigitShifter - 1), /**
* The valid status of the VerseRef.
*/
V(W, "ValidStatusType", Sn);
...
const ma = 6, Fe = B.allBookIds.filter((t) => !B.isObsolete(B.bookIdToNumber(t))), Tn = Object.fromEntries(
  ...
  /^([^:\s]+(?:\s+[^:\s]+)*)$/i,
  // Matches book name or id - specific word boundaries
  /^([^:\s]+(?:\s+[^:\s]+)*)\s+(\d+)$/i,
...
function Ss({
  ...
  const o = U(), [s, i] = A(
    ...
    (m) => {
      const D = a ? a() : Fe;
      return {
        OT: D.filter((x) => B.isBookOT(x)),
        NT: D.filter((x) => B.isBookNT(x)),
        DC: D.filter((x) => B.isBookDC(x))
      }[m].filter((x) => {
        const E = Tn[x].toLowerCase(), Y = w.replace(/[^a-zA-Z]/g, "").toLowerCase();
        return E.includes(Y) || // Match book name
    ...
    (m, D, x, E) => {
      if (g(t.book !== m ? 1 : t.chapterNum), D || Pt(m) === -1) {
        e({
          book: m,
          chapterNum: x ?? 1,
          verseNum: E ?? 1
    ...
    ga.forEach((m) => {
      const D = m.exec(w);
      if (D) {
        const [x, E = void 0, Y = void 0] = D.slice(1), L = va(x);
      ...
      if (m.key === "Tab") {
        if (m.shiftKey)
          ...
          const D = [
            ...
            (E) => {
              var Y, L;
              return E instanceof HTMLElement && ((E.offsetWidth > 0 || E.offsetHeight > 0) && !((Y = z.current) != null && Y.contains(E)) && !((L = C.current) != null && L.contains(E)) || E === m.target);
            }
          ), x = m.target instanceof HTMLElement ? D.indexOf(m.target) : -1;
          x >= 0 ? D[(x + 1) % D.length].focus() : C.current.focus();
        }
    ...
    (m) => {
      const { key: D } = m;
      nn.has(D) || (dt(m), m.preventDefault());
    },
    [dt]
  ), M = q(
    (m, D) => {
      ...
      if (nn.has(x)) {
        if (h === u) {
          ...
          const E = x === "ArrowRight" && !o || x === "ArrowRight" && o === "ltr" || x === "ArrowLeft" && o === "rtl", Y = x === "ArrowLeft" && !o || x === "ArrowLeft" && o === "ltr" || x === "ArrowRight" && o === "rtl";
          let L = 0;
          if (E)
            if (v < Pt(h))
              L = 1;
  ...
  return wt(() => {
    ...
    const m = setTimeout(() => {
      ...
      !y && document.activeElement !== C.current && !((D = C.current) != null && D.contains(document.activeElement)) && document.activeElement !== z.current && !((x = z.current) != null && x.contains(document.activeElement)) && b();
    }, 10);
    return () => {
      clearTimeout(m);
    };
    ...
    /* @__PURE__ */ r(
      Yt,
      {
        ...
        onFocus: (m) => {
          var D, x;
          !((D = C.current) != null && D.contains(m.relatedTarget)) && !((x = z.current) != null && x.contains(m.relatedTarget)) && C.current.select();
        },
        children: /* @__PURE__ */ r("div", { className: "rtl:tw-ps-2", children: en.map((m, D) => {
          const x = gt(m);
          return x.length > 0 && /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ r(te, { className: "tw-font-semibold tw-text-foreground/80", children: ha[m] }),
            x.map((E, Y) => /* @__PURE__ */ r("div", { children: /* @__PURE__ */ r(
              pa,
              {
                bookId: E,
                handleSelectBook: () => R(E, !1),
                isSelected: u === E,
                handleHighlightBook: () => k(E),
                handleKeyDown: (L) => M(L, Y),
                bookType: m,
                ref: (L) => {
                  u === E && ($.current = L);
                },
...
function Fs({
  ...
  const u = pt(n, "%webView_inventory_all%"), f = pt(n, "%webView_inventory_approved%"), v = pt(n, "%webView_inventory_unapproved%"), g = pt(n, "%webView_inventory_unknown%"), h = pt(n, "%webView_inventory_scope_currentBook%"), k = pt(n, "%webView_inventory_scope_chapter%"), T = pt(n, "%webView_inventory_filter_text%"), N = pt(
    ...
    const M = [];
    return b.forEach((m) => {
      const D = m.items[0], x = M.find(
        (E) => E.items[0] === D
      );
      x ? (x.count += m.count, x.occurrences = x.occurrences.concat(m.occurrences)) : M.push({
        items: [D],
        count: m.count,
    ...
    var D, x;
    if (!y) return c;
    const M = (D = a == null ? void 0 : a.tableHeaders) == null ? void 0 : D.length;
    if (!M) return c;
    const m = [];
    for (let E = 0; E < M; E++)
      m.push(
  ...
  return /* @__PURE__ */ d("div", { className: "pr-twp tw-flex tw-h-full tw-flex-col", children: [
    /* @__PURE__ */ d("div", { className: "tw-flex tw-items-stretch", children: [
      ...
      /* @__PURE__ */ d(Tt, { onValueChange: (M) => Bt(M), defaultValue: i, children: [
