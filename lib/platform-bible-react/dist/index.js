...
import { c as b, g as $e, a as Vr, C as er, L as Kn, t as ks, u as tn, T as Vt, b as bt, d as xt, e as yt, f as Hn, A as ba, D as Fe, h as Te, i as je, j as cr, k as We, B as et, x as _s, l as Ns, m as Cs, I as Es, n as Co, o as Je, r as De, p as Ts, q as qn, P as lr, s as Dr, v as dr, w as wr, y as Da, z as Oa, E as Eo, F as ur, G as Ma, H as $t, R as To, J as xa, K as io, M as so, N as co, O as lo, Q as Gn, S as da, U as So, V as jr, W as Ur, X as yr, Y as Ss, Z as ir, _ as Ze, $ as Ue, a0 as Ye, a1 as kr, a2 as Ro, a3 as Do, a4 as ya, a5 as Oo, a6 as Wn, a7 as Rs, a8 as Ds, a9 as Os, aa as wo, ab as Yn, ac as Ia, ad as Ms, ae as Xn, af as Is, ag as zs, ah as Ps, ai as en, aj as As, ak as $s, al as Vs, am as Zn, an as Jn, ao as Mo, ap as Ls, aq as Qn, ar as Ka, as as rn, at as Ha, au as Bs, av as Fs, aw as js, ax as Us, ay as Ks, az as Hs, aA as qs, aB as Gs, aC as Ws, aD as Io, aE as zo, aF as Ys, aG as Xs } from "./resizable-FCXMv0ns.js";
import { aH as xf, aI as yf, aJ as kf, aK as _f, aL as Nf, aM as Cf, aN as Ef, aO as Tf, aP as Sf, aQ as Rf, aR as Df, aS as Of, aT as Mf, aU as If, aV as zf, aW as Pf, aX as Af, aY as $f, aZ as Vf, a_ as Lf, a$ as Bf, b0 as Ff, b1 as jf, b2 as Uf, b3 as Kf, b4 as Hf, b5 as qf, b6 as Gf, b7 as Wf, b8 as Yf, b9 as Xf, ba as Zf, bb as Jf, bc as Qf, bd as tm, be as em, bf as rm, bg as am, bh as om } from "./resizable-FCXMv0ns.js";
...
import { Section as At, compareScrRefs as ka, getChaptersForBook as Mc, formatScrRef as Ve, formatReplacementString as Xe, getSectionForBook as wa, formatRelativeDate as Ic, sanitizeHtml as Po, NumberFormat as ci, formatBytes as zc, getCurrentLocale as Pc, usfmMarkers as ua, isPlatformError as Ac, ABORTED as $c, getErrorMessage as Vc, getFormatCallerFunction as Lc, deepEqual as Bc, isString as sn, scrRefToBBBCCCVVV as qa, defaultScrRef as Ga, formatScrRefRange as Fc, getLocalizeKeyForScrollGroupId as cn, formatReplacementStringToArray as ln, collectUsjMarkers as jc } from "platform-bible-utils";
import ee, { useRef as L, useMemo as P, createContext as Gr, useContext as za, useEffect as Z, useState as _, useCallback as V, useId as _a, useImperativeHandle as Uc, useLayoutEffect as re, Fragment as Wr, Component as Kc, createElement as dn, Suspense as Hc, forwardRef as Ao, memo as li } from "react";
...
function Ya({
  ...
  const B = De(), U = wd(), [M, S] = _(!1), [D, W] = _(""), [Y, z] = _(""), [k, ot] = _("books"), [I, X] = _(void 0), [G, $] = _(
    void 0
    ...
    if (!O) return;
    const v = D.startsWith(`${O.book} `) ? D : "", q = na(v), lt = Ar(v);
    return {
      book: O.book,
      chapterNum: lt ?? O.chapterNum ?? 1,
      verseNum: q ?? O.verseNum ?? 1
    };
  }, [O, D]), qt = L(!1);
  Z(() => {
    if (!qt.current) {
  ...
  const It = V(() => {
    ...
    (v) => {
      const q = G ?? (O == null ? void 0 : O.book), lt = at ?? (O == null ? void 0 : O.chapterNum);
      !q || !lt || (Ot({
        book: q,
        chapterNum: lt,
        verseNum: v
    ...
    (v) => {
      const q = k === "chapters" ? I : O == null ? void 0 : O.book;
      if (q) {
        if (w && w(q, v) > 1) {
          $(q), ut(v), ot("verses"), W("");
          return;
  ...
  const { otLong: Tt, ntLong: Gt, dcLong: ar, extraLong: Ke } = {
    ...
    [t.book, s, U]
  ), gr = `${t.chapterNum}:${t.verseNum}`, T = V((v) => (q) => {
    me.current[v] = q;
  }, []), ct = V((v) => (q) => {
    Dt.current[v] = q;
  }, []), wt = P(
    ...
    (v) => {
      ...
      if (k === "books" && O && !dt && v.key === "Enter" && !(q !== Nt.current && !!(q != null && q.closest('button, a, input, select, textarea, [role="button"]')))) {
      ...
      if (v.shiftKey || k === "books" && q !== Nt.current && (q != null && q.closest('button, a, [role="button"]')))
        return;
      const St = (() => {
        const Bt = k === "books" && O && zt && O.chapterNum ? { bookId: O.book, chapterNum: O.chapterNum } : void 0, pe = k === "verses" ? { bookId: G, chapterNum: at } : Bt;
        if (pe) {
        ...
        if (k === "chapters" || k === "books" && O && Ge(O.book) > 1) {
    ...
    const { isLetter: q, isDigit: lt } = mn(v.key);
    (q || lt) && (v.preventDefault(), z((St) => St + v.key), (Wt = Nt.current) == null || Wt.focus(), tt(!1));
  }, []);
  re(() => {
    const v = setTimeout(() => {
      if (M && k === "books" && Et.current && oe.current) {
        const q = Et.current, lt = oe.current, jt = lt.offsetTop, Wt = q.clientHeight, St = lt.clientHeight, ie = jt - Wt / 2 + St / 2;
        q.scrollTo({
          top: Math.max(0, ie),
          behavior: "smooth"
        }), W(mo(t.book));
  ...
  const rt = O ? `${O.book} ${O.chapterNum ?? ""} ${O.verseNum ?? ""} ${zt}` : "", Lt = L(""), ue = L(""), be = P(() => {
    ...
    const { book: v, chapterNum: q, verseNum: lt } = O;
    if (zt && q && w)
      return {
        max: w(v, q),
        initial: lt ?? (v === t.book && q === t.chapterNum ? t.verseNum : 1),
        parse: na,
        buildValue: (Wt) => pa(v, q, Wt)
      };
    const jt = Ge(v);
    if (!(jt <= 1))
      return {
  ...
  return re(() => {
    ...
    const { max: v, initial: q, parse: lt, buildValue: jt } = be, Wt = (St) => {
      const ie = lt(St);
      return ie !== void 0 && ie >= 1 && ie <= v && St === jt(ie) ? ie : void 0;
    };
    if (Lt.current !== rt) {
      Lt.current = rt;
      const St = jt(Math.min(Math.max(q, 1), v));
      ue.current = St, W(St);
      return;
    }
    if (Wt(D) !== void 0) {
    ...
    W(
      Wt(ue.current) !== void 0 ? ue.current : jt(Math.min(Math.max(q, 1), v))
    );
            ...
            children: [
                ...
                "div",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ a(Oa, { className: "tw:translate-y-px tw:gap-1 tw:pe-2", children: Q.map(
                      ({ onClick: v, disabled: q, title: lt, icon: jt, group: Wt }, St) => {
                        const ie = /* @__PURE__ */ a(
                          et,
                          {
                            variant: "ghost",
              ...
              !dt && /* @__PURE__ */ u(ur, { ref: Et, children: [
                k === "books" && /* @__PURE__ */ u(gt, { children: [
                  ...
                  O && !zt && Ge(O.book) > 1 && // No heading here either, for the same reason as the verse preview above:
...
const Zw = {
  ...
  var gr;
  const [ot, I] = _({}), X = z ?? ot, G = X.editorState ?? Zw, $ = X.assignedUser, at = k !== void 0, ut = L(X);
  ut.current = X;
  const dt = V(
    (T) => {
  ...
  const Ke = V(
    ...
    async (T) => {
      const ct = de(G) ? Sa(G) : void 0, wt = T.status ? T.assignedUser : $ ?? T.assignedUser, zt = await h({
        ...T,
        contents: ct,
        assignedUser: wt
      });
  ...
  const Ne = !Nt || !g || g.length === 0 || !g.includes(c), hr = !de(G) && ($ === void 0 || $ === pt), Zr = /* @__PURE__ */ a(
    Pn,
    {
      comment: It,
      localizedStrings: o,
          ...
          n && /* @__PURE__ */ u(gt, { children: [
            ...
            E !== !1 && (!qt || de(G)) && /* @__PURE__ */ a(
              "div",
              {
                ...
                onKeyDownCapture: (T) => {
                  Wo(T) && (T.preventDefault(), T.stopPropagation(), (de(G) || $ !== void 0 && $ !== pt) && Ke());
                },
                onKeyDown: (T) => {
...
function Bh({
  ...
  const [g, E] = _("generated"), [y, C] = _("generated"), [N, A] = _("*"), [B, U] = _("*"), [M, S] = _("f"), [D, W] = _(!1), [Y, z] = _(!0), [k, ot] = _(!1), I = L(!1), X = L(""), [G, $] = _(!1), [at, ut] = _(), [dt, tt] = _(), [pt, kt] = _(), [mt, Rt] = _(), Ft = L(null), Pt = L(
    void 0
...
function Xh({
  ...
  }, [U, ot]), X = P(() => I.length === 0 ? [] : ju(I, S, W), [I, S, W]), G = P(() => {
    var kt, mt;
    if (!U) return d;
...
function pg({
  ...
  ] }), mt = e ? pt.filter((j) => e.includes(j.value)) : pt, Rt = g ?? Ga, Ft = h ?? Rt, Pt = f ?? Rt, fe = () => {
  ...
  ), Mt = mt.find((j) => j.value === "selectedBooks"), qt = mt.find((j) => j.value === "range"), [It, R] = _(!1), [K, J] = _(void 0), [H, Q] = _(void 0), [F, nt] = _(void 0), [st, it] = _(void 0), [vt, Ct] = _([]), Tt = p === "dropdown" && K === "selectedBooks", Gt = /* @__PURE__ */ a(
    wp,
    {
      availableBookInfo: o,
  ...
  return /* @__PURE__ */ u("div", { id: w, className: "tw:grid tw:gap-4", children: [
    ...
    p === "dropdown" && qt && /* @__PURE__ */ a(io, { open: K === "range", onOpenChange: vr, children: /* @__PURE__ */ a(
      so,
      {
        ref: Ba,
        onCloseAutoFocus: ra,
