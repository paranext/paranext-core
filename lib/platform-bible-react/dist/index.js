...
function qs({
  ...
  const o = J(), [s, i] = A(!1), [w, u] = A(""), [c, m] = A(""), [b, h] = A("books"), [f, x] = A(void 0), _ = U(void 0), y = U(void 0), C = U(void 0), N = U(void 0), D = U({}), O = H(() => a ? a() : Gn, [a]), K = H(() => ({
    ...
    return ["OT", "NT", "DC", "Extra"].forEach((Q) => {
      k[Q] = K[Q].filter((Y) => wt[Y].toLowerCase().includes(v) || Y.toLowerCase().includes(v));
    }), k;
  }, [K, c]), S = I(
    (v) => {
      if (!v.trim()) return;
    ...
    if (t.chapterNum > 1)
      ...
      if (v > 0) {
        const k = G[v - 1], P = Math.max(bt(k), 1);
        e({
          book: k,
          chapterNum: P,
          verseNum: 1
  ...
  }, [f, g == null ? void 0 : g.book, t.book, t.chapterNum]), qt = I((v, k) => `${v} ${wt[v]}${k ? ` ${k}` : ""}`, []), un = I((v) => (k) => {
    D.current[v] = k;
  }, []), Sr = I((v) => {
  ...
  return hn(() => {
    const v = setTimeout(() => {
      if (s && b === "books" && C.current && N.current) {
        const k = C.current, P = N.current, Q = P.offsetTop, Y = k.clientHeight, dt = P.clientHeight, st = Q - Y / 2 + dt / 2;
        k.scrollTo({
          top: Math.max(0, st),
          behavior: "smooth"
        }), u(qt(t.book));
...
function ci({
  ...
  const c = vt(n, "%webView_inventory_all%"), m = vt(n, "%webView_inventory_approved%"), b = vt(n, "%webView_inventory_unapproved%"), h = vt(n, "%webView_inventory_unknown%"), f = vt(n, "%webView_inventory_scope_currentBook%"), x = vt(n, "%webView_inventory_scope_chapter%"), _ = vt(n, "%webView_inventory_scope_verse%"), y = vt(n, "%webView_inventory_filter_text%"), C = vt(
    n,
    "%webView_inventory_show_additional_items%"
