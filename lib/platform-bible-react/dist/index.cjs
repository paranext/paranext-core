...
The color object needs to have a \`main\` property or a \`${T}\` property.`:$t(11,m?` (${m})`:"",T));if(typeof h.main!="string")throw new Error(process.env.NODE_ENV!=="production"?`MUI: The color${m?` (${m})`:""} provided to augmentColor(color) is invalid.
...
`))}return{top:oe.top+di(oe,u.vertical),left:oe.left+pi(oe,u.horizontal)}},[c,u.horizontal,u.vertical,d,f]),Q=E.useCallback(J=>({vertical:di(J,g.vertical),horizontal:pi(J,g.horizontal)}),[g.horizontal,g.vertical]),Z=E.useCallback(J=>{const P={width:J.offsetWidth,height:J.offsetHeight},oe=Q(P);if(f==="none")return{top:null,left:null,transformOrigin:fi(oe)};const ye=D();let ke=ye.top-oe.vertical,me=ye.left-oe.horizontal;const lt=ke+P.height,Oe=me+P.width,We=Mt(Rn(c)),Re=We.innerHeight-m,Ge=We.innerWidth-m;if(m!==null&&ke<m){const xe=ke-m;ke-=xe,oe.vertical+=xe}else if(m!==null&&lt>Re){const xe=lt-Re;ke-=xe,oe.vertical+=xe}if(process.env.NODE_ENV!=="production"&&P.height>Re&&P.height&&Re&&console.error(["MUI: The popover component is too tall.",`Some part of it can not be seen on the screen (${P.height-Re}px).`,"Please consider adding a `max-height` to improve the user-experience."].join(`
`)),m!==null&&me<m){const xe=me-m;me-=xe,oe.horizontal+=xe}else if(Oe>Ge){const xe=Oe-Ge;me-=xe,oe.horizontal+=xe}return{top:`${Math.round(ke)}px`,left:`${Math.round(me)}px`,transformOrigin:fi(oe)}},[c,f,D,Q,m]),[O,M]=E.useState(T),V=E.useCallback(()=>{const J=W.current;if(!J)return;const P=Z(J);P.top!==null&&(J.style.top=P.top),P.left!==null&&(J.style.left=P.left),J.style.transformOrigin=P.transformOrigin,M(!0)},[Z]);E.useEffect(()=>(I&&window.addEventListener("scroll",V),()=>window.removeEventListener("scroll",V)),[c,I,V]);const X=(J,P)=>{j&&j(J,P),V()},F=()=>{M(!1)};E.useEffect(()=>{T&&V()}),E.useImperativeHandle(l,()=>T?{updatePosition:()=>{V()}}:null,[T,V]),E.useEffect(()=>{if(!T)return;const J=Ii(()=>{V()}),P=Mt(c);return P.addEventListener("resize",J),()=>{J.clear(),P.removeEventListener("resize",J)}},[c,T,V]);let U=C;C==="auto"&&!S.muiSupportAuto&&(U=void 0);const H=b||(c?Ee(Rn(c)).body:void 0),G=(o=x==null?void 0:x.root)!=null?o:of,q=(i=x==null?void 0:x.paper)!=null?i:$s,K=vt({elementType:q,externalSlotProps:k({},z,{style:O?z.style:k({},z.style,{opacity:0})}),additionalProps:{elevation:h,ref:L},ownerState:_,className:we(R.paper,z==null?void 0:z.className)}),Y=vt({elementType:G,externalSlotProps:(w==null?void 0:w.root)||{},externalForwardedProps:B,additionalProps:{ref:n,slotProps:{backdrop:{invisible:!0}},container:H,open:T},ownerState:_,className:we(R.root,v)}),{slotProps:re}=Y,N=ue(Y,nf);return y.jsx(G,k({},N,!Hi(G)&&{slotProps:re,disableScrollLock:I},{children:y.jsx(S,k({appear:!0,in:T,onEntering:X,onExited:F,timeout:U},A,{children:y.jsx(q,k({},K,{children:p}))}))}))});process.env.NODE_ENV!=="production"&&(Ms.propTypes={action:jr,anchorEl:Bt(s.oneOfType([Ye,s.func]),e=>{if(e.open&&(!e.anchorReference||e.anchorReference==="anchorEl")){const t=Rn(e.anchorEl);if(t&&t.nodeType===1){const n=t.getBoundingClientRect();if(process.env.NODE_ENV!=="test"&&n.top===0&&n.left===0&&n.right===0&&n.bottom===0)return new Error(["MUI: The `anchorEl` prop provided to the component is invalid.","The anchor element should be part of the document layout.","Make sure the element is present in the document or that it's not display none."].join(`
