...
`}({indent:b,isSelected:C,node:h,nodeKeyDisplay:v,selection:f,typeDisplay:N})}),f===null?": null":g.$isRangeSelection(f)?function(h){let b="";const v=Fd(h);b+=`: range ${v!==""?`{ ${v} }`:""} ${h.style!==""?`{ style: ${h.style} } `:""}`;const N=h.anchor,C=h.focus,_=N.offset,S=C.offset;return b+=`
  ├ anchor { key: ${N.key}, offset: ${_===null?"null":_}, type: ${N.type} }`,b+=`
  └ focus { key: ${C.key}, offset: ${S===null?"null":S}, type: ${C.type} }`,b}(f):mf.$isTableSelection(f)?function(h){return`: table
