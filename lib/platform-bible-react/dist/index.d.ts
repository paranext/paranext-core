...
import { DeltaOpInsertNoteEmbed, EditorOptions, EditorRef, MarkerMenuItem as EditorMarkerMenuItem } from '@eten-tech-foundation/platform-editor';
 ...
 * decides for itself, and the attribute scales nothing, so never put it on a marked element. Mirrors
 * `CONTENT_ZOOM_SCOPE_ATTRIBUTE` in paranext-core's `src/shared/models/web-view.model.ts`; the
 * literal is duplicated here for the same reason as {@link CONTENT_ZOOM_ROOT_ATTRIBUTE}, and a
 ...
 * {@link CONTENT_ZOOM_ROOT_ATTRIBUTE}, it makes the platform's zoom indicator read
 * `<label> · <level>` (for example `HSV · 120 %`) instead of the level alone. `ContentZoomRoot`
 * writes it from its `label` prop. Mirrors `CONTENT_ZOOM_LABEL_ATTRIBUTE` in paranext-core's
...
type ScopeSelectorVariant = "radio" | "dropdown";
...
/** Renders a list of checkboxes. Each checkbox corresponds to an item from the `listItems` array. */
export declare const CheckboxGroup: typeof Checklist;
