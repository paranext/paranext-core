...
// vi.mock calls are hoisted to the top of the file by Vitest, so they run before any of the
// imports above at runtime even though they appear below them in source order.
...
describe('add-shadcn-component main()', () => {
  ...
  beforeEach(() => {
    ...
    // execSync's overloaded type returns Buffer or string depending on encoding option; the
    // production code passes encoding: 'utf8' and calls .trim() on the result, so the mock must
    // return a string. The cast satisfies the union return type without changing runtime behavior.
  ...
  it('exits 1 when npx shadcn add fails', () => {
    ...
    vi.mocked(execFileSync).mockImplementation((_cmd, args) => {
      if (Array.isArray(args) && args.includes('add')) throw new Error('shadcn failed');
      return Buffer.from('');
    });
    expect(() => main()).toThrow('process.exit');
    expect(exitSpy).toHaveBeenCalledWith(1);
  ...
  it('still runs format and lint-fix even when npx shadcn add fails', () => {
    ...
    expect(calls.some((c) => Array.isArray(c[1]) && c[1].includes('format'))).toBe(true);
    expect(calls.some((c) => Array.isArray(c[1]) && c[1].includes('lint-fix'))).toBe(true);
  });
  ...
  it('commits with the proper message containing component, preset, and sentinel on full success', () => {
    ...
    if (!Array.isArray(commitArgs)) throw new Error('commit args not an array');
  ...
  it('exits 1 with manual-commit instructions when the auto-commit fails', () => {
    process.argv = ['node', 'add-shadcn-component.ts', 'progress', 'my-preset'];
    vi.mocked(execFileSync).mockImplementation((_cmd, args) => {
      if (Array.isArray(args) && args.includes('commit')) throw new Error('commit failed');
      return Buffer.from('');
    });
    expect(() => main()).toThrow('process.exit');
    expect(exitSpy).toHaveBeenCalledWith(1);
