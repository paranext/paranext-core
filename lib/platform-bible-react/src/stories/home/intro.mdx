...
**With a live component preview:**

Pass a rendered component to the `preview` prop. It appears in a framed stage above the description text. Wrap two blocks in `ExampleBlockGroup` for a responsive side-by-side layout (two columns on medium+ screens, stacked on mobile).
