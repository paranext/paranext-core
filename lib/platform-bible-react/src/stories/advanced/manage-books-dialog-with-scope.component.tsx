 ...
 * The "with-scope" variant is the SelectionFirst-with-scope direction Sebastian explicitly said
 * to ignore for implementation; kept in the repo as a learning reference for the deferred
 * scope-aware View direction (see FN-008). Lint compliance is not pursued. */
...
export type ManageBooksDialogProps = {
  ...
  /**
   * Current project scope — the set of books planned for this project. When provided, the View mode
   * switches to scope-focused behaviors: the toggle label changes to "Scope", the list is grouped
   * by OT/NT/DC/Extra, each row gets a Missing/Unplanned badge relative to the scope, and the
   * delete per-row icon is replaced by a "remove from scope" action.
   */
  scope?: ManageBooksScope;
  /** Called when the user adds books to the scope via the footer button. */
  onAddBooksToScope?: (args: { projectId: string; books: string[] }) => void | Promise<void>;
  /** Called when the user removes a single book from the scope. */
  onRemoveBookFromScope?: (args: { projectId: string; book: string }) => void | Promise<void>;
};
...
/**
 * Combobox-only variant of the BookSelector pattern: a searchable list of books with checkboxes and
 * an explicit "Add" button. Unlike the shared BookSelector it has no section toggle buttons and no
 * selected-book badges — just the picker — and it never applies directly. The caller receives the
 * staged selection only when the user presses Add.
 *
 * When `fitsScope` is provided the list is split into a "Matching scope" group first, followed by
 * an "Unplanned (out of project scope)" group so the user can see the expected books up top.
 */
function BookSelectorCombobox({
  ...
  /**
   * Optional tooltip shown on the Add button. When a function is passed, it receives the current
   * staged count so the title can reflect it (e.g. "Add 3 books to project scope of WEB").
   */
  addTooltip?:
      ...
      <Command>
        ...
        <CommandList className="tw-max-h-72">
          ...
          {fitsScope ? (
            <>
              {matching.length > 0 && (
                <CommandGroup heading="Matching scope">{matching.map(renderItem)}</CommandGroup>
              )}
              {nonMatching.length > 0 && (
                <CommandGroup heading="Out of scope">{nonMatching.map(renderItem)}</CommandGroup>
              )}
            </>
          ) : (
      ...
      <div className="tw-border-t tw-p-2">
        {(() => {
          const addButton = (
            ...
            >
              {count === 0 ? 'Add' : `Add ${count} book${count === 1 ? '' : 's'}`}
            </Button>
          );
          if (!addTooltip) return addButton;
          const tooltip = typeof addTooltip === 'function' ? addTooltip(count) : addTooltip;
          return (
            <Tooltip>
              ...
              <TooltipContent side="top">
                ...
                {tooltip.description && (
                  <div className="tw-text-xs tw-text-muted-foreground">{tooltip.description}</div>
                )}
              </TooltipContent>
            </Tooltip>
          );
...
export function ManageBooksDialogWithScope({
  ...
  const scopeSet = useMemo(() => (scope ? new Set(scope.bookIds) : undefined), [scope]);
  ...
  const [namesImportSourceId, setNamesImportSourceId] = useState<string | undefined>(undefined);
  ...
  const [copyStateFilter, setCopyStateFilter] = useState<'all' | 'missing' | 'existing'>('all');
  const [importPresenceFilter, setImportPresenceFilter] = useState<'all' | 'missing' | 'existing'>(
    'all',
  );
  ...
  const [pendingScrollTo, setPendingScrollTo] = useState<string | undefined>(undefined);
  ...
  const scopeAvailableBookInfo = useMemo(
    () =>
      Canon.allBookIds
        .map((bookId) => (scopeSet?.has(bookId) || current.present.has(bookId) ? '0' : '1'))
        .join(''),
    [scopeSet, current],
  );
    ...
    switch (action) {
      case 'view':
        if (scopeSet) {
          ...
          const canonical = allBooks.filter((b) => scopeSet.has(b) || current.present.has(b));
          const canonicalSet = new Set(allBooks);
          const extras = Canon.allBookIds.filter(
            (b) => !canonicalSet.has(b) && (scopeSet.has(b) || current.present.has(b)),
          );
  ...
  const actionFilteredBooks = useMemo<string[]>(() => {
    ...
    if (action === 'import' && importPresenceFilter !== 'all') {
      return universe.filter((b) =>
        importPresenceFilter === 'missing' ? !current.present.has(b) : current.present.has(b),
      );
    }
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const row = container.querySelector<HTMLElement>(`[data-book-id="${pendingScrollTo}"]`);
  ...
  const selectableVisibleBooks = useMemo<string[]>(() => {
    if (action === 'view') {
      ...
      return visibleBooks.filter((b) => scopeSet.has(b) || current.present.has(b));
    }
  ...
  const untrackEligible =
    selectedArr.length > 0 && !!scopeSet && selectedArr.every((b) => scopeSet.has(b));
  ...
  const handleAddBooksToScope = useCallback(
    async (books: string[]) => {
      ...
      const touchedCategories = new Set<BookCategory>(books.map((b) => classifyBook(b)));
      ...
      const label =
        books.length === 1 ? `Added ${books[0]} to scope` : `Added ${books.length} books to scope`;
      sonner.success(label, {
        ...
        action: {
          label: 'Undo',
          onClick: () => {
            books.forEach((book) => onRemoveBookFromScope?.({ projectId, book }));
          },
        },
      });
    },
  ...
  const handleRemoveBooksFromScope = useCallback(
    async (books: string[]) => {
      if (books.length === 0) return;
      await Promise.all(
        books.map((book) => Promise.resolve(onRemoveBookFromScope?.({ projectId, book }))),
      );
      const label =
        books.length === 1
  ...
  const renderBookRow = (book: string) => {
    ...
    const fitsScope = scope ? doesBookFitScope(PROJECT_SCOPES[scope.id], book) : true;
    const showCheckbox =
      action === 'create' ||
      action === 'delete' ||
      action === 'copy' ||
      ...
      (action === 'view' && isScopeView && viewPresenceFilter !== 'all' && (inScope || isPresent));
    ...
    const isScopeMismatch = showStatusBadges && !fitsScope && (inScope || isPresent);
    const statusBadges: {
    ...
    return (
      ...
      >
        ...
        {showCheckboxPlaceholder && <span className="tw-h-4 tw-w-4 tw-shrink-0" aria-hidden />}
        <div className="tw-flex tw-min-w-0 tw-flex-1 tw-items-baseline tw-gap-2">
          ...
          {statusBadges
            ...
            .map((badge) => (
              <Badge key={badge.label} variant={badge.variant} className="tw-font-normal">
                {badge.label}
              </Badge>
            ))}
        </div>
  ...
  const groupedVisibleBooks = useMemo(() => {
    ...
    if (!(action === 'view' && isScopeView && viewPresenceFilter === 'all')) return undefined;
  ...
  const [collapsedCategories, setCollapsedCategories] = useState<Set<BookCategory>>(
    () => new Set(),
  );
            ...
            <div className="tw-flex tw-h-full tw-min-h-0 tw-flex-col">
              ...
              <div className="tw-flex tw-flex-col tw-items-start tw-gap-2 tw-border-b tw-px-6 tw-py-3 tw-@container/actions">
                ...
                >
                  ...
                  <ToggleGroupItem
                    ...
                    className="tw-ml-[-1px] tw-h-9 tw-gap-1.5 !tw-rounded-none tw-px-3 data-[state=on]:tw-z-10 data-[state=on]:!tw-bg-primary data-[state=on]:!tw-text-primary-foreground"
                  >
                    <Copy className="tw-hidden tw-h-4 tw-w-4 @lg/actions:tw-inline" aria-hidden />
                    Copy
                  </ToggleGroupItem>
                  <ToggleGroupItem
                    value="delete"
                  ...
                  >
                    <Trash2 className="tw-hidden tw-h-4 tw-w-4 @lg/actions:tw-inline" aria-hidden />
                    Delete
                  </ToggleGroupItem>
                </ToggleGroup>
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="tw-max-w-xs">
                            ...
                            <div className="tw-mt-1 tw-text-xs tw-text-muted-foreground">
                              Describes the intended set of books in the registry. The list below
                              shows what&apos;s actually planned and created — they may differ.
                            </div>
                            <div className="tw-mt-1 tw-text-xs tw-text-muted-foreground">
                ...
                {action === 'names' && (
                  <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setNamesImportSourceId(v || undefined)}
                    >
                      <SelectTrigger id="af-names-import" className="tw-h-8 tw-w-44">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const src = projects.find((p) => p.id === namesImportSourceId);
                            const name = src?.name ?? namesImportSourceId;
                            sonner.success(`Overwrote book names from ${name}`, {
                              duration: Infinity,
                              closeButton: true,
                              cancel: {
                                label: 'Undo',
                                onClick: () => {
                                  sonner.info('Reverted book names to the previous values');
                                },
                              },
                              action: {
                                label: 'Dismiss',
                      ...
                      <TooltipContent>Import and overwrite booknames</TooltipContent>
                    </Tooltip>
                  </div>
                )}
              ...
              <div className="tw-flex tw-flex-nowrap tw-items-center tw-gap-2 tw-border-b tw-px-6 tw-py-2 tw-@container/filterbar">
                {((action === 'view' && isScopeView) ||
                  ...
                  (() => {
                    ...
                    // source has no books).
                    const isScopeAll =
                      action === 'view' && isScopeView && viewPresenceFilter === 'all';
                    if (action === 'copy' && selectableVisibleBooks.length === 0) return undefined;
                    const checkbox = (
                      <Checkbox
                        id="af-sel-all"
                        checked={headerSelectState}
                        disabled={selectableVisibleBooks.length === 0 || isScopeAll}
                ...
                {action === 'import' &&
                  (() => {
                    ...
                    const hasMissing = picked.some((b) => !current.present.has(b));
                    const hasExisting = picked.some((b) => current.present.has(b));
              ...
              {action === 'view' &&
                ...
                (() => {
                  ...
                  if (registryScope.matchesScope(scope.bookIds)) return undefined;
                  return (
                    <div className="tw-flex tw-items-center tw-gap-2 tw-border-b tw-px-6 tw-py-2 tw-text-xs tw-text-muted-foreground">
                      <Info className="tw-h-3.5 tw-w-3.5 tw-shrink-0" aria-hidden />
                      <span>{`Planned books don't yet satisfy "${scope.name}".`}</span>
                    </div>
                  );
                })()}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {visibleBooks.map((book) => {
                        ...
                        return (
                          <TableRow key={book}>
                            <TableCell className="tw-font-medium">{book}</TableCell>
                    ...
                    {BOOK_CATEGORY_ORDER.map((cat) => {
                      ...
                      return (
                        <div key={cat}>
                          ...
                          >
                            {collapsed ? (
                              <ChevronRight className="tw-h-3.5 tw-w-3.5" aria-hidden />
                            ) : (
                              <ChevronDown className="tw-h-3.5 tw-w-3.5" aria-hidden />
                            )}
                            <span>{`${BOOK_CATEGORY_LABELS[cat]} (${books.length})`}</span>
                          </button>
                          {!collapsed && (
                ...
                <div className="tw-flex tw-items-center tw-gap-2">
                    ...
                    <>
                      <Popover open={addScopeOpen} onOpenChange={setAddScopeOpen}>
                        <Tooltip>
                          ...
                          <TooltipContent>Add new books to progress tracking</TooltipContent>
                        </Tooltip>
                        <PopoverContent align="end" side="top" className="tw-w-auto tw-p-0">
                          <BookSelectorCombobox
                            ...
                            addTooltip={(count) => ({
                              ...
                              description: 'CAUTION: This will change progress',
                            })}
                            fitsScope={
                              scope
                                ? (b) => doesBookFitScope(PROJECT_SCOPES[scope.id], b)
                                : undefined
                            }
                            onAdd={async (books) => {
                      ...
                      {(() => {
                        ...
                        const showUntrack =
                          viewPresenceFilter === 'existing' || viewPresenceFilter === 'missing';
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span>
                                    ...
                                    >
                                      <Plus className="tw-mr-1.5 tw-h-4 tw-w-4" aria-hidden />
                                      Track
                                    </Button>
                                  </span>
                                </TooltipTrigger>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span>
                                    ...
                                    >
                                      <Minus className="tw-mr-1.5 tw-h-4 tw-w-4" aria-hidden />
                                      Untrack
                                    </Button>
                                  </span>
                                </TooltipTrigger>
                  ...
                  {action === 'names' && (
                    <Tooltip>
                      ...
                      <TooltipContent>Scripture reference settings</TooltipContent>
                    </Tooltip>
                  )}
                  {action !== 'view' &&
                    action !== 'names' &&
