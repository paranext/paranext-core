...
**Carve-out for info icon buttons:** a tooltip attached to an `Info` icon button (see
<LinkTo title="Guidelines/Providing Help">Providing Help</LinkTo>) may run to 1–2 sentences,
...
Render keyboard shortcut hints as keycaps with the `ShortcutKeys` component, not as plain text in
parentheses. It puts each key in its own `Kbd`, and the `Kbd` styling provides sufficient visual
distinction on its own — no parentheses needed.
...
<ExampleBlockGroup>
  ...
  <ExampleBlock variant="prefer" preview={<span>Undo <ShortcutKeys hint="⌘Z" /></span>} code={'Undo <ShortcutKeys hint="⌘Z" />'}>
    The keycaps provide visual distinction without extra punctuation.
  </ExampleBlock>
</ExampleBlockGroup>

For which symbol or word to use per key on each OS, and how to pick the spelling for the user's OS, see [Keyboard shortcuts](?path=/docs/guidelines-keyboard-shortcuts--docs#detecting-the-platform).
...
"Used to authenticate requests." But if the panel also includes a *Copy key* button,
that interaction belongs in a click-triggered popover, not a tooltip.

See also: <LinkTo title="Guidelines/Providing Help">Providing Help</LinkTo> — the full ladder
from label to online help, and where info icon buttons fit.
