...
Closable tabs can be closed with a middle click anywhere on the tab header, not just on its close button. This works on tab headers in the overflow menu too.

## Layout

Tabs can be rearranged by dragging and dropping them into the desired position. In Power mode, a dragged tab or tab group can also be dropped anywhere in the empty space of a tab header bar to append it as that bar's last tab.

Tabs can be floated by right clicking on the tab header and selecting `Float Tab` from the context menu.

Floating tabs can be docked back into the dock layout of the window they are floating above by dragging and dropping them back into the tab header bar.
