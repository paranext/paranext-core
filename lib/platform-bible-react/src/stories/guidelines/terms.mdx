...
- `Window`: An operating system window that contains some interface. Platform.Bible (and therefore Paratext 10 Studio) can have several of these open at once. They are equal siblings — none of them is a main or parent window, and each has its own dock layout.
...
- `Pane`: A region of a tab content with its own layout. Most tabs are a single pane; the Scripture Editor tab has a text pane and a footnote pane, separated by a divider. Content zoom ("Tab content default zoom" in Settings) scales the text in panes; "Interface scaling" scales the whole application.
