...
[Toolbars](#toolbar) are the exception: they do not hide items at all. Their items shrink and truncate instead.
...
In narrow widths use icon-only buttons; when width allows for, use icon and text.
Always use [Tooltips](https://paranext.github.io/paranext-core/platform-bible-react-storybook/?path=/docs/shadcn-tooltip--docs).

Buttons carrying **user-generated text** — a project name, a scripture reference — are the exception: shorten the text rather than falling back to an icon. A user recognises `TP1` far faster than a generic glyph.

Determine available width with the `useShrinkStep` hook rather than a CSS container query. Tailwind's container-query *variants* have repeatedly failed to be emitted into extension web view bundles, and they fail silently — the class is simply absent and the element keeps its base style at every width.
...
Toolbar items shrink and truncate. They are never hidden, never moved into an overflow menu, and never clipped away — at any width the app supports, every item is still there. The menu button is always visible.
...
#### Absorbing and rigid regions

Mark each region of a toolbar as one of two kinds:

- **Absorbing** — may shrink. Give it `tw:min-w-0`, which is what allows a flex item to go below its content width. Without it, `min-width: auto` pins the region at its content's minimum and the space is taken out of its siblings instead — which is how items end up clipped away.
- **Rigid** — must not shrink. Give it `tw:shrink-0`. Icon-only buttons and menu triggers belong here: they have no shorter form to fall back to.

#### Stepped shrinking

Items carrying two fields shrink in steps, and the **second field is always the truncation target**:

1. Full label — `Genesis 1:1`
2. Abbreviated primary field — `GEN 1:1`
3. Second field clipped with an ellipsis — `GEN 1:…`
4. Second field dropped — `GEN`

Use `ToolbarCompoundLabel` for this; it takes the two fields and handles the ellipsis, the drop, and the tooltip. Steps 1, 2 and 4 come from the toolbar's `useShrinkStep` value; step 3 is plain CSS.

Reserve a **fixed slot** for anything whose content varies in length, so switching projects or references cannot shift the items beside it.

#### When the steps run out

Below the narrowest step a toolbar can still be given less room than its items need — a column dragged to its minimum inside a wide window reaches widths a window resize never does. What gives way then is ordered, and content is last:

1. **The toolbar's own chrome.** `TabToolbarContainer` halves its padding and inter-zone gaps at the narrowest step, which hands the space to the absorbing region.
2. **A control's decoration.** An affordance with no information in it — a dropdown chevron on a trigger that is already a bordered code — is dropped at that step. The control keeps its popover semantics and its accessible name, so nothing changes for keyboard or screen-reader users.
3. **Nothing else.** A field that is a code — a USFM marker, a canon id — has no shorter form, so a control whose label is down to one must stop shrinking rather than cut into it. Floor it at `min-w-min`, and only at the narrowest step: while a second field is still rendered it contributes its longest word to `min-content`, and a floor there stops the control shrinking at all, so it is clipped instead of ellipsising.
...
Tab Headers overflow into an overflow menu. Also see [Interactions/Tab Navigation](?path=/docs/guidelines-interactions--docs#tab-navigation)
