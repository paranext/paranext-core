...
## Design principle
Design narrow first, wide later. For tabs (technically "webviews") design for a `min-width` of `300px`, at which the ui works without vertical srcollbars.
Is this narrow view, parts of the ui may not be visible, truncated or wrapped.

Things that get hidden need to be accessible in another way. E.g. when hidden from the toolbar, it could still be accessible from a menu.
...
- For the time beeing there are currently no efforts to design popovers 
For  - e.g. project selection - menu entries should not wrap, but entries be truncated in small width.
