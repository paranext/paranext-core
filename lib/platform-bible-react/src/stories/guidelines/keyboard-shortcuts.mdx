...
For shortcuts rendered at runtime, use the `ShortcutKeys` component rather than branching on the user's OS and building a `KbdGroup` by hand:

```tsx
import { ShortcutKeys } from 'platform-bible-react';

<ShortcutKeys hint="Ctrl+Z" />
```
...
<ExampleBlockGroup>
  ...
  <ExampleBlock variant="prefer" preview={<ShortcutKeys hint="Ctrl+Z" />} code={'<ShortcutKeys hint="Ctrl+Z" />'}>
    Render the hint through <code>ShortcutKeys</code> instead of hand-building the group — it
    picks the per-OS keycaps and separator for you.
  </ExampleBlock>
</ExampleBlockGroup>
