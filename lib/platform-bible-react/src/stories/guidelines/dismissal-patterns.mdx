...
- **Restore focus on close.** When a surface closes, return focus to the element that opened it
  ...
  *outside* Radix's lifecycle, so it must call `trigger.focus()` on dismiss — otherwise keyboard
  ...
  keydown. For why the browser's own "focus without the indicator" option is not usable here, see
  the `adr-menu-close-focus-restores-without-the-ring` entry in
