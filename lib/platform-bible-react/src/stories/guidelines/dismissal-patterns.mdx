...
For _why_ "outside" is defined this way — and the alternatives we rejected (pointer-only, "any
iframe dismisses", pointer-enter scroll proxies) — see **ADR 0001**.
...
still dismiss them. We deliberately do **not** approximate this from pointer position; the reasoning
and the deferred platform capability are recorded in **ADR 0001**.
