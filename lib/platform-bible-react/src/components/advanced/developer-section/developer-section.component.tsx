...
/** The server environments this UI offers, in display order. */
const SERVER_ROWS = [
  {
    value: 'Production',
...
const isServerType = (value: string): value is ServerType =>
  SERVER_ROWS.some((row) => row.value === value);
    ...
    <div className="tw:border-t tw:pt-2">
      ...
      <div id={contentId} className="tw:mt-2 tw:px-2" hidden={!isExpanded}>
        ...
        <RadioGroup
          ...
          onValueChange={(v) => {
            if (isServerType(v)) onServerChange(v);
          }}
          disabled={disabled}
        >
          {SERVER_ROWS.map((row) => (
