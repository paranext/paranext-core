...
describe('book-chapter-control.navigation', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Set up default mock returns for fetchEndChapter
  ...
  describe('useQuickNavButtons', () => {
    ...
    describe('Previous chapter navigation', () => {
      ...
      test('Navigates to previous book last chapter when at chapter 1', () => {
        ...
        expect(mockHandleSubmit).toHaveBeenCalledWith({
          book: 'GEN',
          chapterNum: 50, // Last chapter of Genesis
          verseNum: 1,
        });
      });
    ...
    describe('Next chapter navigation', () => {
      ...
      test('Navigates to next book first chapter when at last chapter', () => {
        const { result } = renderHook(() =>
          useQuickNavButtons(
            { book: 'GEN', chapterNum: 50, verseNum: 1 }, // Last chapter of Genesis
            availableBooks,
            'ltr',
            mockHandleSubmit,
          ),
      ...
      test('Is disabled when at last chapter of last book', () => {
        const { result } = renderHook(() =>
          useQuickNavButtons(
            { book: 'REV', chapterNum: 22, verseNum: 1 }, // Last chapter of last book
            availableBooks,
            'ltr',
            mockHandleSubmit,
          ),
