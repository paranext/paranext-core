...
const SEARCH_QUERY_FORMATS = [
  /^(.+?)$/i, // Matches book name or id
  /^(.+?)(?:\s+(\d+))$/i, // Matches book name followed by a chapter number
  /^(.+?)(?:\s+(\d+):(\d+))$/i, // Matches book name followed by a chapter and verse number
];
    ...
    (event: ReactKeyboardEvent) => {
      ...
      if (document.activeElement === inputRef.current) {
        ...
        if (/^\d$/.test(event.key)) {
          ...
          const bookMatch = currentValue.match(/^(.+?)\s+\d+:\d+$/);
          if (bookMatch) {
        ...
        if (event.key === ':') {
          ...
          const bookAndChapterMatch = currentValue.match(/^(.+?\s+\d+):\d+$/);
