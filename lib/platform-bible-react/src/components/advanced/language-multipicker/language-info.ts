 ...
 * tags use it (`en`); where it doesn't, BCP-47 falls back to the 3-letter ISO 639-3 code (`grc` for
 * Koine Greek).
 *
 * The codes in this placeholder follow that same convention. When this component is wired to the
 * real platform-bible language source, it should accept whatever BCP-47 tag form the source
 * provides — this module is a _temporary local mock_ meant only to feed the prototype stories.
 *
 * # `LanguageInfo` shape
 *
 * Mirrors `LanguageInfo` from `ui-language-selector.component.tsx` (`autonym`, `otherNames`) so the
 * eventual system-wide language source can serve both components without translation.
 */
export interface LanguageInfo {
  /** Canonical English name. e.g. "German". */
  name: string;
  /**
   * Best-effort BCP-47 tag. Prefer 2-letter ISO 639-1 where available, fall back to 3-letter ISO
   * 639-3 where the language has no 639-1 code.
   */
  code: string;
  /** What the language calls itself, in its own script. e.g. "Deutsch". */
  autonym: string;
  /**
   * Alternative names the language is also known by. Searchable but not normally displayed — users
   * can search "Deutsche" or "Castellano" and find the right match.
   */
  otherNames?: string[];
}
