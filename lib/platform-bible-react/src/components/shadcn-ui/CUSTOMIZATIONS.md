...
- **`export { Toaster as Sonner, sonner }` statement** — A comment documents that `Toaster` is exported as `Sonner` to maintain the API names and the re-export of the `sonner` (i.e., `toast`) function was added manually to the export list.
