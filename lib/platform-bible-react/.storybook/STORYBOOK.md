...
- **Persisted theme key:** The toolbar selection is stored in `localStorage` as `platform-bible-storybook-theme` (see [theme-apply.ts](./theme-apply.ts)). An older ThemeProvider used `vite-ui-theme`; that value is not migrated, so the first load after updating may show the default theme until you choose a theme again (low impact; self-corrects on the next selection).
