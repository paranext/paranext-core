...
describe('Find USJ details for text searches', () => {
  ...
  test('search with verse text filtering excludes footnotes and cross-references', () => {
    ...
    // Import the verse text markers set (we'll need to add this import at the top)
    // For now, create a minimal set for testing - in a real scenario this would be the full set
    ...
    // Test 1: Search for "generaciones" which appears in verse 17 text AND in the footnote
  ...
  test('search with FULL verse text markers set excludes footnotes', () => {
    const usjDoc = new UsjReaderWriter(usjMat1, usjReaderWriterOptions3_1);

    // Create the full verse text markers set (same as production code)
    // This includes all 164 markers from usfm-verse-text-markers.ts
    const fullVerseTextMarkers = new Set([
      'add',
      'addpn',
      'b',
      'bk',
    ...
    // Search for "2 R. 24.14-15" which ONLY appears in footnote
    const footnoteOnlyText = /2 R\. 24\.14-15/g;
    ...
    // With FULL verse text markers - should NOT find it
    const verseTextMatches = usjDoc.search(footnoteOnlyText, fullVerseTextMarkers);
    expect(verseTextMatches.length).toBe(0);

    // Search for "generaciones" which appears in both verse text AND footnote
    const searchTermRegex = /generaciones/gi;
    const allGeneracionesMatches = usjDoc.search(searchTermRegex);
    const verseTextGeneracionesMatches = usjDoc.search(searchTermRegex, fullVerseTextMarkers);

    // Should find fewer with filtering
    expect(verseTextGeneracionesMatches.length).toBeLessThan(allGeneracionesMatches.length);
    expect(verseTextGeneracionesMatches.length).toBeGreaterThan(0);

    // Verify none of the verse text matches are in footnotes
    verseTextGeneracionesMatches.forEach((match) => {
      const { jsonPath } = match.start.documentLocation;
      expect(jsonPath).not.toContain('"marker":"f"');
      expect(jsonPath).not.toContain('"marker":"x"');
      expect(jsonPath).not.toContain('"marker":"fe"');
