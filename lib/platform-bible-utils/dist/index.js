...
class Ce {
  ...
  addOrUpdateContribution(e, t) {
    ...
    let s = this.options.copyDocuments && t ? O(t) : t;
...
const ce = 1e4, z = ce / 1e3, de = 3, ne = /* @__PURE__ */ new Map(), Ne = {
  unsubscribed: {
    ...
    suppressed: (i, e) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers arrived after the list was run in the last ${z}s and were unsubscribed immediately; per-occurrence warnings suppressed.`
  },
  failed: {
    log: (i) => console.error(i),
    first: (i) => `UnsubscriberAsyncList ${i}: Unsubscriber added after the list was run failed!`,
    suppressed: (i, e) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run failed in the last ${z}s; per-occurrence errors suppressed.`
  },
  threw: {
    log: (i) => console.error(i),
    ...
    suppressed: (i, e, t) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run threw in the last ${z}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${de}): ${t.join(" | ")}`
  }
};
function ie(i, e, t) {
...
const Ve = Math.ceil((3 - 0.5) / 0.1), ze = 0.1;
function Wt(i = {}) {
  const e = i.maxSteps ?? Ve, t = i.zoomStep ?? ze, r = i.window ?? globalThis.window, s = 120, n = 100, c = (v) => {
    const { wheelDeltaY: E } = v;
    return typeof E == "number" && Number.isFinite(E) ? -E / s : v.deltaY / n;
  }, p = (v) => v < 0 ? -Math.round(-v) : Math.round(v);
  let d = 0, l, m = 0;
  const b = 100, u = 0.05, h = 100, f = Math.log(1 + t) * b;
  let g = 0, M, I = 0, x = -1 / 0;
  const _ = ["Control", "Meta"], S = 2e3, T = /* @__PURE__ */ new Set(), $ = /* @__PURE__ */ new Set();
  let G = -1 / 0;
  const ye = (v) => T.size > 0 || $.size > 0 && v - G < S, Y = (v) => {
  ...
  r && (r.addEventListener("keydown", Y, !0), r.addEventListener("keyup", X, !0), r.addEventListener("pointerdown", R, !0), r.addEventListener("pointermove", R, !0), r.addEventListener("blur", V), r.document.addEventListener("visibilitychange", W));
  const ke = (v, E, P) => (
    ...
    E === M && P - x < h)
  ), Z = (v) => v < 0 ? -Math.min(-v, e) : Math.min(v, e), be = (v, E, P) => {
  ...
  return {
    ...
    dispose() {
      r && (r.removeEventListener("keydown", Y, !0), r.removeEventListener("keyup", X, !0), r.removeEventListener("pointerdown", R, !0), r.removeEventListener("pointermove", R, !0), r.removeEventListener("blur", V), r.document.removeEventListener("visibilitychange", W));
    }
  };
}
const K = {
...
`) && (n.length === 0 ? (p = p.map((u) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, r = !1) {
    ...
    return !r && !H(N.toPathArray(s), N.toPathArray(c)) ? !1 : H(n, p);
  }
...
export {
  ...
  Ur as getLocalizeKeysForScrollGroupIds,
