...
class Ce {
  ...
  constructor(e, t) {
    ...
    k(this, "latestOutput");
    k(this, "options");
    k(this, "onDidRebuildEmitter", new Te());
...
class lt {
  ...
  get(e) {
    ...
    return t || (t = new qe(), this.mutexesByID.set(e, t), t);
  }
  /**
...
class ht {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: je }, s = t.catch((n) => this.logger.warn(`Error in promise for ${e}: ${n.message}`)).finally(() => {
...
const ce = 1e4, H = ce / 1e3, de = 3, ne = /* @__PURE__ */ new Map(), Ne = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (i, e, t) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run threw in the last ${H}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${de}): ${t.join(" | ")}`
  }
};
function ie(i, e, t) {
  const { log: r, first: s, suppressed: n } = Ne[e], c = `${i}\0${e}`, p = Date.now(), d = ne.get(c);
  if (!d || p >= d.closesAt) {
    r(s(i, t)), ne.set(c, {
      closesAt: p + ce,
      suppressedCount: 0,
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < de && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
...
const gt = "ABORTED", yt = "ALREADY_EXISTS", kt = "CANCELLED", bt = "DATA_LOSS", vt = "DEADLINE_EXCEEDED", Mt = "FAILED_PRECONDITION", xt = "INTERNAL", _t = "INVALID_ARGUMENT", Et = "NOT_FOUND", It = "OUT_OF_RANGE", wt = "PERMISSION_DENIED", St = "RESOURCE_EXHAUSTED", Pt = "UNAUTHENTICATED", Tt = "UNAVAILABLE", Ct = "UNIMPLEMENTED", At = "UNKNOWN", D = 1;
function qt(i, e) {
  if (!i)
    return {
      message: "",
...
function Ot(i) {
  var e;
  return ((e = pe[i]) == null ? void 0 : e.type) === a.Paragraph || i === "v";
}
function Ut(i) {
  const e = pe[i];
...
function Ue(i, e) {
  const t = e && e.length > 0 ? e : le();
  return t[i % t.length];
}
function Rt(i, e) {
  const t = e && e.length > 0 ? e : le(), r = (() => {
...
function Vt() {
  return typeof navigator < "u" && navigator.languages ? navigator.languages[0].replace(/@posix$/i, "") : new Se().resolvedOptions().locale;
}
function zt(i, e = 2) {
...
const Le = 1e3, ue = 60, he = ue * 60, Be = he * 24;
function Jt(i, e, t = /* @__PURE__ */ new Date()) {
  const r = Math.floor((e.getTime() - t.getTime()) / Le), s = Math.round(r / Be);
  if (Math.abs(s) >= 1) return i.format(s, "day");
  const n = Math.round(r / he);
  if (Math.abs(n) >= 1) return i.format(n, "hour");
  const c = Math.round(r / ue);
...
function Wt(i = {}) {
  ...
  let Y = -1 / 0;
  const ye = (v) => T.size > 0 || $.size > 0 && v - Y < S, X = (v) => {
  ...
  const ke = (v, E, P) => (
    ...
    v.ctrlKey && !ye(P) && v.deltaMode === 0 && v.deltaX === 0 && (Math.abs(Math.exp(-v.deltaY / b) - 1) < u || // The running gesture belongs to the scope it is running in: a gesture that lands somewhere
    // else is a new one, and starts on its own evidence however close behind it arrives.
    E === M && P - x < h)
  ), z = (v) => v < 0 ? -Math.min(-v, t) : Math.min(v, t), be = (v, E, P) => {
  ...
  return {
    read(v, E) {
      ...
      return ke(v, E, P) ? be(v, E, P) : ve(v, E);
    },
    dispose() {
...
function Zt(i) {
  const e = {};
  return typeof i.language == "string" && i.language.length > 0 && (e[L.language] = i.language), typeof i.type == "string" && i.type.length > 0 && (e[L.type] = i.type), typeof i.typeName == "string" && i.typeName.length > 0 && (e[L.typeName] = i.typeName), typeof i.lastUsedAt == "number" && Number.isFinite(i.lastUsedAt) && (e[L.lastUsedAt] = i.lastUsedAt), e;
}
function Qt(i) {
...
function ge(i) {
  return Object.freeze(i), i == null || Object.getOwnPropertyNames(i).forEach(function(t) {
    ...
    i[t] !== null && (typeof i[t] == "object" || typeof i[t] == "function") && !Object.isFrozen(i[t]) && ge(i[t]);
  }), i;
}
const K = ge({
  version: "3.0.7",
...
USJ: ${JSON.stringify(
  ...
  static isTopLevelUsjMarker(e, t) {
    return typeof e == "object" && e.type === we && t.length === 0;
  }
  /**
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (l != null && l.markerTypeUsfm && (d = l.markerTypeUsfm, l = this.markersMap.markerTypes[d]), !w(e) && e.type !== c && (!l || e.type !== l.markerTypeUsfm && e.type !== l.markerTypeUsx && e.type !== l.markerTypeUsj) && (console.warn(
  ...
  static transferFragmentsInfoArrayToMaps(e, t, r, s, n, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        if (h.type === Oe && h.code)
        ...
        else if (h.type === se && h.number) {
          const f = parseInt(h.number, 10);
          if (Number.isNaN(f))
            console.warn(
              `Found ${se} type marker with number ${h.number}, but could not parse chapter number from it. Continuing using previous chapter number ${r.chapterNum}`
            );
          else {
          ...
          if (!f)
            ...
            ) : (u = (b = c[r.bookId]) == null ? void 0 : b[r.chapterNum]) != null && u[g] ? console.warn(`Found ${C} marker with existing number ${g} after
                  current ${C} number ${r.verseNum}! Not updating verse start index. All positions in this duplicate verse will be based on the current ${C} marker, not the new duplicate marker.`) : (g < r.verseNum && console.debug(
...
export {
  ...
  Se as DateTimeFormat,
  Ce as DocumentCombiner,
  ...
  le as getDefaultCallerSequence,
