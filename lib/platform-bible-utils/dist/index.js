...
var k = (n, e, t) => ce(n, typeof e != "symbol" ? e + "" : e, t);
import { d as C, i as _ } from "./scripture-util-DArajUVn.js";
import { D as Zt, F as Qt, a as er, b as tr, G as rr, L as nr, M as ir, S as sr, c as ar, e as or, f as cr, g as dr, h as pr, j as lr, k as ur, l as hr, m as fr, n as mr, o as gr, p as yr, q as kr, r as br, s as vr, t as Mr, u as xr, v as _r, w as Er, x as Ir, y as wr, z as Sr, A as Pr, B as Tr, C as Cr, E as Ar, H as qr, I as $r, J as jr, K as Nr, N as Or, O as Ur, P as Fr, Q as Rr, R as Dr, T as Lr, U as Br, V as Vr, W as zr, X as Hr, Y as Jr, Z as Kr, _ as Gr, $ as Yr, a0 as Xr, a1 as Wr, a2 as Zr, a3 as Qr, a4 as en, a5 as tn, a6 as rn, a7 as nn, a8 as sn, a9 as an, aa as on, ab as cn, ac as dn, ad as pn } from "./scripture-util-DArajUVn.js";
import { Mutex as de } from "async-mutex";
import pe from "dompurify";
import { deepEqual as le } from "fast-equals";
import { USJ_TYPE as ue } from "@eten-tech-foundation/scripture-utilities";
...
class ge {
  ...
  constructor(e, t) {
    ...
    k(this, "latestOutput");
    k(this, "options");
    k(this, "onDidRebuildEmitter", new me());
...
class tt {
  ...
  get(e) {
    ...
    return t || (t = new ke(), this.mutexesByID.set(e, t), t);
  }
  /**
...
class nt {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: ve }, s = t.catch((i) => this.logger.warn(`Error in promise for ${e}: ${i.message}`)).finally(() => {
...
const Z = 1e4, O = Z / 1e3, Q = 3, z = /* @__PURE__ */ new Map(), Me = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (n, e, t) => `UnsubscriberAsyncList ${n}: ${e} more unsubscribers added after the list was run threw in the last ${O}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${Q}): ${t.join(" | ")}`
  }
};
function H(n, e, t) {
  const { log: r, first: s, suppressed: i } = Me[e], c = `${n}\0${e}`, p = Date.now(), d = z.get(c);
  if (!d || p >= d.closesAt) {
    r(s(n, t)), z.set(c, {
      closesAt: p + Z,
      suppressedCount: 0,
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < Q && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
...
const at = "ABORTED", ot = "ALREADY_EXISTS", ct = "CANCELLED", dt = "DATA_LOSS", pt = "DEADLINE_EXCEEDED", lt = "FAILED_PRECONDITION", ut = "INTERNAL", ht = "INVALID_ARGUMENT", ft = "NOT_FOUND", mt = "OUT_OF_RANGE", gt = "PERMISSION_DENIED", yt = "RESOURCE_EXHAUSTED", kt = "UNAUTHENTICATED", bt = "UNAVAILABLE", vt = "UNIMPLEMENTED", Mt = "UNKNOWN", q = 1;
function xt(n, e) {
  if (!n)
    return {
      message: "",
...
const Et = (n) => (...e) => n.map((r) => r(...e)).every((r) => r), It = (n) => async (...e) => {
...
function wt(n) {
  var e;
  return ((e = ee[n]) == null ? void 0 : e.type) === a.Paragraph || n === "v";
}
function St(n) {
  const e = ee[n];
  return (e == null ? void 0 : e.type) !== a.Character ? !1 : e.category !== o.DivisionMarks;
}
const G = 0.5, Y = 3, xe = 0.1;
function _e(n) {
...
function Ie(n, e) {
  const t = e && e.length > 0 ? e : te();
  return t[n % t.length];
}
function Ct(n, e) {
  const t = e && e.length > 0 ? e : te(), r = (() => {
    const s = /* @__PURE__ */ new Map();
    let i = 0;
    return n.forEach((c, p) => {
      c.caller === "+" && (s.set(p, Ie(i, t)), i += 1);
    }), s;
  })();
  return (s, i) => {
    if (s === "+") {
...
function Se(n, e) {
  ...
  return Object.keys(t).forEach((i) => {
    s && (Object.hasOwn(r, i) && Se(r[i], t[i]) || (s = !1));
  }), s;
}
function X(n, e, t) {
...
const $t = (n) => n.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function jt() {
  return typeof navigator < "u" && navigator.languages ? navigator.languages[0].replace(/@posix$/i, "") : new he().resolvedOptions().locale;
}
function Nt(n, e = 2) {
...
function Te(n) {
  return !!qe(n.fullName) && n.fullName !== n.shortName;
}
const Ce = " - ";
function Ut(n) {
  return Te(n) ? `${n.shortName}${Ce}${n.fullName}` : n.shortName;
}
function Ae(n, e) {
  return n.localeCompare(e, void 0, { sensitivity: "base" });
}
function Ft(n, e) {
  return Ae(n.shortName, e.shortName);
}
function qe(n) {
  return typeof n == "string" && n.trim().length > 0 ? n : void 0;
}
const $e = 1e3, re = 60, ne = re * 60, je = ne * 24;
function Rt(n, e, t = /* @__PURE__ */ new Date()) {
  const r = Math.floor((e.getTime() - t.getTime()) / $e), s = Math.round(r / je);
  if (Math.abs(s) >= 1) return n.format(s, "day");
  const i = Math.round(r / ne);
  if (Math.abs(i) >= 1) return n.format(i, "hour");
  const c = Math.round(r / re);
...
function Gt(n, e, t) {
  ...
  return s.id = `${De}${t ? `-${t}` : ""}`, s.dataset.themeId = n.id, s.textContent = Be(n), this.document.head.appendChild(s), s;
}
function ae(n) {
  return Object.freeze(n), n == null || Object.getOwnPropertyNames(n).forEach(function(t) {
    ...
    n[t] !== null && (typeof n[t] == "object" || typeof n[t] == "function") && !Object.isFrozen(n[t]) && ae(n[t]);
  }), n;
}
const F = ae({
  version: "3.0.7",
...
class g {
  ...
  static isTopLevelUsjMarker(e, t) {
    return typeof e == "object" && e.type === ue && t.length === 0;
  }
  /**
  ...
  findAllNotes() {
    var e;
    return we((e = this.usj) == null ? void 0 : e.content);
  }
  /**
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const u = parseInt(b.colspan, 10), h = ze.exec(r);
      if (h != null && h[1]) {
  ...
  textContentToUsfm(e) {
    return {
      usfm: this.shouldAllowInvisibleCharacters ? e : e.replace(Ve, "~"),
    ...
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        if (h.type === J && h.code)
          ...
          const f = (m = He.exec(h.number)) == null ? void 0 : m[1];
          if (!f)
            this.reportProblemOnce(
...
export {
  ...
  Qe as Collator,
  dt as DATA_LOSS,
  pt as DEADLINE_EXCEEDED,
  Zt as DEBOUNCE_CANCELED_ERROR_MESSAGE,
  he as DateTimeFormat,
  ge as DocumentCombiner,
  et as EventRollingTimeCounter,
  lt as FAILED_PRECONDITION,
  Qt as FIRST_SCR_BOOK_NUM,
  er as FIRST_SCR_CHAPTER_NUM,
  tr as FIRST_SCR_VERSE_NUM,
  rr as GraphemeString,
  ut as INTERNAL,
  ht as INVALID_ARGUMENT,
  nr as LAST_SCR_BOOK_NUM,
  ir as MAX_PADDING_LENGTH,
  ...
  tt as MutexMap,
  ft as NOT_FOUND,
  rt as NonValidatingDocumentCombiner,
  be as NumberFormat,
  mt as OUT_OF_RANGE,
  gt as PERMISSION_DENIED,
  q as PLATFORM_ERROR_VERSION,
  Ce as PROJECT_NAME_SEPARATOR,
  $ as PROJECT_SELECTOR_CUSTOM_DATA_KEYS,
  me as PlatformEventEmitter,
  nt as PromiseChainingMap,
  yt as RESOURCE_EXHAUSTED,
  sr as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  ar as Section,
  V as SortedNumberMap,
  it as SortedSet,
  De as THEME_STYLE_ELEMENT_ID,
  kt as UNAUTHENTICATED,
  bt as UNAVAILABLE,
  vt as UNIMPLEMENTED,
  Mt as UNKNOWN,
  F as USFM_MARKERS_MAP_3_0,
  Yt as USFM_MARKERS_MAP_PARATEXT_3_0,
  st as UnsubscriberAsyncList,
  g as UsjReaderWriter,
  w as VERSE_TYPE,
  xe as ZOOM_STEP,
  Pt as adjustZoomFactor,
  It as aggregateUnsubscriberAsyncs,
  Et as aggregateUnsubscribers,
  Gt as applyThemeStylesheet,
  or as areUsjContentsEqualExceptWhitespace,
  cr as at,
  dr as charAt,
  _e as clampZoom,
  pr as codePointAt,
  lr as collapseMiddleWords,
  ur as collectUsjMarkers,
  Ae as compareProjectShortNames,
  Ft as compareProjectsByName,
  hr as compareScrRefs,
  Vt as computeEffectiveStructureProtection,
  ...
  gr as defaultScrRef,
  Pe as deserialize,
  Jt as doesCatalogRowCoverProject,
  yr as endsWith,
  kr as ensureArray,
  br as escapeStringRegexp,
  Kt as expandThemeContribution,
  Nt as formatBytes,
  Ut as formatProjectName,
  Dt as formatRelativeDate,
  vr as formatReplacementString,
  Mr as formatReplacementStringToArray,
  xr as formatScrRef,
  _r as formatScrRefRange,
  Rt as formatTimeSpan,
  Er as getAllObjectFunctionNames,
  Ir as getChaptersForBook,
  jt as getCurrentLocale,
  te as getDefaultCallerSequence,
  wr as getErrorMessage,
  Ct as getFormatCallerFunction,
  Bt as getLocalizeKeyForPhysicalKey,
  Sr as getLocalizeKeyForScrollGroupId,
  Pr as getLocalizeKeysForScrollGroupIds,
  Tr as getLocalizedIdFromBookNumber,
  Ie as getNthCaller,
  At as getPaneSizeLimits,
  Cr as getSectionForBook,
  Be as getStylesheetForTheme,
  Ar as groupBy,
  Te as hasDistinctFullName,
  $t as htmlEncode,
  qr as includes,
  $r as indexOf,
  wt as isBlockMarker,
  St as isCharacterMarker,
  jr as isErrorMessageAboutParatextBlockingInternetAccess,
  Nr as isErrorMessageAboutRegistryAuthFailure,
  Or as isLocalizeKey,
  _t as isPlatformError,
  Ur as isSelectableInvisibleCharOrWhiteSpace,
  qt as isSerializable,
  _ as isString,
  Se as isSubset,
  Fr as isWhiteSpace,
  Rr as isolateBidi,
  ...
  zt as makeProjectSelectorCustomData,
  Fe as menuDocumentSchema,
  Lr as newGuid,
  xt as newPlatformError,
  Br as normalize,
  qe as normalizeFullName,
  Ot as normalizeProjectId,
  Vr as normalizeScriptureSpaces,
  zr as offsetBook,
  Hr as offsetChapter,
  Jr as offsetVerse,
  Kr as ordinalCompare,
  Gr as padEnd,
  Yr as padStart,
  Ne as projectSettingsDocumentSchema,
  Ht as recencyMapFromOrderedIds,
  Xr as retryUntil,
  Ee as roundZoom,
  Tt as sanitizeHtml,
  Wr as scrRefToBBBCCC,
  Zr as scrRefToBBBCCCVVV,
  X as serialize,
  Oe as settingsDocumentSchema,
  Qr as slice,
  en as split,
  tn as startsWith,
  rn as stringLength,
  nn as substring,
  Re as themeDocumentSchema,
  sn as toArray,
  an as toKebabCase,
  on as transformAndEnsureRegExpArray,
  cn as transformAndEnsureRegExpRegExpArray,
  ee as usfmMarkers,
  dn as wait,
