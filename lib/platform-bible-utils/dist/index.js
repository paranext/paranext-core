...
var k = (i, e, t) => Ie(i, typeof e != "symbol" ? e + "" : e, t);
import { d as U, i as w } from "./scripture-util-DArajUVn.js";
import { D as ur, F as hr, a as fr, b as mr, G as gr, L as yr, M as kr, S as br, c as Mr, e as vr, f as xr, g as _r, h as Er, j as Ir, k as wr, l as Sr, m as Pr, n as Tr, o as Cr, p as Ar, q as qr, r as $r, s as jr, t as Nr, u as Or, v as Ur, w as Fr, x as Rr, y as Dr, z as Lr, A as Br, B as Vr, C as zr, E as Hr, H as Jr, I as Kr, J as Gr, K as Yr, N as Xr, O as Wr, P as Zr, Q as Qr, R as en, T as tn, U as rn, V as nn, W as sn, X as an, Y as on, Z as cn, _ as dn, $ as pn, a0 as ln, a1 as un, a2 as hn, a3 as fn, a4 as mn, a5 as gn, a6 as yn, a7 as kn, a8 as bn, a9 as Mn, aa as vn, ab as xn, ac as _n, ad as En } from "./scripture-util-DArajUVn.js";
import { Mutex as we } from "async-mutex";
import Se from "dompurify";
import { deepEqual as Pe } from "fast-equals";
import { USJ_TYPE as Te } from "@eten-tech-foundation/scripture-utilities";
...
class $e {
  ...
  constructor(e, t) {
    ...
    k(this, "latestOutput");
    k(this, "options");
    k(this, "onDidRebuildEmitter", new qe());
...
class kt {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: Ue }, s = t.catch((n) => this.logger.warn(`Error in promise for ${e}: ${n.message}`)).finally(() => {
...
const pe = 1e4, H = pe / 1e3, le = 3, se = /* @__PURE__ */ new Map(), Fe = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (i, e, t) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run threw in the last ${H}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${le}): ${t.join(" | ")}`
  }
};
function ae(i, e, t) {
  const { log: r, first: s, suppressed: n } = Fe[e], c = `${i}\0${e}`, p = Date.now(), d = se.get(c);
  if (!d || p >= d.closesAt) {
    r(s(i, t)), se.set(c, {
      closesAt: p + pe,
      suppressedCount: 0,
...
const vt = "ABORTED", xt = "ALREADY_EXISTS", _t = "CANCELLED", Et = "DATA_LOSS", It = "DEADLINE_EXCEEDED", wt = "FAILED_PRECONDITION", St = "INTERNAL", Pt = "INVALID_ARGUMENT", Tt = "NOT_FOUND", Ct = "OUT_OF_RANGE", At = "PERMISSION_DENIED", qt = "RESOURCE_EXHAUSTED", $t = "UNAUTHENTICATED", jt = "UNAVAILABLE", Nt = "UNIMPLEMENTED", Ot = "UNKNOWN", D = 1;
function Ut(i, e) {
  if (!i)
    return {
      message: "",
...
const Rt = (i) => (...e) => i.map((r) => r(...e)).every((r) => r), Dt = (i) => async (...e) => {
  const t = i.map(async (r) => r(...e));
  return (await Promise.all(t)).every((r) => r);
}, Re = "book", oe = "chapter", C = "verse", A = "***";
...
function Lt(i) {
  var e;
  return ((e = ue[i]) == null ? void 0 : e.type) === a.Paragraph || i === "v";
}
function Bt(i) {
  const e = ue[i];
  return (e == null ? void 0 : e.type) !== a.Character ? !1 : e.category !== o.DivisionMarks;
}
const J = 0.5, K = 3, he = 0.1;
function De(i) {
...
function Be(i, e) {
  const t = e && e.length > 0 ? e : fe();
  return t[i % t.length];
}
function Ht(i, e) {
  const t = e && e.length > 0 ? e : fe(), r = (() => {
    const s = /* @__PURE__ */ new Map();
    let n = 0;
    return i.forEach((c, p) => {
      c.caller === "+" && (s.set(p, Be(n, t)), n += 1);
    }), s;
  })();
  return (s, n) => {
    if (s === "+") {
...
function ze(i, e) {
  ...
  return Object.keys(t).forEach((n) => {
    s && (Object.hasOwn(r, n) && ze(r[n], t[n]) || (s = !1));
  }), s;
}
function ce(i, e, t) {
...
const Gt = (i) => i.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function Yt() {
...
const Ge = K - J, Ye = he;
function nr(i = {}) {
  const e = i.zoomStep ?? Ye, t = i.maxSteps ?? Math.ceil(Ge / e), r = i.window ?? globalThis.window, s = 120, n = 100, c = (M) => {
    const { wheelDeltaY: E } = M;
    return typeof E == "number" && Number.isFinite(E) && E !== 0 ? -E / s : M.deltaY / n;
  }, p = (M) => M < 0 ? -Math.round(-M) : Math.round(M);
  let d = 0, l, m = 0;
  const b = 100, u = 0.05, h = 100, f = Math.log(1 + e) * b;
  let g = 0, v, I = 0, x = -1 / 0;
  const _ = ["Control", "Meta"], S = 2e3, T = /* @__PURE__ */ new Set(), $ = /* @__PURE__ */ new Set();
  let W = -1 / 0;
  const Me = (M) => T.size > 0 || $.size > 0 && M - W < S, Z = (M) => {
    _.includes(M.key) && T.add(M.key);
  }, Q = (M) => {
    _.includes(M.key) && (T.delete(M.key), $.delete(M.key));
  }, R = (M) => {
    _.forEach((E) => {
      (E === "Control" ? M.ctrlKey : M.metaKey) ? $.add(E) : ($.delete(E), T.delete(E));
    }), W = performance.now();
  }, V = () => {
    T.clear(), $.clear();
  }, ee = () => {
  ...
  r && (r.addEventListener("keydown", Z, !0), r.addEventListener("keyup", Q, !0), r.addEventListener("pointerdown", R, !0), r.addEventListener("pointermove", R, !0), r.addEventListener("blur", V), r.document.addEventListener("visibilitychange", ee));
  const ve = (M, E, P) => (
    ...
    M.ctrlKey && !Me(P) && M.deltaMode === 0 && M.deltaX === 0 && (Math.abs(Math.exp(-M.deltaY / b) - 1) < u || // The running gesture belongs to the scope it is running in: a gesture that lands somewhere
    // else is a new one, and starts on its own evidence however close behind it arrives.
    E === v && P - x < h)
  ), z = (M) => M < 0 ? -Math.min(-M, t) : Math.min(M, t), xe = (M, E, P) => {
    const j = M.deltaY < 0 ? -1 : 1;
    (E !== v || j !== I) && (g = 0), v = E, I = j, g += M.deltaY, x = P;
  ...
  return {
    read(M, E) {
      if (M.deltaY === 0) return 0;
      if (M.deltaMode !== 0) return -z(M.deltaY < 0 ? -1 : 1);
      const P = performance.now();
      return ve(M, E, P) ? xe(M, E, P) : _e(M, E);
    },
    dispose() {
      r && (r.removeEventListener("keydown", Z, !0), r.removeEventListener("keyup", Q, !0), r.removeEventListener("pointerdown", R, !0), r.removeEventListener("pointermove", R, !0), r.removeEventListener("blur", V), r.document.removeEventListener("visibilitychange", ee));
    }
  };
}
const X = {
...
function cr(i, e, t) {
  ...
  return s.id = `${tt}${t ? `-${t}` : ""}`, s.dataset.themeId = i.id, s.textContent = nt(i), this.document.head.appendChild(s), s;
}
function be(i) {
  return Object.freeze(i), i == null || Object.getOwnPropertyNames(i).forEach(function(t) {
    ...
    i[t] !== null && (typeof i[t] == "object" || typeof i[t] == "function") && !Object.isFrozen(i[t]) && be(i[t]);
  }), i;
}
const Y = be({
  version: "3.0.7",
...
USJ: ${JSON.stringify(
    ...
    else if (y.areUsjVersionsCompatible(this.usj.version, Y.version))
      this.markersMap = Y;
    else
      throw new Error(
  ...
  static isTopLevelUsjMarker(e, t) {
    return typeof e == "object" && e.type === Te && t.length === 0;
  }
  /**
  ...
  findNextLocationOfMatchingText(e, t, r = 1e3) {
    ...
    if (y.findNextMatchingNodeUsingWorkingStack(
      ...
      (h, f) => {
        ...
        if (l && y.areStackItemsShallowEqual(v, l)) {
  ...
  search(e, t) {
    ...
    for (; m !== void 0; )
      m = y.findNextMatchingNodeUsingWorkingStack(
        ...
        // eslint-disable-next-line no-loop-func
        (g, v) => (typeof g != "string" || r && v.some((x) => {
          const _ = x.parent;
    ...
    for (; f; ) {
      if (f[0].length > 0) {
        const g = u ? u[f.index] : f.index, v = u ? u[f.index + f[0].length] : f.index + f[0].length;
        if (g < 0 || g >= b.length)
        ...
        }, _ = d.findClosestLessThanOrEqual(v - 1);
        if (!_)
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const u = parseInt(b.colspan, 10), h = st.exec(r);
      if (h != null && h[1]) {
...
`);
  ...
  closingMarkerToUsfm(e, t) {
    const {
      ...
      var g, v;
      return !(f === "type" || f === "marker" || f === "content" || f === "closed" || (g = p.skipOutputAttributeToUsfm) != null && g.includes(f) || (v = n.leadingAttributes) != null && v.includes(f) || n.textContentAttribute === f || d.some(
    ...
    if ((m.closed === "false" || n.isClosingMarkerOptional && !this.markersMap.shouldOptionalClosingMarkersBePresent && m.closed !== "true") && (b = !1), n.independentClosingMarkers && n.independentClosingMarkers.length > 0 && b) {
      ...
      let g = "";
      const v = [], { usfm: I, fragmentsInfo: x } = this.openingMarkerToUsfm(f, t), _ = x.find((S) => y.isFragmentAMarker(S.fragment));
      if (!_)
        throw new Error(
...
`) && (n.length === 0 ? (p = p.map((u) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, r = !1) {
    ...
    return !r && !G(N.toPathArray(s), N.toPathArray(c)) ? !1 : G(n, p);
  }
    ...
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        if (h.type === Re && h.code)
          ...
          const f = (m = at.exec(h.number)) == null ? void 0 : m[1];
          if (!f)
            console.warn(
  ...
  calculateUsfmProperties() {
    let e = "";
    const t = new ie(), r = /* @__PURE__ */ new Map(), s = {}, n = [], c = {
      bookId: A,
      chapterNum: 0,
      verseNum: 0
    };
...
export {
  ...
  $e as DocumentCombiner,
  mt as EventRollingTimeCounter,
  wt as FAILED_PRECONDITION,
  ...
  K as MAX_ZOOM_FACTOR,
  J as MIN_ZOOM_FACTOR,
  er as MODIFIER_KEYS,
  a as MarkerType,
  Ne as Mutex,
  gt as MutexMap,
  Tt as NOT_FOUND,
  yt as NonValidatingDocumentCombiner,
  Oe as NumberFormat,
  Ct as OUT_OF_RANGE,
  ...
  kt as PromiseChainingMap,
  qt as RESOURCE_EXHAUSTED,
  br as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  Mr as Section,
  ie as SortedNumberMap,
  bt as SortedSet,
  tt as THEME_STYLE_ELEMENT_ID,
  $t as UNAUTHENTICATED,
  jt as UNAVAILABLE,
  Nt as UNIMPLEMENTED,
  Ot as UNKNOWN,
  Y as USFM_MARKERS_MAP_3_0,
  dr as USFM_MARKERS_MAP_PARATEXT_3_0,
  Mt as UnsubscriberAsyncList,
  y as UsjReaderWriter,
  C as VERSE_TYPE,
  he as ZOOM_STEP,
  Vt as adjustZoomFactor,
  Dt as aggregateUnsubscriberAsyncs,
  Rt as aggregateUnsubscribers,
  ...
  Nr as formatReplacementStringToArray,
  Or as formatScrRef,
  ...
  fe as getDefaultCallerSequence,
  ...
  yn as stringLength,
  kn as substring,
  et as themeDocumentSchema,
  bn as toArray,
  Mn as toKebabCase,
  vn as transformAndEnsureRegExpArray,
