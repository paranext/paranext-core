...
import { D as Bt, F as Vt, a as zt, b as Ht, G as Jt, L as Kt, M as Gt, S as Yt, c as Xt, e as Wt, f as Zt, g as Qt, h as er, j as tr, k as rr, l as nr, m as ir, n as sr, o as ar, p as or, q as cr, r as dr, s as pr, t as lr, u as ur, v as hr, w as fr, x as mr, y as gr, z as yr, A as kr, B as br, C as vr, E as Mr, H as xr, I as _r, J as Er, K as Ir, N as wr, O as Sr, P as Pr, Q as Tr, R as Cr, T as Ar, U as qr, V as $r, W as jr, X as Nr, Y as Ur, Z as Or, _ as Fr, $ as Rr, a0 as Dr, a1 as Lr, a2 as Br, a3 as Vr, a4 as zr, a5 as Hr, a6 as Jr, a7 as Kr, a8 as Gr, a9 as Yr, aa as Xr, ab as Wr, ac as Zr, ad as Qr } from "./scripture-util-DArajUVn.js";
...
class Ye {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: ye }, s = t.catch((n) => this.logger.warn(`Error in promise for ${e}: ${n.message}`)).finally(() => {
...
const X = 1e4, U = X / 1e3, W = 3, H = /* @__PURE__ */ new Map(), ke = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (i, e, t) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run threw in the last ${U}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${W}): ${t.join(" | ")}`
  }
};
function J(i, e, t) {
  const { log: r, first: s, suppressed: n } = ke[e], c = `${i}\0${e}`, p = Date.now(), d = H.get(c);
  if (!d || p >= d.closesAt) {
    r(s(i, t)), H.set(c, {
      closesAt: p + X,
      suppressedCount: 0,
...
const Ze = "ABORTED", Qe = "ALREADY_EXISTS", et = "CANCELLED", tt = "DATA_LOSS", rt = "DEADLINE_EXCEEDED", nt = "FAILED_PRECONDITION", it = "INTERNAL", st = "INVALID_ARGUMENT", at = "NOT_FOUND", ot = "OUT_OF_RANGE", ct = "PERMISSION_DENIED", dt = "RESOURCE_EXHAUSTED", pt = "UNAUTHENTICATED", lt = "UNAVAILABLE", ut = "UNIMPLEMENTED", ht = "UNKNOWN", q = 1;
function ft(i, e) {
  if (!i)
    return {
      message: "",
...
const gt = (i) => (...e) => i.map((r) => r(...e)).every((r) => r), yt = (i) => async (...e) => {
  const t = i.map(async (r) => r(...e));
  return (await Promise.all(t)).every((r) => r);
}, be = "book", K = "chapter", w = "verse", S = "***";
...
function kt(i) {
  var e;
  return ((e = R[i]) == null ? void 0 : e.type) === a.Paragraph || i === "v";
}
function bt(i) {
  const e = R[i];
  return (e == null ? void 0 : e.type) !== a.Character ? !1 : e.category !== o.DivisionMarks;
}
const ve = /* @__PURE__ */ new Set(["id", "c"]), vt = Object.entries(R).filter(
  ([i, e]) => e.type === a.Paragraph && !ve.has(i)
).map(([i]) => i).sort();
function Mt(i) {
  return oe.sanitize(i, {
    ALLOWED_TAGS: [
      "p",
      "br",
      "b",
...
function Ft(i, e, t) {
  ...
  return s.id = `${qe}${t ? `-${t}` : ""}`, s.dataset.themeId = i.id, s.textContent = je(i), this.document.head.appendChild(s), s;
}
function ne(i) {
...
USJ: ${JSON.stringify(
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const u = parseInt(b.colspan, 10), h = Ue.exec(r);
      if (h != null && h[1]) {
  ...
  textContentToUsfm(e) {
    return {
      usfm: this.shouldAllowInvisibleCharacters ? e : e.replace(Ne, "~"),
...
export {
  ...
  He as Collator,
  tt as DATA_LOSS,
  rt as DEADLINE_EXCEEDED,
  Bt as DEBOUNCE_CANCELED_ERROR_MESSAGE,
  pe as DateTimeFormat,
  he as DocumentCombiner,
  Je as EventRollingTimeCounter,
  nt as FAILED_PRECONDITION,
  Vt as FIRST_SCR_BOOK_NUM,
  zt as FIRST_SCR_CHAPTER_NUM,
  Ht as FIRST_SCR_VERSE_NUM,
  Jt as GraphemeString,
  it as INTERNAL,
  st as INVALID_ARGUMENT,
  Kt as LAST_SCR_BOOK_NUM,
  Gt as MAX_PADDING_LENGTH,
  At as MODIFIER_KEYS,
  a as MarkerType,
  me as Mutex,
  Ke as MutexMap,
  at as NOT_FOUND,
  Ge as NonValidatingDocumentCombiner,
  ge as NumberFormat,
  ot as OUT_OF_RANGE,
  ct as PERMISSION_DENIED,
  q as PLATFORM_ERROR_VERSION,
  ve as PROGRAMMATICALLY_APPLIED_MARKERS,
  $ as PROJECT_SELECTOR_CUSTOM_DATA_KEYS,
  ue as PlatformEventEmitter,
  Ye as PromiseChainingMap,
  dt as RESOURCE_EXHAUSTED,
  Yt as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  Xt as Section,
  z as SortedNumberMap,
  Xe as SortedSet,
  qe as THEME_STYLE_ELEMENT_ID,
  ...
  gt as aggregateUnsubscribers,
  Ft as applyThemeStylesheet,
  Wt as areUsjContentsEqualExceptWhitespace,
  Zt as at,
  Qt as charAt,
  er as codePointAt,
  tr as collapseMiddleWords,
  rr as collectUsjMarkers,
  nr as compareScrRefs,
  $t as computeEffectiveStructureProtection,
  ir as createSyncProxyForAsyncObject,
  sr as debounce,
  C as deepClone,
  O as deepEqual,
  ar as defaultScrRef,
  Ee as deserialize,
  Ut as doesCatalogRowCoverProject,
  or as endsWith,
  cr as ensureArray,
  dr as escapeStringRegexp,
  ...
  Nt as recencyMapFromOrderedIds,
  Dr as retryUntil,
  Mt as sanitizeHtml,
  Lr as scrRefToBBBCCC,
  Br as scrRefToBBBCCCVVV,
  vt as selectableParagraphMarkers,
  G as serialize,
