...
var oe = (s, e, t) => e in s ? ae(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : s[e] = t;
var k = (s, e, t) => oe(s, typeof e != "symbol" ? e + "" : e, t);
import { d as T, i as _ } from "./scripture-util-DArajUVn.js";
import { D as zt, F as Ht, a as Jt, b as Kt, G as Gt, L as Yt, M as Xt, S as Wt, c as Zt, e as Qt, f as er, g as tr, h as rr, j as nr, k as ir, l as sr, m as ar, n as or, o as cr, p as dr, q as pr, r as lr, s as ur, t as hr, u as fr, v as mr, w as gr, x as yr, y as kr, z as br, A as vr, B as xr, C as Mr, E as _r, H as Er, I as Ir, J as Sr, K as wr, N as Pr, O as Tr, P as Cr, Q as Ar, R as qr, T as $r, U as jr, V as Nr, W as Or, X as Ur, Y as Fr, Z as Rr, _ as Dr, $ as Lr, a0 as Br, a1 as Vr, a2 as zr, a3 as Hr, a4 as Jr, a5 as Kr, a6 as Gr, a7 as Yr, a8 as Xr, a9 as Wr, aa as Zr, ab as Qr, ac as en, ad as tn } from "./scripture-util-DArajUVn.js";
import { Mutex as ce } from "async-mutex";
import de from "dompurify";
import { deepEqual as pe } from "fast-equals";
import { USJ_TYPE as le } from "@eten-tech-foundation/scripture-utilities";
import { JSONPath as w } from "jsonpath-plus";
const C = class C {
  /**
  ...
  resolveToValue(e, t = !1) {
    if (this.resolver)
      C.verboseLoggingEnabled && console.debug(`${this.variableName} is being resolved now`), this.resolver(e), this.complete();
    else {
  ...
  rejectWithReason(e, t = !1) {
    if (this.rejecter)
      C.verboseLoggingEnabled && console.debug(`${this.variableName} is being rejected now with reason: ${e}`), this.rejecter(e), this.complete();
    else {
      if (t) throw Error(`${this.variableName} was already settled`);
...
class me {
  ...
  constructor(e, t) {
    ...
    k(this, "latestOutput");
    k(this, "options");
    k(this, "onDidRebuildEmitter", new fe());
  ...
  addOrUpdateContribution(e, t) {
    ...
    let i = this.options.copyDocuments && t ? T(t) : t;
...
class Xe {
  ...
  get(e) {
    ...
    return t || (t = new ye(), this.mutexesByID.set(e, t), t);
  }
  /**
...
class Ze {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: be }, i = t.catch((n) => this.logger.warn(`Error in promise for ${e}: ${n.message}`)).finally(() => {
...
const Y = 1e4, N = Y / 1e3, X = 3, V = /* @__PURE__ */ new Map(), ve = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (s, e, t) => `UnsubscriberAsyncList ${s}: ${e} more unsubscribers added after the list was run threw in the last ${N}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${X}): ${t.join(" | ")}`
  }
};
function z(s, e, t) {
  const { log: r, first: i, suppressed: n } = ve[e], c = `${s}\0${e}`, p = Date.now(), d = V.get(c);
  if (!d || p >= d.closesAt) {
    r(i(s, t)), V.set(c, {
      closesAt: p + Y,
      suppressedCount: 0,
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < X && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
...
const tt = "ABORTED", rt = "ALREADY_EXISTS", nt = "CANCELLED", it = "DATA_LOSS", st = "DEADLINE_EXCEEDED", at = "FAILED_PRECONDITION", ot = "INTERNAL", ct = "INVALID_ARGUMENT", dt = "NOT_FOUND", pt = "OUT_OF_RANGE", lt = "PERMISSION_DENIED", ut = "RESOURCE_EXHAUSTED", ht = "UNAUTHENTICATED", ft = "UNAVAILABLE", mt = "UNIMPLEMENTED", gt = "UNKNOWN", A = 1;
function yt(s, e) {
  if (!s)
    return {
      message: "",
...
const bt = (s) => (...e) => s.map((r) => r(...e)).every((r) => r), vt = (s) => async (...e) => {
  const t = s.map(async (r) => r(...e));
  return (await Promise.all(t)).every((r) => r);
}, xe = "book", H = "chapter", E = "verse", I = "***", Me = "ws";
...
function xt(s) {
  var e;
  return ((e = W[s]) == null ? void 0 : e.type) === a.Paragraph || s === "v";
}
function Mt(s) {
  const e = W[s];
...
function _e(s, e) {
  const t = e && e.length > 0 ? e : Z();
  return t[s % t.length];
}
function Et(s, e) {
  const t = e && e.length > 0 ? e : Z(), r = (() => {
    const i = /* @__PURE__ */ new Map();
    let n = 0;
    return s.forEach((c, p) => {
      c.caller === "+" && (i.set(p, _e(n, t)), n += 1);
    }), i;
  })();
  return (i, n) => {
    if (i === "+") {
...
function Ie(s, e) {
  ...
  return Object.keys(t).forEach((n) => {
    i && (Object.hasOwn(r, n) && Ie(r[n], t[n]) || (i = !1));
  }), i;
}
function J(s, e, t) {
...
const wt = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function Pt() {
  return typeof navigator < "u" && navigator.languages ? navigator.languages[0].replace(/@posix$/i, "") : new ue().resolvedOptions().locale;
}
function Tt(s, e = 2) {
...
const we = 1e3, Q = 60, ee = Q * 60, Pe = ee * 24;
function At(s, e, t = /* @__PURE__ */ new Date()) {
  const r = Math.floor((e.getTime() - t.getTime()) / we), i = Math.round(r / Pe);
  if (Math.abs(i) >= 1) return s.format(i, "day");
  const n = Math.round(r / ee);
  if (Math.abs(n) >= 1) return s.format(n, "hour");
  const c = Math.round(r / Q);
...
function Ot(s) {
  const e = {};
  return typeof s.language == "string" && s.language.length > 0 && (e[q.language] = s.language), typeof s.type == "string" && s.type.length > 0 && (e[q.type] = s.type), typeof s.typeName == "string" && s.typeName.length > 0 && (e[q.typeName] = s.typeName), typeof s.lastUsedAt == "number" && Number.isFinite(s.lastUsedAt) && (e[q.lastUsedAt] = s.lastUsedAt), e;
}
function Ut(s) {
  const e = /* @__PURE__ */ new Map(), t = s.length;
...
function Dt(s, e, t) {
  ...
  return i.id = `${je}${t ? `-${t}` : ""}`, i.dataset.themeId = s.id, i.textContent = Oe(s), this.document.head.appendChild(i), i;
}
function ne(s) {
  return Object.freeze(s), s == null || Object.getOwnPropertyNames(s).forEach(function(t) {
    ...
    s[t] !== null && (typeof s[t] == "object" || typeof s[t] == "function") && !Object.isFrozen(s[t]) && ne(s[t]);
  }), s;
}
const U = ne({
  version: "3.0.7",
...
}), P = ["figure", "note", "sidebar", "table"];
Object.freeze(P);
const ie = ["para", "table", "table:row", "table:cell", "sidebar"];
Object.freeze(ie);
const Ue = "chapter", K = new RegExp(`^${Me}\\d+$`), Fe = /\u00A0/g, Re = /\w+(\d+)/, De = /(\d+)-?(\d+)?/;
class m {
  constructor(e, t) {
    k(this, "usj");
    k(this, "markersMap");
...
USJ: ${JSON.stringify(
    ...
    else if (m.areUsjVersionsCompatible(this.usj.version, U.version))
      this.markersMap = U;
    else
      throw new Error(
  ...
  findSingleValue(e) {
    const t = w({ path: e, json: this.usj, wrap: !0 });
    if (t === void 0 || t.length === 0) return;
    if (!Array.isArray(t[0])) return t[0];
    const r = w({ path: e, json: this.usj, wrap: !1 });
  ...
  static isTopLevelUsjMarker(e, t) {
    return typeof e == "object" && e.type === le && t.length === 0;
  }
  /**
  ...
  findAllNotes() {
    var e;
    return Ee((e = this.usj) == null ? void 0 : e.content);
  }
  /**
  ...
  static findNextMatchingNodeOrClosingFragmentUsingWorkingStack(e, t, r, i) {
    ...
    if (!_(c)) {
      ...
      t.some((l) => {
        ...
        return !_(h) && r.includes(h.type) ? (d = h, !0) : !1;
      }), d && (n = d);
    }
    for (; n !== void 0; ) {
  ...
  getEffectiveBookId(e) {
    const t = Object.keys(this.indicesInUsfmByVerseRef), r = t.length === 0 || t.length === 1 && t[0] === I, i = r ? I : e;
    if (!this.indicesInUsfmByVerseRef[i])
      throw new Error(
        `Book ID ${e} not found in USJ! ${r ? `There seems to be no USJ content because there is no content in ${I} either` : `Book IDs in USJ: ${JSON.stringify(t)}`}`
      );
    return i;
  }
  /**
  ...
  getVerseRefForIndexInUsfm(e, t) {
    ...
    return d && m.isFragmentAMarker(d.fragment) && d.fragment.type === E && d.fragment.number && d.fragment.number !== `${n.verseNum}` && (n.verse = d.fragment.number), n;
  }
    ...
    if (m.findNextMatchingNodeUsingWorkingStack(
      ...
      (u, g) => {
        ...
        if (l && m.areStackItemsShallowEqual(M, l)) {
          ...
          y = u.substring(e.documentLocation.offset), c += e.documentLocation.offset;
        }
        n += y.length, i = `${i}${y}`;
        const v = i.indexOf(t);
        return v < 0 ? (c += i.length, i.length > t.length && (i = i.substring(i.length - t.length)), c -= i.length, n > r) : (p = c + v, !0);
      }
    ), p < 0) return;
    n = 0;
    let h = 0, b = [];
  ...
  static findNearestBlockAncestor(e) {
    for (let t = e.length - 1; t >= 0; t--) {
      const { parent: r } = e[t];
      if (r && "type" in r && ie.includes(r.type)) return r;
    }
  }
  /**
   ...
   */
  static advancePastCodePoint(e, t) {
    const r = e.codePointAt(t);
    return r === void 0 ? t + 1 : t + (r > 65535 ? 2 : 1);
  }
  /**
  ...
  static hasWhitespaceGapAwayFromBoundary(e, t, r) {
    ...
    return Object.entries(i).some(([c, p]) => {
      if (!K.test(c) || !p) return !1;
      const [d, l] = p;
      if (l > d) return !1;
      const h = r ? r[d] : d;
      return !t.has(h);
   ...
   */
  static hasSearchWhitespaceGroup(e) {
    let t = !1;
    for (let r = 0; r < e.length; r++) {
      ...
      if (i === "\\")
        ...
        const n = /^\(\?<([A-Za-z_$][\w$]*)>/.exec(e.slice(r));
        if (n && K.test(n[1])) return !0;
      }
    }
    return !1;
  }
   ...
   */
  collectSearchText(e, t) {
    const r = {
      node: this.usj,
      documentLocation: {
        jsonPath: "$"
      }
    }, i = [], n = new B(), c = /* @__PURE__ */ new Set();
    let p, d = !1, l = !1;
    const h = [];
    let b = 0, f = r.node;
    for (; f !== void 0; )
      f = m.findNextMatchingNodeUsingWorkingStack(
        r.node,
        ...
        (u, g) => {
          if (typeof u != "string")
            return t && u && "type" in u && u.type === Ue && (l = !0), !1;
          if (e && g.some((M) => {
            const v = M.parent;
            if (!v || !("type" in v) || v.type === "char") return !1;
            let x;
            return "style" in v && typeof v.style == "string" ? x = v.style : "marker" in v && typeof v.marker == "string" && (x = v.marker), x !== void 0 && !e.has(x);
          }))
            return t && h.push(
              m.findNearestBlockAncestor(g)
            ), !1;
          if (t) {
            const y = m.findNearestBlockAncestor(g), M = h.some(
              (v) => v !== p && v !== y
            );
            d && y !== p && !l && !M && c.add(b), p = y, d = !0, l = !1, h.length = 0;
          }
          return i.push(u), n.set(b, {
            node: u,
            documentLocation: {
              offset: 0,
              jsonPath: m.convertWorkingStackToJsonPath(g)
            }
          }), b += u.length, !1;
        }
      );
    return { textChunks: i, fullTextIndexMap: n, blockBoundaryOffsets: c };
  }
  /**
   ...
   */
  static buildSearchResult(e, t, r, i) {
    const n = i ? i[e.index] : e.index, c = i ? i[e.index + e[0].length] : e.index + e[0].length;
    if (n < 0 || n >= t.length)
      throw new Error(`Match index out of bounds: ${n}`);
    const p = r.findClosestLessThanOrEqual(n);
    if (!p)
      throw new Error(`Internal error: no starting node found for index ${n}`);
    const d = {
      node: p.value.node,
      documentLocation: {
    ...
    }, l = r.findClosestLessThanOrEqual(c - 1);
    if (!l)
      throw new Error(`Internal error: no ending node found for index ${n}`);
    const h = {
      node: l.value.node,
      documentLocation: {
        jsonPath: l.value.documentLocation.jsonPath,
        offset: c - l.key
      }
    };
    return { text: i ? t.substring(n, c) : e[0], start: d, end: h };
  }
  search(e, t) {
    const r = t instanceof Set ? { markerStylesToInclude: t } : t, { markerStylesToInclude: i, normalizationForm: n } = r ?? {}, c = !!(r && "flexibleWhitespaceAtBlockBoundaries" in r && r.flexibleWhitespaceAtBlockBoundaries), p = [];
    if (this.usj.content.length === 0) return p;
    const d = c && m.hasSearchWhitespaceGroup(e.source), { textChunks: l, fullTextIndexMap: h, blockBoundaryOffsets: b } = this.collectSearchText(
      i,
      d
    ), f = l.join(""), u = n === "NFD" ? m.buildNFDToOriginalPositionMap(f) : void 0, g = u ? f.normalize("NFD") : f, y = d && !e.flags.includes("d") ? new RegExp(e.source, `${e.flags}d`) : e, M = e.lastIndex;
    try {
      let v = y.exec(g);
      for (; v; )
        if (v[0].length === 0) {
          if (!y.global || y.lastIndex >= g.length) break;
          y.lastIndex = m.advancePastCodePoint(
            g,
            y.lastIndex
          ), v = y.exec(g);
        } else if (
          ...
          d && m.hasWhitespaceGapAwayFromBoundary(
            v,
            b,
            u
          )
          ...
          if (!y.global) break;
          y.lastIndex = m.advancePastCodePoint(g, v.index), v = y.exec(g);
        } else {
          if (p.push(
            m.buildSearchResult(v, f, h, u)
          ), !y.global) break;
          v = y.exec(g);
        }
    } finally {
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const f = parseInt(b.colspan, 10), u = Re.exec(r);
      if (u != null && u[1]) {
  ...
  textContentToUsfm(e) {
    return {
      usfm: this.shouldAllowInvisibleCharacters ? e : e.replace(Fe, "~"),
...
`);
    ...
    return i.push({ fragment: e, indexInUsfm: r.length }), r += p === "optbreak" ? "//" : `\\${b}`, p !== "optbreak" && (i.push({
      ...
      const { usfm: v } = this.textContentToUsfm(g);
      i.push({
        fragment: {
        ...
        indexInUsfm: r.length
      }), r += v, r = this.addMarkerUsfmToString(
        r,
        {
          isClosingMarker: !0,
          forMarker: y
        ...
        if (_(x.fragment) || "attributeKey" in x.fragment)
          throw new Error(
        ...
        if (m.isFragmentAMarker(x.fragment)) {
          i.push({
            ...x,
            fragment: {
        ...
        if ("isClosingMarker" in x.fragment) {
          const { isClosingMarker: j, ...S } = x.fragment, se = {
            ...S,
            forMarker: e,
  ...
  closingMarkerToUsfm(e, t) {
    const {
      ...
      var y, M;
      return !(g === "type" || g === "marker" || g === "content" || g === "closed" || (y = p.skipOutputAttributeToUsfm) != null && y.includes(g) || (M = n.leadingAttributes) != null && M.includes(g) || n.textContentAttribute === g || d.some(
        ([, v]) => v.attributeMarkerAttributeName === g
      ));
    }), h = e;
    ...
    if ((h.closed === "false" || n.isClosingMarkerOptional && !this.markersMap.shouldOptionalClosingMarkersBePresent && h.closed !== "true") && (b = !1), n.independentClosingMarkers && n.independentClosingMarkers.length > 0 && b) {
      ...
      let y = "";
      const M = [], { usfm: v, fragmentsInfo: x } = this.openingMarkerToUsfm(g, t), j = x.find((S) => m.isFragmentAMarker(S.fragment));
      if (!j)
        throw new Error(
      ...
      }), y += v, r !== g.marker && (y = this.addMarkerUsfmToString(
        y,
        {
          isClosingMarker: !0,
          forMarker: g
    ...
    if (l.length > 0 && (f += "|", l.length === 1 && l[0] === n.defaultAttribute ? (u.push({
    ...
    }), f += h[n.defaultAttribute]) : l.forEach((g, y) => {
      ...
      const g = p.isClosingMarkerEmpty ? "" : i, y = t ? p.nestedPrefix ?? "" : "";
      u.push({
        fragment: { isClosingMarker: !0, forMarker: e },
        indexInUsfm: f.length
      }), f += `\\${y}${g}*`;
...
`) && (n.length === 0 ? (p = p.map((f) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, r = !1) {
    ...
    return !r && !O(w.toPathArray(i), w.toPathArray(c)) ? !1 : O(n, p);
  }
  ...
  static transferFragmentsInfoArrayToMaps(e, t, r, i, n, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        const u = d.fragment;
        if (u.type === xe && u.code)
          r.bookId = u.code, r.chapterNum = 0, r.verseNum = 0, c[I] && (c[r.bookId] = c[I], delete c[I]);
        else if (u.type === H && u.number) {
          const g = parseInt(u.number, 10);
          if (Number.isNaN(g))
            console.warn(
              `Found ${H} type marker with number ${u.number}, but could not parse chapter number from it. Continuing using previous chapter number ${r.chapterNum}`
            );
          else {
        ...
        } else if (u.type === E && u.number) {
          const g = (h = De.exec(u.number)) == null ? void 0 : h[1];
          if (!g)
            console.warn(
              `Found ${E} type marker with number ${u.number}, but could not find starting verse number in it. Continuing using previous verse number ${r.verseNum}`
            );
          else {
            const y = parseInt(g, 10);
            Number.isNaN(y) ? console.warn(
              `Found ${E} type marker with number ${u.number}, but could not parse starting verse number from ${g}. Continuing using previous verse number ${r.verseNum}`
            ) : (f = (b = c[r.bookId]) == null ? void 0 : b[r.chapterNum]) != null && f[y] ? console.warn(`Found ${E} marker with existing number ${y} after
                  current ${E} number ${r.verseNum}! Not updating verse start index. All positions in this duplicate verse will be based on the current ${E} marker, not the new duplicate marker.`) : (y < r.verseNum && console.debug(
              `Found ${E} marker with number ${y} lower than current ${E} number ${r.verseNum}. Verses are out of order. There may be some issues.`
            ), r.verseNum = y);
          }
        }
      }
...
export {
  ...
  Xe as MutexMap,
  dt as NOT_FOUND,
  We as NonValidatingDocumentCombiner,
  ke as NumberFormat,
  pt as OUT_OF_RANGE,
  lt as PERMISSION_DENIED,
  A as PLATFORM_ERROR_VERSION,
  q as PROJECT_SELECTOR_CUSTOM_DATA_KEYS,
  fe as PlatformEventEmitter,
  Ze as PromiseChainingMap,
  ut as RESOURCE_EXHAUSTED,
  Me as SEARCH_WHITESPACE_GROUP_PREFIX,
  Wt as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  Zt as Section,
  B as SortedNumberMap,
  Qe as SortedSet,
  je as THEME_STYLE_ELEMENT_ID,
  ht as UNAUTHENTICATED,
  ft as UNAVAILABLE,
  mt as UNIMPLEMENTED,
  gt as UNKNOWN,
  ...
  E as VERSE_TYPE,
  vt as aggregateUnsubscriberAsyncs,
  bt as aggregateUnsubscribers,
  ...
  Nt as computeEffectiveStructureProtection,
  ...
  cr as defaultScrRef,
  Se as deserialize,
  Ft as doesCatalogRowCoverProject,
  dr as endsWith,
  pr as ensureArray,
  lr as escapeStringRegexp,
  Rt as expandThemeContribution,
  Tt as formatBytes,
  ...
  Ir as indexOf,
  xt as isBlockMarker,
  Mt as isCharacterMarker,
  Sr as isErrorMessageAboutParatextBlockingInternetAccess,
  wr as isErrorMessageAboutRegistryAuthFailure,
  Pr as isLocalizeKey,
  kt as isPlatformError,
  ...
  Ar as isolateBidi,
  qr as lastIndexOf,
  Ae as localizedStringsDocumentSchema,
  Ot as makeProjectSelectorCustomData,
  qe as menuDocumentSchema,
  $r as newGuid,
  yt as newPlatformError,
  jr as normalize,
  Ct as normalizeProjectId,
  Nr as normalizeScriptureSpaces,
  Or as offsetBook,
  Ur as offsetChapter,
  Fr as offsetVerse,
  Rr as ordinalCompare,
  Dr as padEnd,
  Lr as padStart,
  Te as projectSettingsDocumentSchema,
  Ut as recencyMapFromOrderedIds,
  Br as retryUntil,
  _t as sanitizeHtml,
  Vr as scrRefToBBBCCC,
  zr as scrRefToBBBCCCVVV,
  J as serialize,
  Ce as settingsDocumentSchema,
  Hr as slice,
  Jr as split,
  Kr as startsWith,
  Gr as stringLength,
  Yr as substring,
  $e as themeDocumentSchema,
  Xr as toArray,
  Wr as toKebabCase,
