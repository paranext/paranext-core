...
import { D as hr, F as fr, a as mr, b as gr, G as yr, L as kr, M as br, S as vr, c as Mr, e as xr, f as _r, g as Er, h as Ir, j as wr, k as Sr, l as Pr, m as Tr, n as Cr, o as Ar, p as qr, q as $r, r as Nr, s as jr, t as Or, u as Ur, v as Fr, w as Rr, x as Dr, y as Lr, z as Br, A as Vr, B as zr, C as Hr, E as Jr, H as Kr, I as Gr, J as Yr, K as Xr, N as Wr, O as Zr, P as Qr, Q as en, R as tn, T as rn, U as nn, V as sn, W as an, X as on, Y as cn, Z as dn, _ as pn, $ as ln, a0 as un, a1 as hn, a2 as fn, a3 as mn, a4 as gn, a5 as yn, a6 as kn, a7 as bn, a8 as vn, a9 as Mn, aa as xn, ab as _n, ac as En, ad as In } from "./scripture-util-DArajUVn.js";
...
const pe = 1e4, H = pe / 1e3, le = 3, ie = /* @__PURE__ */ new Map(), Oe = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (n, e, t) => `UnsubscriberAsyncList ${n}: ${e} more unsubscribers added after the list was run threw in the last ${H}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${le}): ${t.join(" | ")}`
  }
};
function se(n, e, t) {
  const { log: r, first: s, suppressed: i } = Oe[e], c = `${n}\0${e}`, p = Date.now(), d = ie.get(c);
  if (!d || p >= d.closesAt) {
    r(s(n, t)), ie.set(c, {
      closesAt: p + pe,
      suppressedCount: 0,
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < le && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
...
function Dt(n) {
  var e;
  return ((e = G[n]) == null ? void 0 : e.type) === a.Paragraph || n === "v";
}
function Lt(n) {
  const e = G[n];
...
function Bt(n) {
  var e;
  return ((e = G[n]) == null ? void 0 : e.type) === a.Paragraph;
}
function Vt(n) {
...
const Kt = (n) => n.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function Gt() {
...
function Qt(n, e, t = /* @__PURE__ */ new Date()) {
...
function ir(n = {}) {
  ...
  let X = -1 / 0;
  const ke = (v) => T.size > 0 || $.size > 0 && v - X < S, W = (v) => {
  ...
  r && (r.addEventListener("keydown", W, !0), r.addEventListener("keyup", Z, !0), r.addEventListener("pointerdown", R, !0), r.addEventListener("pointermove", R, !0), r.addEventListener("blur", V), r.document.addEventListener("visibilitychange", Q));
  const be = (v, E, P) => (
  ...
  return {
    ...
    dispose() {
      r && (r.removeEventListener("keydown", W, !0), r.removeEventListener("keyup", Z, !0), r.removeEventListener("pointerdown", R, !0), r.removeEventListener("pointermove", R, !0), r.removeEventListener("blur", V), r.document.removeEventListener("visibilitychange", Q));
    }
  };
}
const Y = {
...
class y {
  ...
  getBookIdForLogging() {
    ...
    const e = (t = this.usj.content) == null ? void 0 : t.find(
      (r) => typeof r != "string" && r.type === ae && !!r.code
    );
...
`) && (i.length === 0 ? (p = p.map((u) => ({
 ...
 `);
  ...
  transferFragmentsInfoArrayToMaps(e, t, r, s, i, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        else if (h.type === oe && h.number) {
          const f = parseInt(h.number, 10);
          if (Number.isNaN(f))
            this.reportProblemOnce(
              `Found ${oe} type marker with number ${h.number}, but could not parse chapter number from it. Continuing using previous chapter number ${r.chapterNum}`
            );
          else {
  ...
  calculateUsfmProperties() {
    let e = "";
    const t = new ne(), r = /* @__PURE__ */ new Map(), s = {}, i = [], c = {
      bookId: A,
      chapterNum: 0,
      verseNum: 0
    }, p = (m) => {
...
export {
  ...
  hr as DEBOUNCE_CANCELED_ERROR_MESSAGE,
  Pe as DateTimeFormat,
  Ae as DocumentCombiner,
  ft as EventRollingTimeCounter,
  It as FAILED_PRECONDITION,
  fr as FIRST_SCR_BOOK_NUM,
  mr as FIRST_SCR_CHAPTER_NUM,
  gr as FIRST_SCR_VERSE_NUM,
  yr as GraphemeString,
  wt as INTERNAL,
  St as INVALID_ARGUMENT,
  kr as LAST_SCR_BOOK_NUM,
  br as MAX_PADDING_LENGTH,
  tr as MODIFIER_KEYS,
  a as MarkerType,
  $e as Mutex,
  mt as MutexMap,
  ...
  vr as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  Mr as Section,
  ne as SortedNumberMap,
  kt as SortedSet,
  ...
  pr as USFM_MARKERS_MAP_PARATEXT_3_0,
  bt as UnsubscriberAsyncList,
  y as UsjReaderWriter,
  C as VERSE_TYPE,
  Rt as aggregateUnsubscriberAsyncs,
  Ft as aggregateUnsubscribers,
  dr as applyThemeStylesheet,
  xr as areUsjContentsEqualExceptWhitespace,
  _r as at,
  Er as charAt,
  Ir as codePointAt,
  wr as collapseMiddleWords,
  Sr as collectUsjMarkers,
  Ve as compareProjectShortNames,
  Zt as compareProjectsByName,
  Pr as compareScrRefs,
  nr as computeEffectiveStructureProtection,
  ir as createContentZoomWheelReader,
  Tr as createSyncProxyForAsyncObject,
  Cr as debounce,
  U as deepClone,
  J as deepEqual,
  Ar as defaultScrRef,
  De as deserialize,
  or as doesCatalogRowCoverProject,
  qr as endsWith,
  $r as ensureArray,
  Nr as escapeStringRegexp,
  cr as expandThemeContribution,
  Yt as formatBytes,
  Wt as formatProjectName,
  er as formatRelativeDate,
  jr as formatReplacementString,
  Or as formatReplacementStringToArray,
  Ur as formatScrRef,
  Fr as formatScrRefRange,
  Qt as formatTimeSpan,
  Rr as getAllObjectFunctionNames,
  Dr as getChaptersForBook,
  Gt as getCurrentLocale,
  ue as getDefaultCallerSequence,
  Lr as getErrorMessage,
  zt as getFormatCallerFunction,
  rr as getLocalizeKeyForPhysicalKey,
  Br as getLocalizeKeyForScrollGroupId,
  Vr as getLocalizeKeysForScrollGroupIds,
  zr as getLocalizedIdFromBookNumber,
  Ue as getNthCaller,
  Ht as getPaneSizeLimits,
  Hr as getSectionForBook,
  ...
  Yr as isErrorMessageAboutParatextBlockingInternetAccess,
  Xr as isErrorMessageAboutRegistryAuthFailure,
  Wr as isLocalizeKey,
  Bt as isParagraphMarker,
  Ut as isPlatformError,
  Zr as isSelectableInvisibleCharOrWhiteSpace,
  Jt as isSerializable,
  w as isString,
  Re as isSubset,
  Qr as isWhiteSpace,
  en as isolateBidi,
  ...
  sr as makeProjectSelectorCustomData,
  Ze as menuDocumentSchema,
  rn as newGuid,
  Ot as newPlatformError,
  nn as normalize,
  ze as normalizeFullName,
  Xt as normalizeProjectId,
  sn as normalizeScriptureSpaces,
  an as offsetBook,
  on as offsetChapter,
  cn as offsetVerse,
  dn as ordinalCompare,
  pn as padEnd,
  ln as padStart,
  Ye as projectSettingsDocumentSchema,
  ar as recencyMapFromOrderedIds,
  un as retryUntil,
  Vt as sanitizeHtml,
  hn as scrRefToBBBCCC,
  fn as scrRefToBBBCCCVVV,
  ce as serialize,
  ...
  vn as toArray,
  Mn as toKebabCase,
  xn as transformAndEnsureRegExpArray,
  _n as transformAndEnsureRegExpRegExpArray,
  G as usfmMarkers,
  En as wait,
  In as waitForDuration
};
