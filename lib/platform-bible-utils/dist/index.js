...
var _e = (r, e, t) => e in r ? Ne(r, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : r[e] = t;
var b = (r, e, t) => _e(r, typeof e != "symbol" ? e + "" : e, t);
import { Mutex as Ie } from "async-mutex";
import { Canon as S, VerseRef as ce } from "@sillsdev/scripture";
import { USJ_TYPE as de } from "@eten-tech-foundation/scripture-utilities";
import { indexOf as Se, limit as le, length as we, substring as Pe, toArray as Te, substr as Ce } from "stringz";
import Ae from "dompurify";
import { deepEqual as qe } from "fast-equals";
...
const Qt = "ABORTED", er = "ALREADY_EXISTS", tr = "CANCELLED", rr = "DATA_LOSS", nr = "DEADLINE_EXCEEDED", ir = "FAILED_PRECONDITION", sr = "INTERNAL", ar = "INVALID_ARGUMENT", or = "NOT_FOUND", cr = "OUT_OF_RANGE", dr = "PERMISSION_DENIED", lr = "RESOURCE_EXHAUSTED", pr = "UNAUTHENTICATED", ur = "UNAVAILABLE", hr = "UNIMPLEMENTED", fr = "UNKNOWN", U = 1;
function mr(r, e) {
  if (!r)
    return {
      message: "",
...
function He(r, e) {
  ...
  let n = 0, s = 0;
  function i(d, l, p) {
    const f = T(r, s, l), g = t.length > 0 && v(t[t.length - 1]) ? `${t.pop()}${f}` : f;
...
function Mr(r, e, t = " ") {
  return e <= N(r) ? r : le(r, e, t, "right");
}
function xr(r, e, t = " ") {
  return e <= N(r) ? r : le(r, e, t, "left");
}
function W(r, e) {
...
], Ze = 1, Qe = me.length - 1, et = 1, tt = 1, Pr = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, rt = (r) => {
...
function $r(r) {
  return new ce(S.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCC;
}
function ee(r) {
  return new ce(S.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCCVVV;
}
function nt(r, e) {
  return ee(r) - ee(e);
}
...
function ye(r, e, t, n) {
  ...
  if (s && i) {
    const c = ne(r), d = ne(t);
    if (c !== d) {
      if (!C(D(c, -1) ?? "") && !C(D(d, -1) ?? "") || !se(r, e) || !se(t, n)) return !1;
      let l = c;
      for (; C(D(l, -1) ?? ""); ) l = Z(l, 0, -1);
      let p = d;
      for (; C(D(p, -1) ?? ""); ) p = Z(p, 0, -1);
      if (l !== p) return !1;
    }
  } else if (!s && !i) {
    ...
    if (!p && !f) {
      ...
      V.includes(c.type) && v(h) && C(h) && (g = g.slice(0, -1));
      const m = u[u.length - 1];
      if (V.includes(d.type) && v(m) && C(m) && (u = u.slice(0, -1)), g.length !== u.length) return !1;
...
const Rr = (r) => (...e) => r.map((n) => n(...e)).every((n) => n), Lr = (r) => async (...e) => {
  const t = r.map(async (n) => n(...e));
  return (await Promise.all(t)).every((n) => n);
}, ut = "book", ae = "chapter", w = "verse", P = "***";
...
function tn(r, e, t) {
  ...
  return s.id = `${Nt}${t ? `-${t}` : ""}`, s.dataset.themeId = r.id, s.textContent = It(r), this.document.head.appendChild(s), s;
}
function Ee(r) {
  return Object.freeze(r), r == null || Object.getOwnPropertyNames(r).forEach(function(t) {
    ...
    // eslint-disable-next-line no-null/no-null
    r[t] !== null && (typeof r[t] == "object" || typeof r[t] == "function") && !Object.isFrozen(r[t]) && Ee(r[t]);
  }), r;
}
const J = Ee({
  version: "3.0.7",
...
USJ: ${JSON.stringify(
    ...
    else if (k.areUsjVersionsCompatible(this.usj.version, J.version))
      this.markersMap = J;
    else
      throw new Error(
...
`) && (i.length === 0 ? (d = d.map((u) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, n = !1) {
    ...
    return !n && !z(q.toPathArray(s), q.toPathArray(c)) ? !1 : z(i, d);
  }
...
export {
  ...
  w as VERSE_TYPE,
  Lr as aggregateUnsubscriberAsyncs,
  Rr as aggregateUnsubscribers,
  ...
  _r as transformAndEnsureRegExpRegExpArray,
  Br as usfmMarkers,
  Ue as wait,
