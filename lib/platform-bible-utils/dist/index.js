...
function an(r) {
  ...
  return w(r) ? r.includes(e) : Ce(r).includes(e);
}
function cn(r) {
  const e = "401 Unauthorized error while getting shared projects.", t = "User registration is not valid. Cannot retrieve resources from DBL.", n = w(r) ? r : Ce(r);
...
], Rt = 1, Vt = Je.length - 1, qt = 1, Lt = 1, On = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, Ft = (r) => {
...
function ze(r, e, t, n) {
  ...
  if (i && s) {
    ...
    if (!l && !u) {
      ...
      Q.includes(o.type) && w(d) && R(d) && (h = h.slice(0, -1));
      const f = p[p.length - 1];
      if (Q.includes(a.type) && w(f) && R(f) && (p = p.slice(0, -1)), h.length !== p.length) return !1;
...
const Ln = (r) => (...e) => r.map((n) => n(...e)).every((n) => n), Fn = (r) => async (...e) => {
...
var dr = "[object Arguments]", mr = "[object Boolean]", gr = "[object Date]", yr = "[object Error]", br = "[object Map]", vr = "[object Number]", kr = "[object Object]", Nr = "[object RegExp]", wr = "[object Set]", Ir = "[object String]", xr = "[object URL]", Er = Array.isArray, Ae = typeof ArrayBuffer == "function" && ArrayBuffer.isView ? ArrayBuffer.isView : null, Me = Object.assign, Sr = Object.prototype.toString.call.bind(Object.prototype.toString);
function Ar(r) {
  ...
  return function(f, m, b) {
    ...
    if (N === Set)
      return u(f, m, b);
    var S = Sr(f);
    return S === gr ? t(f, m, b) : S === Nr ? l(f, m, b) : S === br ? s(f, m, b) : S === wr ? u(f, m, b) : S === kr ? typeof f.then != "function" && typeof m.then != "function" && a(f, m, b) : S === xr ? p(f, m, b) : S === yr ? n(f, m, b) : S === dr ? a(f, m, b) : S === mr || S === vr || S === Ir ? c(f, m, b) : !1;
  };
}
function Mr(r) {
  var e = r.circular, t = r.createCustomConfig, n = r.strict, i = {
    ...
    areMapsEqual: n ? Ne(Ee, J) : Ee,
    areNumbersEqual: cr,
    areObjectsEqual: n ? J : lr,
    arePrimitiveWrappersEqual: ur,
    areRegExpsEqual: fr,
    areSetsEqual: n ? Ne(Se, J) : Se,
...
const Hn = (r) => r.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function Yn() {
...
function Zn(r, e, t = /* @__PURE__ */ new Date()) {
    ...
    return i.push({ fragment: e, indexInUsfm: n.length }), n += a === "optbreak" ? "//" : `\\${h}`, a !== "optbreak" && (i.push({
      ...
      i.push({
        ...
        if ("isClosingMarker" in N.fragment) {
          const { isClosingMarker: S, ...D } = N.fragment, Ze = {
            ...D,
            forMarker: e,
...
export {
  ...
  ni as USFM_MARKERS_MAP_PARATEXT_3_0,
  dn as UnsubscriberAsyncList,
  g as UsjReaderWriter,
  O as VERSE_TYPE,
  Fn as aggregateUnsubscriberAsyncs,
  Ln as aggregateUnsubscribers,
  ri as applyThemeStylesheet,
  qn as areUsjContentsEqualExceptWhitespace,
  H as at,
  V as charAt,
  bn as codePointAt,
  Bn as compareScrRefs,
  ...
  ti as expandThemeContribution,
  Wn as formatBytes,
  Qn as formatRelativeDate,
  vn as formatReplacementString,
  Ct as formatReplacementStringToArray,
  Rn as formatScrRef,
  Zn as formatTimeSpan,
  sn as getAllObjectFunctionNames,
  Ft as getChaptersForBook,
  Yn as getCurrentLocale,
  Ke as getDefaultCallerSequence,
  Ce as getErrorMessage,
  Gn as getFormatCallerFunction,
  Jt as getLocalizeKeyForScrollGroupId,
  Dn as getLocalizeKeysForScrollGroupIds,
  jn as getLocalizedIdFromBookNumber,
  Ht as getNthCaller,
  _n as getPaneSizeLimits,
  Vn as getSectionForBook,
  Jr as getStylesheetForTheme,
  rn as groupBy,
  Jn as hasCustomParatextTags,
  Hn as htmlEncode,
  $t as includes,
  _ as indexOf,
  ...
  Tn as truncateOmittingMiddleWords,
  pt as wait,
