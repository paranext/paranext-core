...
const et = "ABORTED", tt = "ALREADY_EXISTS", rt = "CANCELLED", nt = "DATA_LOSS", it = "DEADLINE_EXCEEDED", st = "FAILED_PRECONDITION", at = "INTERNAL", ot = "INVALID_ARGUMENT", ct = "NOT_FOUND", dt = "OUT_OF_RANGE", pt = "PERMISSION_DENIED", lt = "RESOURCE_EXHAUSTED", ut = "UNAUTHENTICATED", ht = "UNAVAILABLE", ft = "UNIMPLEMENTED", mt = "UNKNOWN", q = 1;
function gt(n, e) {
  if (!n)
    return {
      message: "",
...
const kt = (n) => (...e) => n.map((r) => r(...e)).every((r) => r), bt = (n) => async (...e) => {
...
const wt = (n) => n.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function St() {
...
const Se = 1e3, Q = 60, ee = Q * 60, Pe = ee * 24;
function $t(n, e, t = /* @__PURE__ */ new Date()) {
  const r = Math.floor((e.getTime() - t.getTime()) / Se), s = Math.round(r / Pe);
...
function Bt(n, e, t) {
  ...
  return s.id = `${je}${t ? `-${t}` : ""}`, s.dataset.themeId = n.id, s.textContent = Ue(n), this.document.head.appendChild(s), s;
}
function ne(n) {
...
USJ: ${JSON.stringify(
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const u = parseInt(b.colspan, 10), h = Fe.exec(r);
      if (h != null && h[1]) {
  ...
  textContentToUsfm(e) {
    return {
      usfm: this.shouldAllowInvisibleCharacters ? e : e.replace(Oe, "~"),
    ...
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        if (h.type === be && h.code)
          ...
          const f = (m = Re.exec(h.number)) == null ? void 0 : m[1];
          if (!f)
            console.warn(
