...
const ve = class X {
  ...
  parse(e) {
    ...
    if (i.length !== 2 || we.bookIdToNumber(n[0]) === 0 || !Number.isInteger(a) || a < 0 || !X.isVerseParseable(i[1]))
      throw new Ct("Invalid reference : " + e);
  ...
  updateInternal(e, n, i) {
    this.bookNum = we.bookIdToNumber(e), this.chapter = n, this.verse = i;
  }
};
    ...
    for (var I = 0, P = 0, re = /.*(?:\r\n?|\n)|.*$/g, K = O.locator, Ce = [{ currentNSMap: A }], ye = [], se = 0; ; ) {
      try {
        ...
        switch (d.charAt(oe + 1)) {
          ...
          default:
            K && y(oe);
            var Q = new Me(), ge = Ce[Ce.length - 1].currentNSMap, le = D(d, oe, Q, ge, G, B, N), Je = Q.length;
...
const $t = ["chapter", "book", "para", "row", "sidebar", jt], Bn = "​", Ur = [
  ...
  book: we.bookNumberToId(
    Math.max(
      _n,
      Math.min(we.bookIdToNumber(t.book) + e, Pn)
    )
  ),
  chapterNum: 1,
  verseNum: 1
...
function Er(t) {
  return new Gt(we.bookIdToNumber(t.book), t.chapterNum, t.verseNum).BBBCCCVVV;
}
function mi(t, e) {
  return Er(t) - Er(e);
}
...
function gi(t, e, n, i) {
  ...
  switch (e ?? "id") {
    case "English":
      a = we.bookIdToEnglishName(t.book);
      break;
    case "id":
      a = t.book;
      break;
