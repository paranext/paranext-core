...
function z(s, e, t) {
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < X && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
    const { suppressedCount: l } = d, f = [...d.suppressedErrorSamples];
    d.suppressedCount = 0, d.suppressedErrorSamples.clear(), d.summaryTimeout = void 0, l > 0 && r(n(s, l, f));
...
USJ: ${JSON.stringify(
  ...
  static findNextMatchingNodeOrClosingFragmentUsingWorkingStack(e, t, r, i) {
    ...
    if (!_(c)) {
      ...
      let d;
      t.some((l) => {
        const f = l.parent.content[l.index];
        return !_(f) && r.includes(f.type) ? (d = f, !0) : !1;
      }), d && (n = d);
    }
    for (; n !== void 0; ) {
  ...
  findNextLocationOfMatchingText(e, t, r = 1e3) {
    ...
    if (m.findNextMatchingNodeUsingWorkingStack(
      ...
      (h, g) => {
        ...
        if (l && m.areStackItemsShallowEqual(v, l)) {
        ...
        n += y.length, i = `${i}${y}`;
        const x = i.indexOf(t);
        return x < 0 ? (c += i.length, i.length > t.length && (i = i.substring(i.length - t.length)), c -= i.length, n > r) : (p = c + x, !0);
      }
    ), p < 0) return;
    n = 0;
    let f = 0, b = [];
    const u = m.findNextMatchingNodeUsingWorkingStack(
      e.node,
      this.convertJsonPathToWorkingStack(e.documentLocation.jsonPath),
      P,
      (h, g) => typeof h != "string" || (n += h.length, n < p + 1) ? !1 : (f = p - n + h.length, b = g, !0)
    );
  ...
  collectSearchText(e, t) {
    ...
    let p, d = !1, l = !1, f = 0, b = r.node;
    for (; b !== void 0; )
      b = m.findNextMatchingNodeUsingWorkingStack(
        r.node,
        ...
        (u, h) => {
          ...
          if (e && h.some((y) => {
            ...
            return !1;
          if (t) {
            const g = m.findNearestBlockAncestor(h);
            d && g !== p && !l && c.add(f), p = g, d = !0, l = !1;
          }
          return i.push(u), n.set(f, {
            node: u,
            documentLocation: {
              offset: 0,
              jsonPath: m.convertWorkingStackToJsonPath(h)
            }
          }), f += u.length, !1;
        }
      );
  ...
  static buildSearchResult(e, t, r, i) {
    ...
    return { text: i ? t.substring(n, c) : e[0], start: d, end: f };
  }
  search(e, t) {
    ...
    const d = c && m.hasSearchWhitespaceGroup(e.source), { textChunks: l, fullTextIndexMap: f, blockBoundaryOffsets: b } = this.collectSearchText(
      i,
      d
    ), u = l.join(""), h = n === "NFD" ? m.buildNFDToOriginalPositionMap(u) : void 0, g = h ? u.normalize("NFD") : u, y = d && !e.flags.includes("d") ? new RegExp(e.source, `${e.flags}d`) : e, v = e.lastIndex;
    try {
      let x = y.exec(g);
      for (; x; )
        if (x[0].length === 0) {
          ...
          if (!y.global) break;
          y.lastIndex = m.advancePastCodePoint(g, x.index), x = y.exec(g);
        } else {
          if (p.push(
            m.buildSearchResult(x, u, f, h)
          ), !y.global) break;
          x = y.exec(g);
        }
    } finally {
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const u = parseInt(b.colspan, 10), h = Re.exec(r);
      if (h != null && h[1]) {
        const g = parseInt(h[1], 10);
        !Number.isNaN(g) && !Number.isNaN(u) && (p = `${r}-${g + u - 1}`, l = {
          ...l,
  ...
  openingMarkerToUsfm(e, t) {
    let r = "";
    const i = [], { markerName: n, markerInfo: c, markerType: p, markerTypeInfo: d, attributeMarkerInfoEntries: l } = this.getInfoForMarker(e), f = e;
...
`);
    ...
    return i.push({ fragment: e, indexInUsfm: r.length }), r += p === "optbreak" ? "//" : `\\${b}`, p !== "optbreak" && (i.push({
    ...
    }), c.textContentAttribute && f[c.textContentAttribute] && (i.push({
      fragment: { attributeValueForKey: c.textContentAttribute, forMarker: e },
      indexInUsfm: r.length
    }), r += `${f[c.textContentAttribute]} `), c.attributeMarkers && l.forEach(([u, h]) => {
      const g = f[h.attributeMarkerAttributeName];
      if (!g) return;
      const y = {
        type: h.type,
        marker: u,
      ...
      const { usfm: x } = this.textContentToUsfm(g);
      i.push({
        fragment: {
        ...
        indexInUsfm: r.length
      }), r += x, r = this.addMarkerUsfmToString(
        r,
        {
          isClosingMarker: !0,
          forMarker: y
        ...
        if (m.isFragmentAMarker(M.fragment)) {
          i.push({
            ...M,
            fragment: {
        ...
        if ("isClosingMarker" in M.fragment) {
          const { isClosingMarker: j, ...S } = M.fragment, se = {
            ...S,
            forMarker: e,
  ...
  closingMarkerToUsfm(e, t) {
    const {
      ...
      var y, v;
      return !(g === "type" || g === "marker" || g === "content" || g === "closed" || (y = p.skipOutputAttributeToUsfm) != null && y.includes(g) || (v = n.leadingAttributes) != null && v.includes(g) || n.textContentAttribute === g || d.some(
        ([, x]) => x.attributeMarkerAttributeName === g
      ));
    }), f = e;
    ...
    let b = !0;
    if ((f.closed === "false" || n.isClosingMarkerOptional && !this.markersMap.shouldOptionalClosingMarkersBePresent && f.closed !== "true") && (b = !1), n.independentClosingMarkers && n.independentClosingMarkers.length > 0 && b) {
      const g = {
        type: c,
      ...
      let y = "";
      const v = [], { usfm: x, fragmentsInfo: M } = this.openingMarkerToUsfm(g, t), j = M.find((S) => m.isFragmentAMarker(S.fragment));
      if (!j)
        throw new Error(
      ...
      }), y += x, r !== g.marker && (y = this.addMarkerUsfmToString(
        y,
        {
          isClosingMarker: !0,
          forMarker: g
    ...
    }), u += f[n.defaultAttribute]) : l.forEach((g, y) => {
      const v = c === "figure" && g === "file" ? "src" : g;
      y > 0 && (u += " "), h.push({
  ...
  addMarkerUsfmToString(e, t, r, i) {
    let n = e, c, p;
    const { markerNameOriginal: d, markerType: l, markerTypeInfo: f } = this.getInfoForMarker(
    ...
    if ("isClosingMarker" in t) {
      ...
      p = h, c = u, f.isCloseable && f.isClosingMarkerEmpty && // No contents
 ...
 `);
  ...
  static transferFragmentsInfoArrayToMaps(e, t, r, i, n, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        const h = d.fragment;
        if (h.type === xe && h.code)
          r.bookId = h.code, r.chapterNum = 0, r.verseNum = 0, c[I] && (c[r.bookId] = c[I], delete c[I]);
        else if (h.type === H && h.number) {
          const g = parseInt(h.number, 10);
          if (Number.isNaN(g))
            console.warn(
        ...
        } else if (h.type === E && h.number) {
          const g = (f = De.exec(h.number)) == null ? void 0 : f[1];
          if (!g)
            console.warn(
              `Found ${E} type marker with number ${h.number}, but could not find starting verse number in it. Continuing using previous verse number ${r.verseNum}`
            );
          else {
            const y = parseInt(g, 10);
            Number.isNaN(y) ? console.warn(
              `Found ${E} type marker with number ${h.number}, but could not parse starting verse number from ${g}. Continuing using previous verse number ${r.verseNum}`
            ) : (u = (b = c[r.bookId]) == null ? void 0 : b[r.chapterNum]) != null && u[y] ? console.warn(`Found ${E} marker with existing number ${y} after
      ...
      const l = d.nodeAndDocumentLocation.documentLocation.jsonPath, f = n.get(l);
      f ? f.push(d) : n.set(l, [d]), c[r.bookId] || (c[r.bookId] = {}), c[r.bookId][r.chapterNum] || (c[r.bookId][r.chapterNum] = {}), c[r.bookId][r.chapterNum][r.verseNum] === void 0 && (c[r.bookId][r.chapterNum][r.verseNum] = d.indexInUsfm);
    }), e.splice(0);
  }
  ...
  calculateUsfmProperties() {
    ...
    return m.findNextMatchingNodeOrClosingFragmentUsingWorkingStack(
      ...
      (f, b) => {
        if (typeof f != "object") {
          const { usfm: h, fragmentsInfo: g } = this.textContentToUsfm(f);
        ...
        let u;
        return b.length > 0 && (u = b[b.length - 1].parent), "isClosingMarker" in f ? l.length > 0 && l[l.length - 1] === f.forMarker ? (l.pop(), !1) : (e = this.addMarkerUsfmToString(
          e,
          f,
          u,
          n
        ), p(b), f.forMarker.type === "book" && d && (e = this.addMarkerUsfmToString(e, d, u, n), p(b), d = void 0), !1) : this.shouldSkipOutputMarkerToUsfm(f) ? (l.push(f), !1) : m.isTopLevelUsjMarker(f, b) && !d ? (f.version !== "3.0" && (d = f), !1) : (e = this.addMarkerUsfmToString(e, f, u, n), p(b), !1);
      }
