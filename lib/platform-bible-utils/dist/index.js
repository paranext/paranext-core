...
var ue = (s, e, t) => e in s ? le(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : s[e] = t;
var y = (s, e, t) => ue(s, typeof e != "symbol" ? e + "" : e, t);
import { d as N, i as _ } from "./scripture-util-DArajUVn.js";
import { D as Ht, F as Jt, a as Kt, b as Gt, G as Yt, L as Xt, M as Wt, S as Zt, c as Qt, e as er, f as tr, g as rr, h as nr, j as ir, k as sr, l as ar, m as or, n as cr, o as dr, p as pr, q as lr, r as ur, s as hr, t as fr, u as mr, v as gr, w as yr, x as kr, y as br, z as vr, A as Mr, B as xr, C as _r, E as Er, H as Ir, I as wr, J as Sr, K as Pr, N as Tr, O as Cr, P as Ar, Q as qr, R as $r, T as jr, U as Nr, V as Fr, W as Ur, X as Rr, Y as Or, Z as Dr, _ as Lr, $ as Br, a0 as Vr, a1 as zr, a2 as Hr, a3 as Jr, a4 as Kr, a5 as Gr, a6 as Yr, a7 as Xr, a8 as Wr, a9 as Zr, aa as Qr, ab as en, ac as tn, ad as rn } from "./scripture-util-DArajUVn.js";
import { Mutex as he } from "async-mutex";
import fe from "dompurify";
import { deepEqual as me } from "fast-equals";
import { USJ_TYPE as ge } from "@eten-tech-foundation/scripture-utilities";
...
class ve {
  ...
  constructor(e, t) {
    ...
    y(this, "latestOutput");
    y(this, "options");
    y(this, "onDidRebuildEmitter", new be());
...
class Qe {
  ...
  get(e) {
    ...
    return t || (t = new xe(), this.mutexesByID.set(e, t), t);
  }
  /**
...
class tt {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const r = { promise: Ee }, i = t.catch((n) => this.logger.warn(`Error in promise for ${e}: ${n.message}`)).finally(() => {
...
const ee = 1e4, D = ee / 1e3, te = 3, G = /* @__PURE__ */ new Map(), Ie = {
  unsubscribed: {
    ...
    suppressed: (s, e) => `UnsubscriberAsyncList ${s}: ${e} more unsubscribers arrived after the list was run in the last ${D}s and were unsubscribed immediately; per-occurrence warnings suppressed.`
  },
  failed: {
    log: (s) => console.error(s),
    first: (s) => `UnsubscriberAsyncList ${s}: Unsubscriber added after the list was run failed!`,
    suppressed: (s, e) => `UnsubscriberAsyncList ${s}: ${e} more unsubscribers added after the list was run failed in the last ${D}s; per-occurrence errors suppressed.`
  },
  threw: {
    log: (s) => console.error(s),
    ...
    suppressed: (s, e, t) => `UnsubscriberAsyncList ${s}: ${e} more unsubscribers added after the list was run threw in the last ${D}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${te}): ${t.join(" | ")}`
  }
};
function Y(s, e, t) {
  const { log: r, first: i, suppressed: n } = Ie[e], c = `${s}\0${e}`, p = Date.now(), d = G.get(c);
  if (!d || p >= d.closesAt) {
    r(i(s, t)), G.set(c, {
      closesAt: p + ee,
      suppressedCount: 0,
...
const it = "ABORTED", st = "ALREADY_EXISTS", at = "CANCELLED", ot = "DATA_LOSS", ct = "DEADLINE_EXCEEDED", dt = "FAILED_PRECONDITION", pt = "INTERNAL", lt = "INVALID_ARGUMENT", ut = "NOT_FOUND", ht = "OUT_OF_RANGE", ft = "PERMISSION_DENIED", mt = "RESOURCE_EXHAUSTED", gt = "UNAUTHENTICATED", yt = "UNAVAILABLE", kt = "UNIMPLEMENTED", bt = "UNKNOWN", U = 1;
function vt(s, e) {
  if (!s)
    return {
      message: "",
...
const xt = (s) => (...e) => s.map((r) => r(...e)).every((r) => r), _t = (s) => async (...e) => {
  const t = s.map(async (r) => r(...e));
  return (await Promise.all(t)).every((r) => r);
}, we = "book", X = "chapter", P = "verse", C = "***", W = "ws";
...
function Et(s) {
  var e;
  return ((e = re[s]) == null ? void 0 : e.type) === a.Paragraph || s === "v";
}
function It(s) {
  const e = re[s];
...
function Se(s, e) {
  const t = e && e.length > 0 ? e : ne();
  return t[s % t.length];
}
function St(s, e) {
  const t = e && e.length > 0 ? e : ne(), r = (() => {
    const i = /* @__PURE__ */ new Map();
    let n = 0;
    return s.forEach((c, p) => {
      c.caller === "+" && (i.set(p, Se(n, t)), n += 1);
    }), i;
  })();
  return (i, n) => {
    if (i === "+") {
...
function Te(s, e) {
  ...
  return Object.keys(t).forEach((n) => {
    i && (Object.hasOwn(r, n) && Te(r[n], t[n]) || (i = !1));
  }), i;
}
function Z(s, e, t) {
...
const Ct = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function At() {
  return typeof navigator < "u" && navigator.languages ? navigator.languages[0].replace(/@posix$/i, "") : new ye().resolvedOptions().locale;
}
function qt(s, e = 2) {
...
const Ae = 1e3, ie = 60, se = ie * 60, qe = se * 24;
function jt(s, e, t = /* @__PURE__ */ new Date()) {
  const r = Math.floor((e.getTime() - t.getTime()) / Ae), i = Math.round(r / qe);
  if (Math.abs(i) >= 1) return s.format(i, "day");
  const n = Math.round(r / se);
  if (Math.abs(n) >= 1) return s.format(n, "hour");
  const c = Math.round(r / ie);
  return Math.abs(c) >= 1 ? s.format(c, "minute") : s.format(r, "second");
}
function Nt(s, e, t, r, i = {
  year: "numeric",
  month: "short",
  day: "numeric"
}) {
...
function Lt(s, e, t) {
  ...
  return i.id = `${Re}${t ? `-${t}` : ""}`, i.dataset.themeId = s.id, i.textContent = De(s), this.document.head.appendChild(i), i;
}
function ce(s) {
  return Object.freeze(s), s == null || Object.getOwnPropertyNames(s).forEach(function(t) {
    ...
    s[t] !== null && (typeof s[t] == "object" || typeof s[t] == "function") && !Object.isFrozen(s[t]) && ce(s[t]);
  }), s;
}
const B = ce({
  version: "3.0.7",
...
Object.freeze(j);
const de = ["para", "table", "row", "cell", "sidebar"];
Object.freeze(de);
...
USJ: ${JSON.stringify(
    ...
    else if (m.areUsjVersionsCompatible(this.usj.version, B.version))
      this.markersMap = B;
    else
      throw new Error(
  ...
  static isTopLevelUsjMarker(e, t) {
    return typeof e == "object" && e.type === ge && t.length === 0;
  }
  /**
  ...
  getEffectiveBookId(e) {
    const t = Object.keys(this.indicesInUsfmByVerseRef), r = t.length === 0 || t.length === 1 && t[0] === C, i = r ? C : e;
    if (!this.indicesInUsfmByVerseRef[i])
      throw new Error(
        `Book ID ${e} not found in USJ! ${r ? `There seems to be no USJ content because there is no content in ${C} either` : `Book IDs in USJ: ${JSON.stringify(t)}`}`
      );
    return i;
  }
  /**
  ...
  static findNearestBlockAncestor(e) {
    for (let t = e.length - 1; t >= 0; t--) {
      const { parent: r } = e[t];
      if (r && "type" in r && de.includes(r.type)) return r;
    }
  }
  /**
  ...
  search(e, t) {
    ...
    }, d = [], l = new K(), h = /* @__PURE__ */ new Set();
    let b, f = !1, u = 0, g = p.node;
    for (; g !== void 0; )
      g = m.findNextMatchingNodeUsingWorkingStack(
        p.node,
        ...
        // eslint-disable-next-line no-loop-func
        (I, A) => {
          if (typeof I != "string" || r && A.some((O) => {
            const S = O.parent;
            if (!S || !("type" in S) || S.type === "char") return !1;
            let q;
            return "style" in S && typeof S.style == "string" ? q = S.style : "marker" in S && typeof S.marker == "string" && (q = S.marker), q !== void 0 && !r.has(q);
          }))
            return !1;
          if (n) {
            const T = m.findNearestBlockAncestor(A);
            f && T !== b && h.add(u), b = T, f = !0;
          }
          return d.push(I), l.set(u, {
            node: I,
            documentLocation: {
              offset: 0,
              jsonPath: m.convertWorkingStackToJsonPath(A)
            }
          }), u += I.length, !1;
        }
      );
    const k = d.join(""), v = i === "NFD" ? m.buildNFDToOriginalPositionMap(k) : void 0, E = v ? k.normalize("NFD") : k, M = e.source.includes(`(?<${W}`), w = n && M && !e.flags.includes("d") ? new RegExp(e.source, `${e.flags}d`) : e;
    let x = w.exec(E);
    for (; x; )
      if (x[0].length === 0) {
        if (!w.global || w.lastIndex >= E.length) break;
        w.lastIndex += 1, x = w.exec(E);
      } else if (n && m.hasWhitespaceGapAwayFromBoundary(
        x,
        h,
        v
      )) {
        if (!w.global) break;
        w.lastIndex = x.index + 1, x = w.exec(E);
      } else {
        const I = v ? v[x.index] : x.index, A = v ? v[x.index + x[0].length] : x.index + x[0].length;
        if (I < 0 || I >= k.length)
          throw new Error(`Match index out of bounds: ${I}`);
        const T = l.findClosestLessThanOrEqual(I);
        if (!T)
          throw new Error(`Internal error: no starting node found for index ${I}`);
        const O = {
          node: T.value.node,
          documentLocation: {
            jsonPath: T.value.documentLocation.jsonPath,
            offset: I - T.key
          }
        }, S = l.findClosestLessThanOrEqual(A - 1);
        if (!S)
          throw new Error(`Internal error: no ending node found for index ${I}`);
        const q = {
          node: S.value.node,
          documentLocation: {
            jsonPath: S.value.documentLocation.jsonPath,
            offset: A - S.key
          }
        }, pe = v ? k.substring(I, A) : x[0];
        if (c.push({ text: pe, start: O, end: q }), !w.global) break;
        x = w.exec(E);
      }
    return c;
  }
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && d === "cell" && b.colspan) {
      const f = parseInt(b.colspan, 10), u = Be.exec(r);
      if (u != null && u[1]) {
...
`);
    ...
    return i.push({ fragment: e, indexInUsfm: r.length }), r += p === "optbreak" ? "//" : `\\${b}`, p !== "optbreak" && (i.push({
      ...
      i.push({
        ...
        if ("isClosingMarker" in M.fragment) {
          const { isClosingMarker: w, ...x } = M.fragment, I = {
            ...x,
            forMarker: e,
  ...
  closingMarkerToUsfm(e, t) {
    const {
      ...
      var k, v;
      return !(g === "type" || g === "marker" || g === "content" || g === "closed" || (k = p.skipOutputAttributeToUsfm) != null && k.includes(g) || (v = n.leadingAttributes) != null && v.includes(g) || n.textContentAttribute === g || d.some(
    ...
    if ((h.closed === "false" || n.isClosingMarkerOptional && !this.markersMap.shouldOptionalClosingMarkersBePresent && h.closed !== "true") && (b = !1), n.independentClosingMarkers && n.independentClosingMarkers.length > 0 && b) {
      ...
      let k = "";
      const v = [], { usfm: E, fragmentsInfo: M } = this.openingMarkerToUsfm(g, t), w = M.find((x) => m.isFragmentAMarker(x.fragment));
      if (!w)
        throw new Error(
...
`) && (n.length === 0 ? (p = p.map((f) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, r = !1) {
    ...
    return !r && !L($.toPathArray(i), $.toPathArray(c)) ? !1 : L(n, p);
  }
  ...
  static transferFragmentsInfoArrayToMaps(e, t, r, i, n, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        const u = d.fragment;
        if (u.type === we && u.code)
          r.bookId = u.code, r.chapterNum = 0, r.verseNum = 0, c[C] && (c[r.bookId] = c[C], delete c[C]);
          ...
          const g = (h = Ve.exec(u.number)) == null ? void 0 : h[1];
          if (!g)
            console.warn(
...
export {
  ...
  Ht as DEBOUNCE_CANCELED_ERROR_MESSAGE,
  ye as DateTimeFormat,
  ve as DocumentCombiner,
  Ze as EventRollingTimeCounter,
  dt as FAILED_PRECONDITION,
  Jt as FIRST_SCR_BOOK_NUM,
  Kt as FIRST_SCR_CHAPTER_NUM,
  Gt as FIRST_SCR_VERSE_NUM,
  Yt as GraphemeString,
  pt as INTERNAL,
  lt as INVALID_ARGUMENT,
  Xt as LAST_SCR_BOOK_NUM,
  Wt as MAX_PADDING_LENGTH,
  Ft as MODIFIER_KEYS,
  a as MarkerType,
  xe as Mutex,
  Qe as MutexMap,
  ...
  Rt as computeEffectiveStructureProtection,
  or as createSyncProxyForAsyncObject,
  cr as debounce,
  N as deepClone,
  L as deepEqual,
  dr as defaultScrRef,
  Ce as deserialize,
  Ot as doesCatalogRowCoverProject,
  pr as endsWith,
  lr as ensureArray,
  ur as escapeStringRegexp,
  Dt as expandThemeContribution,
  qt as formatBytes,
  Nt as formatRelativeDate,
  hr as formatReplacementString,
  fr as formatReplacementStringToArray,
  mr as formatScrRef,
  gr as formatScrRefRange,
  jt as formatTimeSpan,
  yr as getAllObjectFunctionNames,
  kr as getChaptersForBook,
  At as getCurrentLocale,
  ne as getDefaultCallerSequence,
  br as getErrorMessage,
  St as getFormatCallerFunction,
  Ut as getLocalizeKeyForPhysicalKey,
  vr as getLocalizeKeyForScrollGroupId,
  Mr as getLocalizeKeysForScrollGroupIds,
  xr as getLocalizedIdFromBookNumber,
  Se as getNthCaller,
  Pt as getPaneSizeLimits,
  _r as getSectionForBook,
  ...
  wr as indexOf,
  Et as isBlockMarker,
  It as isCharacterMarker,
  Sr as isErrorMessageAboutParatextBlockingInternetAccess,
  Pr as isErrorMessageAboutRegistryAuthFailure,
  Tr as isLocalizeKey,
  Mt as isPlatformError,
  Cr as isSelectableInvisibleCharOrWhiteSpace,
  ...
  qr as isolateBidi,
  $r as lastIndexOf,
  Ne as localizedStringsDocumentSchema,
  Fe as menuDocumentSchema,
  jr as newGuid,
  vt as newPlatformError,
  Nr as normalize,
  $t as normalizeProjectId,
  Fr as normalizeScriptureSpaces,
  Ur as offsetBook,
  Rr as offsetChapter,
  Or as offsetVerse,
  Dr as ordinalCompare,
  Lr as padEnd,
  Br as padStart,
  $e as projectSettingsDocumentSchema,
  Vr as retryUntil,
  wt as sanitizeHtml,
  zr as scrRefToBBBCCC,
  Hr as scrRefToBBBCCCVVV,
  Z as serialize,
  je as settingsDocumentSchema,
  Jr as slice,
  Kr as split,
  Gr as startsWith,
  Yr as stringLength,
  Xr as substring,
  Ue as themeDocumentSchema,
  Wr as toArray,
  Zr as toKebabCase,
  Qr as transformAndEnsureRegExpArray,
  en as transformAndEnsureRegExpRegExpArray,
  re as usfmMarkers,
  tn as wait,
  rn as waitForDuration
};
