...
import { indexOf as Se, limit as le, length as we, substring as Pe, toArray as Ce, substr as Te } from "stringz";
...
function Rt(r, e = 300) {
  let t, n, s, i;
  const c = (...d) => (clearTimeout(t), n || (n = new Promise((l, p) => {
    s = l, i = p;
  })), t = setTimeout(async () => {
    try {
      s(await r(...d));
  ...
  return c.cancel = () => {
    clearTimeout(t), n && (s(void 0), n = void 0);
  }, c;
}
function Lt(r, e, t) {
...
const Qt = "ABORTED", er = "ALREADY_EXISTS", tr = "CANCELLED", rr = "DATA_LOSS", nr = "DEADLINE_EXCEEDED", ir = "FAILED_PRECONDITION", sr = "INTERNAL", ar = "INVALID_ARGUMENT", or = "NOT_FOUND", cr = "OUT_OF_RANGE", dr = "PERMISSION_DENIED", lr = "RESOURCE_EXHAUSTED", pr = "UNAUTHENTICATED", ur = "UNAVAILABLE", hr = "UNIMPLEMENTED", fr = "UNKNOWN", F = 1;
function mr(r, e) {
  if (!r)
    return {
      message: "",
...
function ye(r, e, t, n) {
  ...
  if (s && i) {
    const c = ne(r), d = ne(t);
    if (c !== d) {
      if (!T(D(c, -1) ?? "") && !T(D(d, -1) ?? "") || !se(r, e) || !se(t, n)) return !1;
      let l = c;
      for (; T(D(l, -1) ?? ""); ) l = Z(l, 0, -1);
      let p = d;
      for (; T(D(p, -1) ?? ""); ) p = Z(p, 0, -1);
      if (l !== p) return !1;
    }
  } else if (!s && !i) {
