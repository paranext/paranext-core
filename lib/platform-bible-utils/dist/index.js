...
import { Canon as w, VerseRef as de } from "@sillsdev/scripture";
import { USJ_TYPE as le } from "@eten-tech-foundation/scripture-utilities";
import { indexOf as we, limit as pe, length as Pe, substring as Te, toArray as Ce, substr as Ae } from "stringz";
...
class Yt {
  ...
  cleanupPromiseChain(e) {
    ...
    if (!t) return;
    const n = { promise: ze }, s = t.catch((i) => this.logger.warn(`Error in promise for ${e}: ${i.message}`)).finally(() => {
...
const Qt = "ABORTED", er = "ALREADY_EXISTS", tr = "CANCELLED", rr = "DATA_LOSS", nr = "DEADLINE_EXCEEDED", ir = "FAILED_PRECONDITION", sr = "INTERNAL", ar = "INVALID_ARGUMENT", or = "NOT_FOUND", cr = "OUT_OF_RANGE", dr = "PERMISSION_DENIED", lr = "RESOURCE_EXHAUSTED", pr = "UNAUTHENTICATED", ur = "UNAVAILABLE", hr = "UNIMPLEMENTED", fr = "UNKNOWN", D = 1;
function mr(r, e) {
  if (!r)
    return {
      message: "",
...
function Ke(r, e) {
  ...
  let n = 0, s = 0;
  function i(d, l, p) {
    const f = C(r, s, l), g = t.length > 0 && M(t[t.length - 1]) ? `${t.pop()}${f}` : f;
...
const z = ["chapter", "book", "para", "row", "sidebar", le], Ze = "​", ge = [
...
], Qe = 1, et = ge.length - 1, tt = 1, rt = 1, Pr = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, nt = (r) => {
  ...
  book: w.bookNumberToId(
    Math.max(
      Qe,
      Math.min(w.bookIdToNumber(r.book) + e, et)
    )
  ),
  chapterNum: 1,
  verseNum: 1
...
function $r(r) {
  return new de(w.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCC;
}
function te(r) {
  return new de(w.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCCVVV;
}
function it(r, e) {
  return te(r) - te(e);
}
...
function ae(r, e) {
  if (!e || !z.includes(e.type)) return !1;
  if (!e.content)
    throw new Error(
...
function ke(r, e, t, n) {
  ...
  if (s && i) {
    const c = ie(r), d = ie(t);
    if (c !== d) {
      if (!A(R(c, -1) ?? "") && !A(R(d, -1) ?? "") || !ae(r, e) || !ae(t, n)) return !1;
      let l = c;
      for (; A(R(l, -1) ?? ""); ) l = Q(l, 0, -1);
      let p = d;
      for (; A(R(p, -1) ?? ""); ) p = Q(p, 0, -1);
      if (l !== p) return !1;
    }
  } else if (!s && !i) {
    ...
    if (!p && !f) {
      ...
      z.includes(c.type) && M(h) && A(h) && (g = g.slice(0, -1));
      const m = u[u.length - 1];
      if (z.includes(d.type) && M(m) && A(m) && (u = u.slice(0, -1)), g.length !== u.length) return !1;
...
USJ: ${JSON.stringify(
  ...
  getEffectiveBookId(e) {
    const t = Object.keys(this.indicesInUsfmByVerseRef), n = t.length === 0 || t.length === 1 && t[0] === T, s = n ? T : e;
    if (!this.indicesInUsfmByVerseRef[s])
      throw new Error(
        `Book ID ${e} not found in USJ! ${n ? `There seems to be no USJ content because there is no content in ${T} either` : `Book IDs in USJ: ${JSON.stringify(t)}`}`
      );
    return s;
  }
  /**
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && l === "cell" && g.colspan) {
      const u = parseInt(g.colspan, 10), h = wt.exec(n);
      if (h != null && h[1]) {
...
`) && (i.length === 0 ? (d = d.map((u) => ({
 ...
 `);
  ...
  static areUsjDocumentLocationsEqual(e, t, n = !1) {
    ...
    return !n && !J($.toPathArray(s), $.toPathArray(c)) ? !1 : J(i, d);
  }
  ...
  static transferFragmentsInfoArrayToMaps(e, t, n, s, i, c) {
    e.map((l) => {
      ...
      if (typeof l.fragment == "object" && "type" in l.fragment) {
        const h = l.fragment;
        if (h.type === ut && h.code)
          n.bookId = h.code, n.chapterNum = 0, n.verseNum = 0, c[T] && (c[n.bookId] = c[T], delete c[T]);
          ...
          const m = (f = Pt.exec(h.number)) == null ? void 0 : f[1];
          if (!m)
            console.warn(
...
export {
  ...
  Rr as areUsjContentsEqualExceptWhitespace,
  R as at,
  q as charAt,
  yr as codePointAt,
