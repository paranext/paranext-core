...
const V = ["chapter", "book", "para", "row", "sidebar", oe], Ye = "​", fe = [
...
], _e = 1, We = fe.length - 1, Ze = 1, Qe = 1, ur = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, et = (n) => {
  var e;
  return ((e = fe[n]) == null ? void 0 : e.chapters) ?? -1;
}, pr = (n, e) => ({
...
function he(n, e, t, r) {
  ...
  if (i && s) {
    ...
    if (!c && !u) {
      ...
      V.includes(o.type) && b(f) && E(f) && (d = d.slice(0, -1));
      const m = p[p.length - 1];
      if (V.includes(a.type) && b(m) && E(m) && (p = p.slice(0, -1)), d.length !== p.length) return !1;
...
USJ: ${JSON.stringify(
    ...
    else if (h.areUsjVersionsCompatible(this.usj.version, R.version))
      this.markersMap = R;
    else
      throw new Error(
  ...
  findNextLocationOfMatchingText(e, t, r = 1e3) {
    ...
    if (h.findNextMatchingNodeUsingWorkingStack(
      ...
      (f, m) => {
        ...
        if (c && h.areStackItemsShallowEqual(N, c)) {
          ...
          y = f.substring(e.documentLocation.offset), o += e.documentLocation.offset;
        }
        s += y.length, i = `${i}${y}`;
        const w = i.indexOf(t);
    ...
    s = 0;
    let u = 0, d = [];
    const p = h.findNextMatchingNodeUsingWorkingStack(
      e.node,
      this.convertJsonPathToWorkingStack(e.documentLocation.jsonPath),
      $,
      (f, m) => typeof f != "string" || (s += f.length, s < a + 1) ? !1 : (u = a - s + f.length, d = m, !0)
    );
      ...
      if (c[0].length > 0) {
        ...
        const f = {
          ...
          documentLocation: {
            ...
            offset: c.index + c[0].length - p.key
          }
        };
        t.push({ text: c[0], start: d, end: f });
      }
      if (!e.global) break;
      c = e.exec(l);
    }
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    s.attributeMarkers && s.attributeMarkers.forEach((p) => {
      const f = this.getMarkerInfo(p);
      f && "attributeMarkerAttributeName" in f && u.push([p, f]);
    });
    const d = e;
    if (t === "usfm" && l === "cell" && d.colspan) {
      const p = parseInt(d.colspan, 10), f = Mt.exec(r);
      if (f != null && f[1]) {
        const m = parseInt(f[1], 10);
        !Number.isNaN(m) && !Number.isNaN(p) && (a = `${r}-${m + p - 1}`, c = {
          ...c,
...
`);
    ...
    return i.push({ fragment: e, indexInUsfm: r.length }), r += a === "optbreak" ? "//" : `\\${d}`, a !== "optbreak" && (i.push({
      ...
      indexInUsfm: r.length
    }), r += `${u[o.textContentAttribute]} `), o.attributeMarkers && c.forEach(([p, f]) => {
      const m = u[f.attributeMarkerAttributeName];
      if (!m) return;
      const y = {
        type: f.type,
        marker: p,
  ...
  closingMarkerToUsfm(e, t) {
    ...
    let p = "";
    const f = [];
    if (c.length > 0 && (p += "|", c.length === 1 && c[0] === s.defaultAttribute ? (f.push({
      fragment: { attributeValueForKey: s.defaultAttribute, forMarker: e },
      indexInUsfm: p.length
    }), p += u[s.defaultAttribute]) : c.forEach((m, y) => {
  ...
  addMarkerUsfmToString(e, t, r, i) {
    ...
    if ("isClosingMarker" in t) {
      const { usfm: p, fragmentsInfo: f } = this.closingMarkerToUsfm(
        t.forMarker,
        d
      );
      a = f, o = p, u.isCloseable && u.isClosingMarkerEmpty && // No contents
      (!t.forMarker.content || t.forMarker.content.length === 0) && // No closing marker attributes
      !o.startsWith("|") && (s = h.removeEndSpace(s));
    } else {
      const { usfm: p, fragmentsInfo: f } = this.openingMarkerToUsfm(
        t,
        d
      );
      a = f, o = p;
 ...
 `);
  ...
  static transferFragmentsInfoArrayToMaps(e, t, r, i, s, o) {
    e.map((l) => {
      ...
      if (typeof l.fragment == "object" && "type" in l.fragment) {
        const f = l.fragment;
        if (f.type === lt && f.code)
          r.bookId = f.code, r.chapterNum = 0, r.verseNum = 0, o[M] && (o[r.bookId] = o[M], delete o[M]);
        else if (f.type === ne && f.number) {
          const m = parseInt(f.number, 10);
          if (Number.isNaN(m))
            console.warn(
              `Found ${ne} type marker with number ${f.number}, but could not parse chapter number from it. Continuing using previous chapter number ${r.chapterNum}`
            );
          else {
        ...
        } else if (f.type === I && f.number) {
          const m = (u = St.exec(f.number)) == null ? void 0 : u[1];
          if (!m)
            console.warn(
              `Found ${I} type marker with number ${f.number}, but could not find starting verse number in it. Continuing using previous verse number ${r.verseNum}`
            );
          else {
            const y = parseInt(m, 10);
            Number.isNaN(y) ? console.warn(
              `Found ${I} type marker with number ${f.number}, but could not parse starting verse number from ${m}. Continuing using previous verse number ${r.verseNum}`
            ) : (p = (d = o[r.bookId]) == null ? void 0 : d[r.chapterNum]) != null && p[y] ? console.warn(`Found ${I} marker with existing number ${y} after
  ...
  calculateUsfmProperties() {
    ...
    return h.findNextMatchingNodeOrClosingFragmentUsingWorkingStack(
      ...
      (u, d) => {
        if (typeof u != "object") {
          const { usfm: f, fragmentsInfo: m } = this.textContentToUsfm(u);
        ...
        let p;
        return d.length > 0 && (p = d[d.length - 1].parent), "isClosingMarker" in u ? c.length > 0 && c[c.length - 1] === u.forMarker ? (c.pop(), !1) : (e = this.addMarkerUsfmToString(
          e,
          u,
          p,
          s
        ), a(d), u.forMarker.type === "book" && l && (e = this.addMarkerUsfmToString(e, l, p, s), a(d), l = void 0), !1) : this.shouldSkipOutputMarkerToUsfm(u) ? (c.push(u), !1) : h.isTopLevelUsjMarker(u, d) && !l ? (u.version !== "3.0" && (l = u), !1) : (e = this.addMarkerUsfmToString(e, u, p, s), a(d), !1);
      }
