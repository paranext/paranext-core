...
var Se = (r, e, t) => e in r ? Ie(r, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : r[e] = t;
var b = (r, e, t) => Se(r, typeof e != "symbol" ? e + "" : e, t);
import { Mutex as we } from "async-mutex";
import { Canon as S, VerseRef as le } from "@sillsdev/scripture";
import { USJ_TYPE as pe } from "@eten-tech-foundation/scripture-utilities";
import { indexOf as Pe, limit as ue, length as Ce, substring as Te, toArray as qe, substr as Ae } from "stringz";
import $e from "dompurify";
import { deepEqual as je } from "fast-equals";
...
function Kt(r) {
  ...
  return v(r) ? r.includes(e) : he(r).includes(e);
}
function Gt(r) {
  const e = "401 Unauthorized error while getting shared projects.", t = "User registration is not valid. Cannot retrieve resources from DBL.", n = v(r) ? r : he(r);
...
class Le {
  ...
  constructor(e, t) {
    ...
    b(this, "latestOutput");
    b(this, "options");
    b(this, "onDidRebuildEmitter", new Ue());
...
const tr = "ABORTED", rr = "ALREADY_EXISTS", nr = "CANCELLED", ir = "DATA_LOSS", sr = "DEADLINE_EXCEEDED", ar = "FAILED_PRECONDITION", or = "INTERNAL", cr = "INVALID_ARGUMENT", dr = "NOT_FOUND", lr = "OUT_OF_RANGE", pr = "PERMISSION_DENIED", ur = "RESOURCE_EXHAUSTED", hr = "UNAUTHENTICATED", fr = "UNAVAILABLE", mr = "UNIMPLEMENTED", gr = "UNKNOWN", F = 1;
function yr(r, e) {
  if (!r)
    return {
      message: "",
...
function Er(r, e, t = " ") {
  return e <= _(r) ? r : ue(r, e, t, "right");
}
function _r(r, e, t = " ") {
  return e <= _(r) ? r : ue(r, e, t, "left");
}
function Q(r, e) {
...
function ee(r, e, t) {
  ...
  if (e > n || t && (e > t && !(e >= 0 && e < n && t < 0 && t > -n) || t < -n))
    return "";
  const s = Q(n, e), i = t ? Q(n, t) : void 0;
  return C(r, s, i);
}
function te(r, e, t) {
  const n = [];
  if (t !== void 0 && t <= 0)
    return [r];
  if (e === "") return We(r).slice(0, t);
  let s = e;
  (typeof e == "string" || e instanceof RegExp && !Xe(e.flags, "g")) && (s = new RegExp(e, "g"));
  const i = r.match(s);
  let c = 0;
  if (!i) return [r];
  for (let d = 0; d < (t ? t - 1 : i.length); d++) {
    const l = U(r, i[d], c), p = _(i[d]);
...
function wr(r) {
  return r ? me(r).map((n) => new RegExp(n)) : [];
}
const Ze = (
...
const V = ["chapter", "book", "para", "row", "sidebar", pe], Qe = "​", ye = [
...
], et = 1, tt = ye.length - 1, rt = 1, nt = 1, Tr = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, it = (r) => {
  var e;
  return ((e = ye[r]) == null ? void 0 : e.chapters) ?? -1;
}, qr = (r, e) => ({
  book: S.bookNumberToId(
    Math.max(
      et,
      Math.min(S.bookIdToNumber(r.book) + e, tt)
    )
  ),
  chapterNum: 1,
  verseNum: 1
  ...
  ...r,
  chapterNum: Math.min(
    Math.max(rt, r.chapterNum + e),
    it(S.bookIdToNumber(r.book))
  ),
  verseNum: 1
}), $r = (r, e) => ({
  ...r,
  verseNum: Math.max(nt, r.verseNum + e)
});
async function jr(r, e, t) {
  const n = S.bookNumberToId(r);
  if (!ge(Intl.getCanonicalLocales(e)[0], "zh"))
    return t({
...
function Or(r) {
  return new le(S.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCC;
}
function re(r) {
  return new le(S.bookIdToNumber(r.book), r.chapterNum, r.verseNum).BBBCCCVVV;
}
function st(r, e) {
  return re(r) - re(e);
}
...
function ne(r, e) {
  const t = ke(r, e == null ? void 0 : e.optionOrLocalizedBookName), n = ct(
    r,
...
function Dr(r, e, t) {
  const n = ne(r, t);
  if (st(r, e) === 0) return n;
  const s = r.book === e.book && !(t != null && t.repeatBookName) ? "" : (t == null ? void 0 : t.endRefOptionOrLocalizedBookName) ?? (t == null ? void 0 : t.optionOrLocalizedBookName), i = ne(e, {
    ...t,
    optionOrLocalizedBookName: s
  });
  return `${n}${(t == null ? void 0 : t.rangeSeparator) ?? " - "}${i}`;
}
var dt = /* @__PURE__ */ ((r) => (r.OT = "OT", r.NT = "NT", r.DC = "DC", r.Extra = "Extra", r))(dt || {});
const Rr = (r) => {
...
function se(r) {
  ...
  for (let s = 0; s < r.length; s += 1) {
    const i = r[s];
    i.charCodeAt(0) < 32 ? (t || (e += " "), t = !0) : !t && i === Qe && s + 1 < r.length && ie(r[s + 1]) || (ie(i) ? (t || (e += i), t = !0) : ht(i) && n === i || (e += i, t = !1)), n = i;
  }
  return e;
}
function ae(r) {
...
function be(r, e, t, n) {
  ...
  if (s && i) {
    const c = se(r), d = se(t);
    if (c !== d) {
      if (!T(D(c, -1) ?? "") && !T(D(d, -1) ?? "") || !oe(r, e) || !oe(t, n)) return !1;
      let l = c;
      for (; T(D(l, -1) ?? ""); ) l = ee(l, 0, -1);
      let p = d;
      for (; T(D(p, -1) ?? ""); ) p = ee(p, 0, -1);
      if (l !== p) return !1;
    }
  } else if (!s && !i) {
    ...
    const p = ae(c.content), f = ae(d.content);
    if (p !== f) return !1;
    if (!p && !f) {
...
function Lr(r, e) {
  return be(r, void 0, e, void 0);
}
const Br = (r) => (...e) => r.map((n) => n(...e)).every((n) => n), Vr = (r) => async (...e) => {
  const t = r.map(async (n) => n(...e));
  return (await Promise.all(t)).every((n) => n);
}, ft = "book", ce = "chapter", w = "verse", P = "***";
...
function mt(r, e) {
  const t = e && e.length > 0 ? e : ve();
  return t[r % t.length];
}
function Hr(r, e) {
  const t = e && e.length > 0 ? e : ve(), n = (() => {
    const s = /* @__PURE__ */ new Map();
    let i = 0;
    return r.forEach((c, d) => {
      c.caller === "+" && (s.set(d, mt(i, t)), i += 1);
    }), s;
  })();
  return (s, i) => {
    if (s === "+") {
...
function yt(r, e) {
  ...
  return Object.keys(t).forEach((i) => {
    s && (Object.hasOwn(n, i) && yt(n[i], t[i]) || (s = !1));
  }), s;
}
function de(r, e, t) {
...
const Xr = (r) => r.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#x27;").replace(/\//g, "&#x2F;");
function Yr() {
  return typeof navigator < "u" && navigator.languages ? navigator.languages[0].replace(/@posix$/i, "") : new Oe().resolvedOptions().locale;
}
function Wr(r, e = 2) {
...
const bt = 1e3, Me = 60, xe = Me * 60, vt = xe * 24;
function Zr(r, e, t = /* @__PURE__ */ new Date()) {
  const n = Math.floor((e.getTime() - t.getTime()) / bt), s = Math.round(n / vt);
  if (Math.abs(s) >= 1) return r.format(s, "day");
  const i = Math.round(n / xe);
  if (Math.abs(i) >= 1) return r.format(i, "hour");
  const c = Math.round(n / Me);
...
const en = /* @__PURE__ */ new Set([
  ...
  return n >= e && n <= t;
}, tn = H.filter((r) => K(r, 1, 39)), rn = H.filter((r) => K(r, 40, 66)), nn = H.filter((r) => K(r, 67, 88)), G = {
...
function an(r, e, t) {
  ...
  return s.id = `${It}${t ? `-${t}` : ""}`, s.dataset.themeId = r.id, s.textContent = wt(r), this.document.head.appendChild(s), s;
}
function Ne(r) {
  return Object.freeze(r), r == null || Object.getOwnPropertyNames(r).forEach(function(t) {
    ...
    // eslint-disable-next-line no-null/no-null
    r[t] !== null && (typeof r[t] == "object" || typeof r[t] == "function") && !Object.isFrozen(r[t]) && Ne(r[t]);
  }), r;
}
const z = Ne({
  version: "3.0.7",
...
USJ: ${JSON.stringify(
  ...
  findAllNotes() {
    var e;
    return gt((e = this.usj) == null ? void 0 : e.content);
  }
  /**
  ...
  getEffectiveBookId(e) {
    const t = Object.keys(this.indicesInUsfmByVerseRef), n = t.length === 0 || t.length === 1 && t[0] === P, s = n ? P : e;
  ...
  getInfoForMarker(e, t = "usfm") {
    ...
    if (t === "usfm" && l === "cell" && g.colspan) {
      const u = parseInt(g.colspan, 10), h = Ct.exec(n);
      if (h != null && h[1]) {
  ...
  textContentToUsfm(e) {
    return {
      usfm: this.shouldAllowInvisibleCharacters ? e : e.replace(Pt, "~"),
...
`);
    ...
    return s.push({ fragment: e, indexInUsfm: n.length }), n += d === "optbreak" ? "//" : `\\${g}`, d !== "optbreak" && (s.push({
      ...
      s.push({
        ...
        if ("isClosingMarker" in M.fragment) {
          const { isClosingMarker: E, ...I } = M.fragment, B = {
            ...I,
            forMarker: e,
  ...
  closingMarkerToUsfm(e, t) {
    ...
    if ((f.closed === "false" || i.isClosingMarkerOptional && !this.markersMap.shouldOptionalClosingMarkersBePresent && f.closed !== "true") && (g = !1), i.independentClosingMarkers && i.independentClosingMarkers.length > 0 && g) {
      ...
      let y = "";
      const x = [], { usfm: N, fragmentsInfo: M } = this.openingMarkerToUsfm(m, t), E = M.find((I) => k.isFragmentAMarker(I.fragment));
      if (!E)
        throw new Error(
...
`) && (i.length === 0 ? (d = d.map((u) => ({
 ...
 `);
  ...
  static transferFragmentsInfoArrayToMaps(e, t, n, s, i, c) {
    e.map((l) => {
      ...
      if (typeof l.fragment == "object" && "type" in l.fragment) {
        const h = l.fragment;
        if (h.type === ft && h.code)
          n.bookId = h.code, n.chapterNum = 0, n.verseNum = 0, c[P] && (c[n.bookId] = c[P], delete c[P]);
        else if (h.type === ce && h.number) {
          const m = parseInt(h.number, 10);
          if (Number.isNaN(m))
            console.warn(
              `Found ${ce} type marker with number ${h.number}, but could not parse chapter number from it. Continuing using previous chapter number ${n.chapterNum}`
            );
          else {
          ...
          const m = (f = Tt.exec(h.number)) == null ? void 0 : f[1];
          if (!m)
            console.warn(
  ...
  calculateUsfmProperties() {
    let e = "";
    const t = new Z(), n = /* @__PURE__ */ new Map(), s = {}, i = [], c = {
      bookId: P,
      chapterNum: 0,
      verseNum: 0
    };
...
export {
  ...
  ir as DATA_LOSS,
  nn as DC_BOOK_IDS,
  sr as DEADLINE_EXCEEDED,
  Oe as DateTimeFormat,
  Le as DocumentCombiner,
  Xt as EventRollingTimeCounter,
  ar as FAILED_PRECONDITION,
  et as FIRST_SCR_BOOK_NUM,
  rt as FIRST_SCR_CHAPTER_NUM,
  nt as FIRST_SCR_VERSE_NUM,
  or as INTERNAL,
  cr as INVALID_ARGUMENT,
  ...
  dr as NOT_FOUND,
  rn as NT_BOOK_IDS,
  Wt as NonValidatingDocumentCombiner,
  Je as NumberFormat,
  tn as OT_BOOK_IDS,
  lr as OUT_OF_RANGE,
  pr as PERMISSION_DENIED,
  F as PLATFORM_ERROR_VERSION,
  Ue as PlatformEventEmitter,
  Zt as PromiseChainingMap,
  ur as RESOURCE_EXHAUSTED,
  pt as SELECTABLE_INVISIBLE_CHAR_OR_WHITESPACE_CLASS,
  dt as Section,
  Z as SortedNumberMap,
  Qt as SortedSet,
  It as THEME_STYLE_ELEMENT_ID,
  hr as UNAUTHENTICATED,
  fr as UNAVAILABLE,
  mr as UNIMPLEMENTED,
  gr as UNKNOWN,
  z as USFM_MARKERS_MAP_3_0,
  on as USFM_MARKERS_MAP_PARATEXT_3_0,
  er as UnsubscriberAsyncList,
  k as UsjReaderWriter,
  w as VERSE_TYPE,
  Vr as aggregateUnsubscriberAsyncs,
  Br as aggregateUnsubscribers,
  an as applyThemeStylesheet,
  Lr as areUsjContentsEqualExceptWhitespace,
  D as at,
  q as charAt,
  br as codePointAt,
  Cr as collapseMiddleWords,
  st as compareScrRefs,
  ...
  sn as expandThemeContribution,
  Wr as formatBytes,
  Qr as formatRelativeDate,
  vr as formatReplacementString,
  Ge as formatReplacementStringToArray,
  Fr as formatScrRef,
  Dr as formatScrRefRange,
  Zr as formatTimeSpan,
  zt as getAllObjectFunctionNames,
  it as getChaptersForBook,
  Yr as getCurrentLocale,
  ve as getDefaultCallerSequence,
  he as getErrorMessage,
  Hr as getFormatCallerFunction,
  at as getLocalizeKeyForScrollGroupId,
  Ur as getLocalizeKeysForScrollGroupIds,
  jr as getLocalizedIdFromBookNumber,
  mt as getNthCaller,
  Kr as getPaneSizeLimits,
  ...
  Xe as includes,
  U as indexOf,
  Kt as isErrorMessageAboutParatextBlockingInternetAccess,
  Gt as isErrorMessageAboutRegistryAuthFailure,
  Nr as isLocalizeKey,
  kr as isPlatformError,
  ht as isSelectableInvisibleCharOrWhiteSpace,
  Gr as isSerializable,
  v as isString,
  yt as isSubset,
  T as isWhiteSpace,
  Ye as lastIndexOf,
  Et as localizedStringsDocumentSchema,
  _t as menuDocumentSchema,
  Lt as newGuid,
  yr as newPlatformError,
  Mr as normalize,
  se as normalizeScriptureSpaces,
  qr as offsetBook,
  Ar as offsetChapter,
  $r as offsetVerse,
  ...
  Er as padEnd,
  _r as padStart,
  Mt as projectSettingsDocumentSchema,
  zr as sanitizeHtml,
  Or as scrRefToBBBCCC,
  re as scrRefToBBBCCCVVV,
  de as serialize,
  xt as settingsDocumentSchema,
  ee as slice,
  te as split,
  ge as startsWith,
  _ as stringLength,
