...
const Q = 1e4, O = Q / 1e3, ee = 3, z = /* @__PURE__ */ new Map(), xe = {
  unsubscribed: {
  ...
  threw: {
    ...
    suppressed: (n, e, t) => `UnsubscriberAsyncList ${n}: ${e} more unsubscribers added after the list was run threw in the last ${O}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${ee}): ${t.join(" | ")}`
  }
};
function H(n, e, t) {
  const { log: r, first: s, suppressed: i } = xe[e], c = `${n}\0${e}`, p = Date.now(), d = z.get(c);
  if (!d || p >= d.closesAt) {
    r(s(n, t)), z.set(c, {
      closesAt: p + Q,
      suppressedCount: 0,
  ...
  d.suppressedCount += 1, t !== void 0 && d.suppressedErrorSamples.size < ee && d.suppressedErrorSamples.add(`${t}`), d.summaryTimeout || (d.summaryTimeout = setTimeout(() => {
...
function St(n) {
  var e;
  return ((e = te[n]) == null ? void 0 : e.type) === a.Paragraph || n === "v";
}
function Pt(n) {
  const e = te[n];
  return (e == null ? void 0 : e.type) !== a.Character ? !1 : e.category !== o.DivisionMarks;
}
const G = 0.5, Y = 3, _e = 0.1;
function Ee(n) {
  return n < G ? G : n > Y ? Y : n;
}
function Ie(n) {
...
function Tt(n, e) {
  let t = n;
  return e > 0 ? t = Math.floor(n * 10 + X) / 10 : e < 0 && (t = Math.ceil(n * 10 - X) / 10), Ie(Ee(t + e * _e));
}
function Ct(n) {
...
class g {
  ...
  getBookIdForLogging() {
    ...
    const e = (t = this.usj.content) == null ? void 0 : t.find(
      (r) => typeof r != "string" && r.type === J && !!r.code
    );
...
`) && (i.length === 0 ? (p = p.map((u) => ({
 ...
 `);
  ...
  transferFragmentsInfoArrayToMaps(e, t, r, s, i, c) {
    e.map((d) => {
      ...
      if (typeof d.fragment == "object" && "type" in d.fragment) {
        ...
        else if (h.type === K && h.number) {
          const f = parseInt(h.number, 10);
          if (Number.isNaN(f))
            this.reportProblemOnce(
              `Found ${K} type marker with number ${h.number}, but could not parse chapter number from it. Continuing using previous chapter number ${r.chapterNum}`
            );
          else {
  ...
  calculateUsfmProperties() {
    let e = "";
    const t = new V(), r = /* @__PURE__ */ new Map(), s = {}, i = [], c = {
      bookId: S,
      chapterNum: 0,
      verseNum: 0
    }, p = (m) => {
...
export {
  ...
  Y as MAX_ZOOM_FACTOR,
  G as MIN_ZOOM_FACTOR,
  Vt as MODIFIER_KEYS,
  a as MarkerType,
  be as Mutex,
  rt as MutexMap,
  ...
  Nt as getCurrentLocale,
  re as getDefaultCallerSequence,
  Pr as getErrorMessage,
  At as getFormatCallerFunction,
