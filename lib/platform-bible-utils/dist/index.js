...
const G = 1e4, N = G / 1e3, Y = 3, V = /* @__PURE__ */ new Map(), ye = {
  unsubscribed: {
    ...
    suppressed: (i, e) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers arrived after the list was run in the last ${N}s and were unsubscribed immediately; per-occurrence warnings suppressed.`
  },
  failed: {
    log: (i) => console.error(i),
    first: (i) => `UnsubscriberAsyncList ${i}: Unsubscriber added after the list was run failed!`,
    suppressed: (i, e) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run failed in the last ${N}s; per-occurrence errors suppressed.`
  },
  threw: {
    log: (i) => console.error(i),
    ...
    suppressed: (i, e, t) => `UnsubscriberAsyncList ${i}: ${e} more unsubscribers added after the list was run threw in the last ${N}s; per-occurrence errors suppressed. Distinct errors sampled (up to ${Y}): ${t.join(" | ")}`
  }
};
function z(i, e, t) {
  const { log: r, first: s, suppressed: n } = ye[e], c = `${i}\0${e}`, p = Date.now(), d = V.get(c);
  if (!d || p >= d.closesAt) {
    r(s(i, t)), V.set(c, {
      closesAt: p + G,
      suppressedCount: 0,
...
const Ze = "ABORTED", Qe = "ALREADY_EXISTS", et = "CANCELLED", tt = "DATA_LOSS", rt = "DEADLINE_EXCEEDED", nt = "FAILED_PRECONDITION", it = "INTERNAL", st = "INVALID_ARGUMENT", at = "NOT_FOUND", ot = "OUT_OF_RANGE", ct = "PERMISSION_DENIED", dt = "RESOURCE_EXHAUSTED", pt = "UNAUTHENTICATED", lt = "UNAVAILABLE", ut = "UNIMPLEMENTED", ht = "UNKNOWN", A = 1;
function ft(i, e) {
  if (!i)
    return {
      message: "",
...
function Me(i, e) {
  ...
  return Object.keys(t).forEach((n) => {
    s && (Object.hasOwn(r, n) && Me(r[n], t[n]) || (s = !1));
  }), s;
}
function J(i, e, t) {
...
function _e(i) {
  return !!i.fullName && i.fullName !== i.shortName;
}
const Ee = " - ";
function Pt(i) {
  return _e(i) ? `${i.shortName}${Ee}${i.fullName}` : i.shortName;
}
function Tt(i, e) {
  return i.shortName.localeCompare(e.shortName, void 0, { sensitivity: "base" });
}
function Ct(i) {
  return typeof i == "string" && i.length > 0 ? i : void 0;
}
const Ie = 1e3, Z = 60, Q = Z * 60, we = Q * 24;
function qt(i, e, t = /* @__PURE__ */ new Date()) {
...
function Ot(i, e, t) {
  ...
  return s.id = `${Ae}${t ? `-${t}` : ""}`, s.dataset.themeId = i.id, s.textContent = je(i), this.document.head.appendChild(s), s;
}
function re(i) {
...
`) && (n.length === 0 ? (p = p.map((u) => ({
 ...
 `);
  ...
  calculateUsfmProperties() {
    let e = "";
    const t = new B(), r = /* @__PURE__ */ new Map(), s = {}, n = [], c = {
      bookId: S,
      chapterNum: 0,
      verseNum: 0
    };
