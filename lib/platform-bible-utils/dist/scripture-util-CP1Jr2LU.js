...
function Le(e) {
  return T(e).includes("Bug in Paratext caused attempted access to Internet. Request has been blocked.");
}
function Je(e) {
  return T(e).includes("Paratext.Data.VpnDisconnectedException");
}
function $e(e) {
  const t = "401 Unauthorized error while getting shared projects.", s = "User registration is not valid. Cannot retrieve resources from DBL.", r = T(e);
...
class u {
  ...
  *[Symbol.iterator]() {
   ...
   */
  indexOf(t, s) {
    const r = x(t), a = S(s ?? 0, this.graphemes.length);
   ...
   */
  lastIndexOf(t, s) {
    const r = x(t), { length: a } = this.graphemes, n = s === void 0 ? NaN : Number(s), o = Number.isNaN(n) ? a : S(n, a);
  ...
  split(t, s) {
    ...
    return r === 0 ? [] : t === void 0 ? [this] : se(t) ? this.splitOnRegExp(t, r) : this.splitOnString(x(t), r);
  }
  /**
...
const y = ["chapter", "book", "para", "row", "sidebar", X], ce = "​", k = " ", F = [
...
], ie = 1, le = F.length - 1, ue = 1, me = 1, lt = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, fe = (e) => {
...
function xe(e) {
  ...
  for (let a = 0; a < e.length; a += 1) {
    const n = e[a];
    n.charCodeAt(0) < 32 ? (s || (t += " "), s = !0) : !s && n === ce && a + 1 < e.length && R(e[a + 1]) || (R(n) ? (s || (t += n), s = !0) : be(n) && r === n || (t += n, s = !1)), r = n;
  }
  return t;
}
function L(e) {
...
function D(e) {
  return e.split(k).map(xe).join(k);
}
function z(e, t, s, r) {
  if (!e && !s) return !0;
  if (!e || !s) return !1;
  ...
  if (a && n) {
    ...
    if (o !== h) {
      ...
      if (!C(c.at(-1) ?? "") && !C(f().at(-1) ?? "") || !$(e, t) || !$(s, r) || J(c) !== J(f())) return !1;
    }
  } else if (!a && !n) {
    ...
    if (!m && !f) {
      ...
      y.includes(o.type) && g(d) && C(d) && (i = i.slice(0, -1));
      const A = p[p.length - 1];
      if (y.includes(h.type) && g(A) && C(A) && (p = p.slice(0, -1)), i.length !== p.length) return !1;
...
//# sourceMappingURL=scripture-util-CP1Jr2LU.js.map
