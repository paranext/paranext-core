import { Canon as i, VerseRef as D } from "@sillsdev/scripture";
...
function re(e, t) {
  ...
  let r = 0, s = 0;
  function o(l, h, f) {
    const N = d(e, s, h), c = a.length > 0 && E(a[a.length - 1]) ? `${a.pop()}${N}` : N;
...
const qe = (e, t) => ({
  book: i.bookNumberToId(
    Math.max(
      ce,
      Math.min(i.bookIdToNumber(e.book) + t, he)
    )
  ),
  chapterNum: 1,
  verseNum: 1
...
function tt(e) {
  return new D(i.bookIdToNumber(e.book), e.chapterNum, e.verseNum).BBBCCC;
}
function v(e) {
  return new D(i.bookIdToNumber(e.book), e.chapterNum, e.verseNum).BBBCCCVVV;
}
function ie(e, t) {
  return v(e) - v(t);
}
...
function V(e, t, a, r) {
  ...
  if (s && o) {
    ...
    if (n !== l) {
      ...
      let h = n;
      for (; g(B(h, -1) ?? ""); ) h = M(h, 0, -1);
      let f = l;
      for (; g(B(f, -1) ?? ""); ) f = M(f, 0, -1);
      if (h !== f) return !1;
    }
  } else if (!s && !o) {
    ...
    const f = R(n.content), N = R(l.content);
    if (f !== N) return !1;
    if (!f && !N) {
...
//# sourceMappingURL=scripture-util-Du06ZpjG.js.map
