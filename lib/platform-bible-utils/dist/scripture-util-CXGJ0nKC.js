...
import { indexOf as H, limit as $, length as V, substring as K, toArray as X, substr as j } from "stringz";
function Te() {
...
function ye(e, t = 300) {
  let a, r, s, o;
  const l = (...n) => (clearTimeout(a), r || (r = new Promise((c, f) => {
    s = c, o = f;
  })), a = setTimeout(async () => {
    try {
      s(await e(...n));
...
function te(e, t) {
  ...
  let r = 0, s = 0;
  function o(n, c, f) {
    const S = p(e, s, c), h = a.length > 0 && i(a[a.length - 1]) ? `${a.pop()}${S}` : S;
...
const T = ["chapter", "book", "para", "row", "sidebar", z], le = "​", G = [
...
], ue = 1, ce = G.length - 1, he = 1, me = 1, Ve = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, fe = (e) => {
...
function L(e) {
  ...
  for (let s = 0; s < e.length; s += 1) {
    const o = e[s];
    o.charCodeAt(0) < 32 ? (a || (t += " "), a = !0) : !a && o === le && s + 1 < e.length && I(e[s + 1]) || (I(o) ? (a || (t += o), a = !0) : Ce(o) && r === o || (t += o, a = !1)), r = o;
  }
  return t;
}
function R(e) {
...
function W(e, t, a, r) {
  ...
  if (s && o) {
    const l = L(e), n = L(a);
    if (l !== n) {
      if (!d(A(l, -1) ?? "") && !d(A(n, -1) ?? "") || !v(e, t) || !v(a, r)) return !1;
      let c = l;
      for (; d(A(c, -1) ?? ""); ) c = M(c, 0, -1);
      let f = n;
      for (; d(A(f, -1) ?? ""); ) f = M(f, 0, -1);
      if (c !== f) return !1;
    }
  } else if (!s && !o) {
    ...
    const f = R(l.content), S = R(n.content);
    if (f !== S) return !1;
    if (!f && !S) {
      let h = l.content, E = n.content;
      const O = h[h.length - 1];
      T.includes(l.type) && i(O) && d(O) && (h = h.slice(0, -1));
      const y = E[E.length - 1];
      if (T.includes(n.type) && i(y) && d(y) && (E = E.slice(0, -1)), h.length !== E.length) return !1;
...
function nt(e) {
  const t = [], a = /* @__PURE__ */ new Set(), r = (s) => {
    s && s.forEach((o) => {
      if (i(o)) return;
      const { marker: l } = o;
      l && !l.startsWith("z") && !a.has(l) && (a.add(l), t.push(l)), r(o.content);
    });
  };
...
//# sourceMappingURL=scripture-util-CXGJ0nKC.js.map
