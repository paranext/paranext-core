...
function Ge(e, t, r) {
  ...
  return e.forEach((a, n) => {
    const o = t(a, n), i = s.get(o), h = r ? r(a, o, n) : a;
    i ? i.push(h) : s.set(o, [h]);
  }), s;
}
function ae(e) {
...
function oe(e, t, r) {
  let s = e.length, a = 0, n = 0, o = -1, i = () => {
    let h = 0, c = 0, u;
    do
      u = parseInt(e[a++], 36), h += (u & 15) << c, c += 4;
    while (u & 16);
    return h;
  };
  for (; a < s; ) {
    let h = o + 1 + i();
    r(h, o = h + i(), parseInt(t[n++], 36));
  }
}
function ie(e, t, r) {
...
let H, q;
{
  let e = (a, n, o, i, h) => {
    ...
    c > 0 && o - n < u && a.fill(h, o > n ? o - n : 0, c > u ? u : c);
...
function ce(e) {
  return e < 57344 ? e <= 55238 ? e >= 55216 ? 13 : 0 : e >= 55243 && e <= 55291 ? 12 : 0 : e < 65024 ? e === 64286 ? 3 : 0 : e >= 917504 ? e > 921599 ? 0 : e >= 917536 && e < 917632 || e >= 917760 && e < 918e3 ? 3 : 2 : ie(e, H, q);
}
...
function* k(e) {
  ...
  for (; s < t; ) {
    r = /** @type {number} */
    e.codePointAt(s);
    let i = O(r), h = !(n & fe[a << 4 | i]);
    n = w(n, i, r), h && (yield e.slice(o, s), o = s), s += r > 65535 ? 2 : 1, a = i;
  }
  yield e.slice(o);
}
...
class m {
  ...
  *[Symbol.iterator]() {
   ...
   */
  indexOf(t, r) {
    const s = S(t), a = x(r ?? 0, this.graphemes.length);
   ...
   */
  lastIndexOf(t, r) {
    const s = S(t), { length: a } = this.graphemes, n = r === void 0 ? NaN : Number(r), o = Number.isNaN(n) ? a : x(n, a);
  ...
  split(t, r) {
    ...
    return s === 0 ? [] : t === void 0 ? [this] : de(t) ? this.splitOnRegExp(t, s) : this.splitOnString(S(t), s);
  }
  /**
  ...
  splitOnRegExp(t, r) {
    ...
    const o = [];
    let i = 0, h = 0;
    for (; h < this.str.length; ) {
      n.lastIndex = h;
      const c = n.exec(this.str);
      if (!c) break;
      const u = c.index + c[0].length, l = this.graphemeIndexAtOffset(c.index), b = h + 1;
      if (l < 0 || !this.isBoundary(u))
        h = Math.max(c.index + 1, b);
      else {
        const g = u >= this.str.length ? s : this.graphemeIndexAtOffset(u);
        if (g === i) {
          if (l + 1 >= s) break;
          h = Math.max(this.offsets()[l + 1], b);
        } else {
          if (o.push(this.derive(i, l)), o.length === r) return o;
  ...
  buildPadding(t, r) {
    ...
    for (let h = 0; h < o; h++) i[h] = n[h % n.length];
    return i;
  }
...
function Ee(e, t) {
  ...
  let r = t;
  const s = e.length;
  for (; r < s && (r = e.indexOf("}", r), !(r === -1 || e.charAt(r - 1) !== "\\")); )
    r += 1;
  return r >= s ? -1 : r;
}
function xe(e, t) {
  ...
  let s = 0, a = 0;
  function n(i, h, c) {
    const u = e.slice(a, h).toString(), l = r.length > 0 && p(r[r.length - 1]) ? `${r.pop()}${u}` : u;
    p(i) ? r.push(`${l}${i}`) : (l && r.push(l), r.push(i)), a = h + c;
  }
  const o = e.length;
  for (; s < o; ) {
    ...
    switch (e.charAt(s)) {
      case "{":
        if (i !== "\\") {
          const h = Ee(e, s);
          if (h >= 0) {
            const c = e.slice(s + 1, h).toString(), u = Object.hasOwn(t, c) ? (
...
const B = ["chapter", "book", "para", "row", "sidebar", re], Se = "​", M = " ", Z = [
...
], Ce = 1, ye = Z.length - 1, Be = 1, Oe = 1, xt = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, we = (e) => {
...
function Fe(e) {
  ...
  for (let a = 0; a < e.length; a += 1) {
    const n = e[a];
    n.charCodeAt(0) < 32 ? (r || (t += " "), r = !0) : !r && n === Se && a + 1 < e.length && L(e[a + 1]) || (L(n) ? (r || (t += n), r = !0) : Le(n) && s === n || (t += n, r = !1)), s = n;
  }
  return t;
}
function F(e) {
...
function Q(e, t, r, s) {
  ...
  if (a && n) {
    ...
    if (o !== i) {
      ...
      if (!C(h.at(-1) ?? "") && !C(u().at(-1) ?? "") || !U(e, t) || !U(r, s) || D(h) !== D(u())) return !1;
    }
  } else if (!a && !n) {
    const o = e, i = r, h = Object.keys(o).filter(
      (l) => l !== "content"
    );
    if (h.length !== Object.keys(i).filter((l) => l !== "content").length || h.some((l) => !(l in i) || o[l] !== i[l])) return !1;
    const c = F(o.content), u = F(i.content);
    if (c !== u) return !1;
    if (!c && !u) {
      let l = o.content, b = i.content;
      const g = l[l.length - 1];
      B.includes(o.type) && p(g) && C(g) && (l = l.slice(0, -1));
      const E = b[b.length - 1];
      if (B.includes(i.type) && p(E) && C(E) && (b = b.slice(0, -1)), l.length !== b.length) return !1;
...
//# sourceMappingURL=scripture-util-B5humGWR.js.map
