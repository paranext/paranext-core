...
var x = (e, t, s) => V(e, typeof t != "symbol" ? t + "" : t, s);
import { Canon as N, VerseRef as _ } from "@sillsdev/scripture";
import { USJ_TYPE as X } from "@eten-tech-foundation/scripture-utilities";
import { splitGraphemes as T } from "unicode-segmenter/grapheme";
function Be() {
...
function we(e, t = 300) {
  ...
  const h = () => (r || (r = new Promise((f, i) => {
    ...
    clearTimeout(s);
    const i = h();
    return o = f, s = setTimeout(() => {
      const p = o;
      o = void 0, p && c(p);
    }, t), i;
  };
  return m.cancel = () => {
    clearTimeout(s), o = void 0, r && (n(new Error(K)), r = void 0);
  }, m.flush = () => {
    if (o === void 0) return;
    clearTimeout(s);
    const f = o;
...
function Re(e) {
  ...
  return g(e) ? e.includes(t) : B(e).includes(t);
}
function Le(e) {
  return (g(e) ? e : B(e)).includes("VpnDisconnectedException");
}
function Je(e) {
  const t = "401 Unauthorized error while getting shared projects.", s = "User registration is not valid. Cannot retrieve resources from DBL.", r = g(e) ? e : B(e);
  return r.includes(t) || r.includes(s);
}
const Y = 2 ** 32 - 1, w = 2 ** 20;
function b(e) {
  const t = Number(e);
...
function I(e, t) {
  const s = b(e);
  return s < 0 ? Math.max(t + s, 0) : Math.min(s, t);
}
function S(e, t) {
  const s = b(e);
...
class u {
  ...
  constructor(t) {
     ...
     */
    x(this, "graphemes");
    this.str = t, this.graphemes = t === "" ? [] : Array.from(T(t));
  }
  /**
  ...
  *[Symbol.iterator]() {
   ...
   */
  formatReplacement(t) {
    return this.formatReplacementToArray(t).map(ee).join("");
  }
  /**
   ...
   */
  slice(t, s) {
    const { length: r } = this.graphemes, a = t === void 0 ? 0 : I(t, r), n = s === void 0 ? r : I(s, r);
  ...
  split(t, s) {
    ...
    return r === 0 ? [] : t === void 0 ? [this] : te(t) ? this.splitOnRegExp(t, r) : this.splitOnString(C(t), r);
  }
  /**
  ...
  splitOnRegExp(t, s) {
    ...
    for (; c < this.str.length; ) {
      n.lastIndex = c;
      const m = n.exec(this.str);
      if (!m) break;
      const f = m.index + m[0].length, i = this.graphemeIndexAtOffset(m.index), p = c + 1;
      if (i < 0 || !this.isBoundary(f))
        c = Math.max(m.index + 1, p);
      else {
        const d = f >= this.str.length ? r : this.graphemeIndexAtOffset(f);
        if (d === h) {
          if (i + 1 >= r) break;
          c = Math.max(this.offsets()[i + 1], p);
        } else {
          if (o.push(this.derive(h, i)), o.length === s) return o;
          for (let A = 1; A < m.length; A++) {
            const E = m[A];
            if (o.push(E === void 0 ? void 0 : new u(E)), o.length === s) return o;
          }
          h = d, c = Math.max(f, p);
        }
      }
  ...
  buildPadding(t, s) {
    ...
    const a = s === void 0 ? " " : `${s}`, n = Array.from(T(a));
  ...
  derive(t, s) {
    return t >= s ? new u("") : u.fromSegmented(
...
function se(e, t) {
  ...
  let s = t;
  const r = e.length;
  for (; s < r && (s = e.indexOf("}", s), !(s === -1 || e.charAt(s - 1) !== "\\")); )
    s += 1;
  return s >= r ? -1 : s;
}
function re(e, t) {
  ...
  let r = 0, a = 0;
  function n(h, c, m) {
    const f = e.slice(a, c).toString(), i = s.length > 0 && g(s[s.length - 1]) ? `${s.pop()}${f}` : f;
    g(h) ? s.push(`${i}${h}`) : (i && s.push(i), s.push(h)), a = c + m;
  }
  const o = e.length;
  for (; r < o; ) {
    ...
    switch (e.charAt(r)) {
      case "{":
        if (h !== "\\") {
          const c = se(e, r);
          if (c >= 0) {
            const m = e.slice(r + 1, c).toString(), f = Object.hasOwn(t, m) ? (
...
function Ve(e, t, s) {
  return new u(e).padEnd(t, s);
}
function Xe(e, t, s) {
  return new u(e).padStart(t, s);
}
function Ke(e, t, s) {
...
const y = ["chapter", "book", "para", "row", "sidebar", X], he = "​", M = " ", F = [
...
], ce = 1, ie = F.length - 1, le = 1, ue = 1, it = {
  book: "GEN",
  chapterNum: 1,
  verseNum: 1
}, me = (e) => {
...
const ft = (e, t) => ({
  book: N.bookNumberToId(
    Math.max(
      ce,
      Math.min(N.bookIdToNumber(e.book) + t, ie)
    )
  ),
  chapterNum: 1,
  verseNum: 1
  ...
  ...e,
  chapterNum: Math.min(
    Math.max(le, e.chapterNum + t),
    me(N.bookIdToNumber(e.book))
  ),
  verseNum: 1
}), pt = (e, t) => ({
  ...e,
  verseNum: Math.max(ue, e.verseNum + t)
});
async function gt(e, t, s) {
  const r = N.bookNumberToId(e);
  if (!Intl.getCanonicalLocales(t)[0].startsWith("zh"))
    return s({
  ...
  return new u(a).split("-")[0].split("（")[0].toString().trim();
}
function dt(e) {
  return new _(N.bookIdToNumber(e.book), e.chapterNum, e.verseNum).BBBCCC;
}
function v(e) {
  return new _(N.bookIdToNumber(e.book), e.chapterNum, e.verseNum).BBBCCCVVV;
}
function fe(e, t) {
  return v(e) - v(t);
}
...
function Ne(e, t) {
  const s = j(e, t == null ? void 0 : t.optionOrLocalizedBookName), r = (t == null ? void 0 : t.bookChapterSeparator) ?? " ", a = (t == null ? void 0 : t.chapterVerseSeparator) ?? ":";
...
function P(e, t) {
  const s = j(e, t == null ? void 0 : t.optionOrLocalizedBookName), r = pe(
    e,
...
function D(e) {
  return e.split(M).map(be).join(M);
}
function z(e, t, s, r) {
  if (!e && !s) return !0;
  if (!e || !s) return !1;
  ...
  if (a && n) {
    ...
    if (o !== h) {
      const c = new u(o);
      let m;
      const f = () => (m ?? (m = new u(h)), m);
      if (!O(c.at(-1) ?? "") && !O(f().at(-1) ?? "") || !$(e, t) || !$(s, r) || J(c) !== J(f())) return !1;
    }
  } else if (!a && !n) {
    const o = e, h = s, c = Object.keys(o).filter(
      (i) => i !== "content"
    );
    if (c.length !== Object.keys(h).filter((i) => i !== "content").length || c.some((i) => !(i in h) || o[i] !== h[i])) return !1;
    const m = L(o.content), f = L(h.content);
    if (m !== f) return !1;
    if (!m && !f) {
      let i = o.content, p = h.content;
      const d = i[i.length - 1];
      y.includes(o.type) && g(d) && O(d) && (i = i.slice(0, -1));
      const A = p[p.length - 1];
      if (y.includes(h.type) && g(A) && O(A) && (p = p.slice(0, -1)), i.length !== p.length) return !1;
...
function xt(e) {
  const t = [], s = /* @__PURE__ */ new Set(), r = (a) => {
    a && a.forEach((n) => {
      if (g(n)) return;
      const { marker: o } = n;
      o && !o.startsWith("z") && !s.has(o) && (s.add(o), t.push(o)), r(n.content);
    });
  };
...
//# sourceMappingURL=scripture-util-CCD8aGNl.js.map
