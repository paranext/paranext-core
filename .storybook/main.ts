...
const config: StorybookConfig = {
  ...
  webpackFinal: async (webpackConfig, { configType }) => {
    ...
    webpackConfig.plugins.push(
      ...
      // `spyOn(commandService, 'sendCommand')` cannot work in Storybook 9 — webpack builds the
      // module namespace with non-configurable getters and tags it `Symbol.toStringTag = 'Module'`,
      // which is exactly what makes spying throw "Module namespace is not configurable in ESM".
      // Stories that need to answer a PAPI command set a responder via
      // `mocks/command-service-mock-channel.ts` instead, which this replacement reads at call time.
      // The replacement is global, so its default must match the unanswerable-request path every
      // other story already takes: it rejects.
      new NormalModuleReplacementPlugin(
