...
- **No `as` keyword** - Use type guards or proper typing instead of type assertions
- **No `null`** - Use `undefined` instead (exception: DOM APIs that require null)
- **No `for` loops in React** - Use `map`, `filter`, `reduce` for iteration
...
- **Use path aliases** for imports (e.g., `@shared/...`) instead of relative paths
