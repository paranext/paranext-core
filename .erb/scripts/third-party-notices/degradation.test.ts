 ...
 * impossible.
 *
 * The assertion each case makes is therefore a NON-ZERO EXIT of the real `main.js`, not merely that
...
/**
 * Runs the real `main.js` in a child process, with `setup` evaluated first.
 *
 * `setup` runs with three helpers in scope:
 *
 * - `load(relative)` - require a module of this pipeline for real.
 * - `stub(relative, exports)` - seed `require.cache` so `main.js`'s own `require` of that module
 *   returns `exports`. Seeding the cache rather than rewriting `main.js` is what keeps `main.js`
...
// of these cases is about licensee. Mutating the shared child_process module before main.js is
...
process.argv = [process.argv[0], path.join(DIR, 'main.js'), ...${JSON.stringify(argv)}];
load('main.js').main();
`;
  const result = spawnSync(process.execPath, ['-e', script], { encoding: 'utf8', cwd: REPO });
...
describe('a degraded environment fails closed', () => {
    ...
    const run = runGenerator(
      ...
      stub('./lock', {
        ...
        readLock: () => ({
          ...
          corpusVersion: load('corpus.js').corpusVersion(),
          packages: [
            {
              ecosystem: 'npm',
              name: 'relicensed',
  ...
  it('exits non-zero when the npm shipping set is implausibly small', () => {
    ...
    const run = runGenerator(
      ...
      // not answer for is a package nothing is known about, and main.js refuses that run rather
  ...
  it('leaves the artifact untouched when writing the lock fails', () => {
    ...
    const run = runGenerator(
      ...
      stub('./render', { ...load('render.js'), render: () => 'CLOBBERED\\n' });
      stub('./lock', {
        ...load('lock.js'),
        writeLock: () => {
  ...
  it('exits non-zero rather than writing the artifact on a non-Linux platform', () => {
    ...
    const result = spawnSync(process.execPath, [path.join(DIR, 'main.js')], {
      encoding: 'utf8',
      cwd: REPO,
...
describe('NOTICES_FORCE_PLATFORM can only turn the write refusal ON', () => {
  ...
  function runOnFakePlatform(actual: string, forced: string | undefined): Run {
    const script = `
      ...
      const path = require('path');
      process.argv = [process.argv[0], path.join(${JSON.stringify(DIR)}, 'main.js')];
      require(path.join(${JSON.stringify(DIR)}, 'main.js')).main();
    `;
