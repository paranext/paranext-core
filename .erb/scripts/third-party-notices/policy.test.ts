...
describe('classify', () => {
  ...
  it('blocks a text-derived id that is neither on the allow list nor the copyleft list', () => {
    ...
    // `policy.js`. Testing `copyleft.has(...)` alone makes it a denylist, and an npm package with
  ...
  it('CASE 1: resolves a dual-licensed package whose detected ids match its declared disjunction', () => {
  ...
  it('CASE 2: resolves on the declared id when a bundled extra is allow-listed', () => {
  ...
  it('CASE 2 hazard: blocks when a bundled extra is copyleft, even though the primary id is permissive', () => {
    // The finding that started this round: an undisclosed copyleft obligation must not hide behind
    // a permissive primary license just because reconciliation no longer blocks on every
    // disagreement. This must not regress.
    const v = classify({
      ...base,
      declaredField: 'MIT',
      detection: detectedFiles([
  ...
  it('CASE 2 hazard: the copyleft check is independent of the allow-list check', () => {
  ...
  it('CASE 0: blocks a lone license file that identifies as none of a declared disjunction', () => {
  ...
  it('CASE 0: two files agreeing on ONE id are reconciled the same way as one file', () => {
  ...
  it('CASE 0: a lone file that IS one of the declared operands still elects', () => {
  ...
  it('CASE 1 vs CASE 3: a disjunction with only a PARTIAL detected match still blocks', () => {
  ...
  it('CASE 2 hazard: blocks when a bundled extra is neither allowed nor copyleft', () => {
  ...
  it('CASE 3: blocks when no detected id matches the declaration at all', () => {
  ...
  it('CASE 3: blocks when multiple usable license files disagree and nothing is declared', () => {
  ...
  it('does not apply an exception with no textSha256 recorded (unpinned)', () => {
    ...
    // hash, zero verification. An explicit `textSha256: null` already failed the old check
    // correctly; the hole is specifically the omitted key, the shape most likely hand-authored by
    // mistake.
    const policy = {
      ...POLICY,
      exceptions: [
  ...
  it('blocks an npm disjunction whose recorded election names a branch not in the declared expression', () => {
    // Kills a mutant deleting the npm election branch's "elected not in declared" check
    // specifically: BSD-3-Clause is on the allow list, so if this check were removed the wrongly
    // elected id would still pass the allow-list check and the verdict would flip to `elected`.
    const policy = {
      ...POLICY,
      elections: {
        ...POLICY.elections,
        'npm:stale-npm-election': {
          elected: 'BSD-3-Clause',
          of: 'MIT OR Apache-2.0',
          reason: 'test',
        },
  ...
  it('blocks an npm disjunction whose recorded election names an id absent from the allow list', () => {
    // Kills a mutant deleting the npm election branch's allow-list check specifically: BSD-2-Clause
    // is present in the declared expression (so the "not one of" check alone cannot catch it) but
    // absent from this test policy's allow list.
    const policy = {
      ...POLICY,
      elections: {
        ...POLICY.elections,
        'npm:unlisted-election': {
          elected: 'BSD-2-Clause',
          of: 'MIT OR BSD-2-Clause',
          reason: 'test',
        },
  ...
  it('CASE 3: an npm package that ships no license file resolves on its declaration', () => {
    ...
    // Under the previous rule they blocked, and NO instrument could clear them - an exception is
    // refused without a text hash, and an override applies only where nothing parseable is declared.
  ...
  it('CASE 2: an npm package whose license file cannot be identified still blocks', () => {
  ...
  it('CASE 1 is unweakened: a declaration contradicted by the license file still blocks', () => {
  ...
  it('CASE 3 still blocks copyleft, with no file to soften it', () => {
    const v = classify({
      ...base,
      declaredField: 'GPL-3.0-or-later',
      detection: { dir: '/x', files: [] },
  ...
  it('CASE 3 still blocks an unlisted identifier', () => {
  ...
  it('CASE 3 still blocks a conjunction, which no election can resolve', () => {
  ...
  it('CASE 3 blocks a declaration that is not an SPDX expression at all', () => {
...
describe('notices-policy.json', () => {
  ...
  it('keeps a copyright notice for each yalc dev-linked package', () => {
    expect(Object.keys(policy.copyrightNotices ?? {}).sort()).toEqual([
  ...
  it('no reviewed exception admits a copyleft license', () => {
    // `applyException` now enforces this itself, against BOTH lists, at apply time (ADR-0027) - the
    // earlier note here argued the check should stay narrower than the allow list, on the grounds
    // that an exception exists for cases the lists cannot express. That reasoning did not survive
    // the observation that a data-level test over the COMMITTED policy cannot cover an entry added
    // in the same pull request as the policy change that would accompany it, so the bound has to
    // live in the mechanism. This check is kept as the cheaper, earlier signal over the shipped
    // file, and because a copyleft exception is the specific failure worth naming on its own.
...
describe('curated overrides', () => {
  ...
  const OVERRIDE_POLICY = {
    ...POLICY,
    overrides: {
      'nuget:ParatextData': { license: 'Proprietary - SIL Global / United Bible Societies' },
  ...
  it('resolves a package whose nuspec declares nothing at all', () => {
    ...
    expect(v.spdxId).toBe('Unicode-3.0');
  });
