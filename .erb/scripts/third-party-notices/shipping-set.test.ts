...
describe('collectShippedPackages', () => {
  ...
  // The `Array.isArray(modules)` guard covers the REQUIRED bundles only, while the loop that reads
  // the manifests iterates every `*.json` in the directory. `.notices/` is gitignored, and only
  // `npm run package` clears it (see `clean.ts`) - an ordinary build does not - so a bundle renamed
  // in a later commit leaves its old manifest behind indefinitely.
  it('names the file when a manifest in the directory has no module list', () => {
    writeManifest('main', [
      path.join(writePackage('node_modules/kept', 'kept', '1.0.0'), 'index.js'),
    ]);
    const stray = path.join(repo, '.notices', 'modules', 'renamed-bundle.json');
    fs.writeFileSync(stray, JSON.stringify({ bundle: 'renamed-bundle', buildId: 'one-build' }));
...
describe('collectShippedPackages - prebuilt lib scan', () => {
  // A `lib/*` package is consumed as its own built bundle (`platform-bible-react` resolves to
  // `dist/index.js`), and vite externalizes only peerDependencies and dependencies - so anything it
  // imports from `devDependencies` is INLINED into that one file. Webpack then sees one module and
  // the package's own dependencies are gone as separate modules, exactly the blind spot the
  // stylesheet scan compensates for on the CSS side and nothing compensated for here.
  function writeLibPackage(name: string, license: string, source: string) {
    const dir = path.join(repo, 'lib', name);
    fs.mkdirSync(path.join(dir, 'src'), { recursive: true });
    fs.mkdirSync(path.join(dir, 'dist'), { recursive: true });
    fs.writeFileSync(
      path.join(dir, 'package.json'),
      JSON.stringify({ name, version: '1.0.0', license, main: 'dist/index.cjs' }),
    );
    fs.writeFileSync(path.join(dir, 'src', 'index.ts'), source);
    fs.writeFileSync(path.join(dir, 'dist', 'index.cjs'), '');
    return dir;
  }

  /** The module path webpack reports for a `lib/*` package's prebuilt bundle. */
  const libBundleModule = (name: string) => path.join(repo, 'lib', name, 'dist', 'index.cjs');

  it('finds a package a shipped lib package inlines into its own bundle', () => {
    const inlined = writePackage('node_modules/inlined-dep', 'inlined-dep', '3.0.0');
    writeLibPackage('shipping-lib', 'MIT', "import x from 'inlined-dep';\nexport default x;\n");
    writeManifest('main', [libBundleModule('shipping-lib')]);

    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(packages.find((p) => p.name === 'inlined-dep')).toMatchObject({
      version: '3.0.0',
      dir: inlined,
      reachedVia: ['prebuilt-lib'],
    });
  });

  /** The packages the scan reports for one `lib/*` source file. */
  function scannedFrom(source: string): string[] {
    writeLibPackage('shipping-lib', 'MIT', source);
    writeManifest('main', [libBundleModule('shipping-lib')]);
    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
  ...
  it('sees a SIDE-EFFECT import that precedes a named one', () => {
    ...
    expect(
      scannedFrom("import 'side-effect-dep';\nimport x from 'named-dep';\nexport default x;\n"),
    ).toEqual(['named-dep', 'side-effect-dep']);
  });

  it('does not read an import out of a COMMENT', () => {
    // Live on this repository: the pattern's start anchor matched inside a word, so
    // `// Re-exported so consumers can type the argument…` began a match on the `export` in
    // "Re-exported" and swallowed the `export type { … } from '…'` two lines below it - recording a
    // type-only re-export of `@eten-tech-foundation/scripture-utilities` as a runtime dependency of
    // `platform-bible-utils`. Nine matches in the scanned sources started this way.
    writePackage('node_modules/build-time-only', 'build-time-only', '1.0.0');

    expect(
      scannedFrom(
        "// Re-exported so consumers can type it\nexport type { A } from 'build-time-only';\n",
      ),
    ).toEqual([]);
  });

  it('does not read an import out of a STRING LITERAL', () => {
    writePackage('node_modules/not-a-dependency', 'not-a-dependency', '1.0.0');
    writePackage('node_modules/real-dependency', 'real-dependency', '1.0.0');

    expect(
      scannedFrom(
        "const example = \"import x from 'not-a-dependency'\";\nimport a from 'real-dependency';\nexport default [example, a];\n",
      ),
    ).toEqual(['real-dependency']);
  });

  it('sees a dynamic import() and a require()', () => {
    writePackage('node_modules/dynamic-dep', 'dynamic-dep', '1.0.0');
    writePackage('node_modules/required-dep', 'required-dep', '1.0.0');

    expect(
      scannedFrom(
        "const a = require('required-dep');\nexport const load = () => import('dynamic-dep');\nexport default a;\n",
      ),
    ).toEqual(['dynamic-dep', 'required-dep']);
  });

  it('keeps the value half of a mixed named import, and skips a wholly type-only one', () => {
    // `import { type A, b }` still brings `b` in at runtime; `import type { … }` brings nothing.
    writePackage('node_modules/mixed-dep', 'mixed-dep', '1.0.0');
    writePackage('node_modules/types-only-dep', 'types-only-dep', '1.0.0');

    expect(
      scannedFrom(
        "import { type A, b } from 'mixed-dep';\nimport type { C } from 'types-only-dep';\nexport default b;\n",
      ),
    ).toEqual(['mixed-dep']);
  });

  it('ignores a lib package no bundle compiles, whose code does not reach a shipped artifact', () => {
    // `lib/eslint-plugin-paranext` and `lib/browserslist-config-detect-electron` are build-time
    // tooling and appear in no module manifest. Scanning them would put eslint and Storybook into a
    // document about what ships.
    writePackage('node_modules/build-only-dep', 'build-only-dep', '1.0.0');
    writeLibPackage('tooling-lib', 'MIT', "import x from 'build-only-dep';\n");
    writeManifest('main', []);
  ...
  it('scans a shipped lib package whatever licence it declares', () => {
    ...
    // document.
    const inlined = writePackage('node_modules/relicensed-dep', 'relicensed-dep', '2.0.0');
    writeLibPackage(
      'relicensed-lib',
      'AGPL-3.0-or-later',
      "import x from 'relicensed-dep';\nexport default x;\n",
    );
    writeManifest('main', [libBundleModule('relicensed-lib')]);

    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(packages.find((p) => p.name === 'relicensed-dep')).toMatchObject({ dir: inlined });
  });

  it('finds a side-effect import, which names its package nowhere else', () => {
    // `import 'some-polyfill'` has no `from` clause, so a pattern requiring one matched it nowhere
    // - and a runtime-only import is the ordinary way a polyfill or a stylesheet enters a bundle.
    const polyfill = writePackage('node_modules/some-polyfill', 'some-polyfill', '1.2.3');
    writeLibPackage('polyfilled-lib', 'MIT', "import 'some-polyfill';\nexport const a = 1;\n");
    writeManifest('main', [libBundleModule('polyfilled-lib')]);

    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(packages.find((p) => p.name === 'some-polyfill')).toMatchObject({ dir: polyfill });
  });
  ...
  it('finds a specifier whose import clause is longer than a couple of lines', () => {
    ...
    // before the `from` clause and the case would pass for a reason that has nothing to do with the
    // window.
    const names = Array.from({ length: 40 }, (unused, index) => `alphaValue${index}`).join(',\n  ');
    writeLibPackage(
      'wide-lib',
      'MIT',
      `import {\n  ${names},\n} from 'wide-import-dep';\nconst first = alphaValue0;\n`,
    );
    writeManifest('main', [libBundleModule('wide-lib')]);

    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(packages.find((p) => p.name === 'wide-import-dep')).toMatchObject({ dir: wide });
  });

  it('ignores a type-only import, which the compiler erases', () => {
    writePackage('node_modules/types-only-dep', 'types-only-dep', '1.0.0');
    writeLibPackage(
      'typed-lib',
      'MIT',
      "import type { A } from 'types-only-dep';\nexport type B = A;\n",
    );
    writeManifest('main', [libBundleModule('typed-lib')]);

    const { packages } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(packages.find((p) => p.name === 'types-only-dep')).toBeUndefined();
  });

  it('ignores a first-party workspace package installed as a symlink', () => {
    // platform-bible-react imports platform-bible-utils; a package of this repository is not a
    // third-party notice. npm installs a workspace package as a SYMLINK from node_modules into
    // lib/, so the resolved path looks like any other installed dependency until the link is
    // followed - which is exactly how this slipped through and put a first-party package into the
    // committed artifact. The fixture links it the same way rather than writing a plain directory.
    writeLibPackage('sibling-lib', 'MIT', 'export const a = 1;\n');
    writeLibPackage('importer-lib', 'MIT', "import { a } from 'sibling-lib';\nexport default a;\n");
    writeManifest('main', [libBundleModule('importer-lib')]);
    fs.mkdirSync(path.join(repo, 'node_modules'), { recursive: true });
    fs.symlinkSync(
      path.join(repo, 'lib', 'sibling-lib'),
      path.join(repo, 'node_modules', 'sibling-lib'),
      'junction',
    );
    ...
    expect(packages.find((p) => p.name === 'sibling-lib')).toBeUndefined();
  });
});
...
describe('collectShippedPackages - stylesheet leaf scan', () => {
  ...
  it('discounts a bare Sass import that resolves to first-party source', () => {
    ...
    // `packageOfSpecifier`'s test and resolves against no `node_modules`, so without the discount
    // it is reported as an unresolved package on every run - and a note that always fires is a note
    // nobody reads on the day it names a genuinely missing font.
    writeStylesheet('src/renderer/styles/_vars.scss', '$x: 1;');
    writeStylesheet('src/renderer/app.component.scss', "@use 'styles/vars';");
    writeManifest('main', []);
    const { packages, unresolvedStylesheetSpecifiers } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
  ...
  it('still reports a bare specifier with no first-party file behind it', () => {
    // The discount is taken only on the evidence of a file Sass would load. With nothing on disk
    // the specifier is what the note exists for: a CSS-only package missing from the tree.
    writeStylesheet('src/renderer/app.component.scss', "@use 'styles/vars';");
    writeManifest('main', []);
    const { unresolvedStylesheetSpecifiers } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
    });
    expect(unresolvedStylesheetSpecifiers).toEqual(['styles']);
  });

  it('does not let a same-named first-party file hide an installed package', () => {
    // The discount must key on what Sass would actually load. A first-party `styles/_vars.scss`
    // says nothing about an unrelated `@import 'some-font'` in the same file.
    writePackage('node_modules/some-font', 'some-font', '1.0.0');
    writeStylesheet('src/renderer/styles/_vars.scss', '$x: 1;');
    writeStylesheet('src/renderer/app.component.scss', "@use 'styles/vars';\n@import 'some-font';");
    writeManifest('main', []);
  ...
  it('does not read a commented-out or merely described @import as a dependency', () => {
    // The scan matches import syntax, and prose about import syntax is written in it. An SCSS
    // partial explaining why a `.css` import behaves differently, or a stale import left commented
    // out, names a package this repository does not redistribute - and if that name resolves, the
    // row reaches a legal document.
    writePackage('node_modules/described-only', 'described-only', '1.0.0');
    writePackage('node_modules/commented-out', 'commented-out', '1.0.0');
    writeStylesheet(
      'src/renderer/app.scss',
      [
        "// Why this is a partial: sass-loader passes `@import 'described-only'` through as a",
        '// runtime CSS @import the bundled style string cannot resolve.',
  ...
  it('still reads an @import whose line contains a URL, which is not a comment', () => {
    // `//` opens a line comment in SCSS but also sits inside every absolute URL, so the scheme and
    // a protocol-relative `url(//…)` both have to survive comment removal.
    writePackage('node_modules/real-font', 'real-font', '1.0.0');
    writeStylesheet(
    ...
    expect(packages.map((p) => p.name)).toEqual(['real-font']);
  });
  ...
  it('surfaces an unresolved stylesheet specifier instead of dropping it silently', () => {
    ...
    // installed is real information - a pruned font, lockfile-vs-tree drift in CI - and nothing else
    // in the pipeline is positioned to catch it, so it must not vanish silently. It must NOT throw
    // (see the false positives documented at resolvePackageLeaf) and must NOT affect the resolved
    // package set.
...
describe('a yalc dev link is described from package-lock.json, subtree and all', () => {
  ...
  it('CONTROL: the same, when the importer is a WORKSPACE rather than the root', () => {
    // Every extension and `lib/*` package is a workspace, so this is the shape the live case has.
    const link = writeLinkedPackage('linked-pkg', '9.9.9');
    const hoisted = writePackage('node_modules/dep', 'dep', '1.0.0');
    writeManifest('main', [path.join(link, 'index.js'), path.join(hoisted, 'index.js')]);
    writeLock({
...
describe('the module manifests must all come from one build', () => {
  ...
  /** What `prebuild` leaves behind for the most recent build to START. */
  function writeBuildId(id: string) {
    const dir = path.join(repo, '.notices');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'build-id'), `${id}\n`);
  }

  it('refuses a coherent set that a later build has already superseded', () => {
    // The case agreeing-with-each-other cannot see. `prebuild` mints the id before any webpack
    // starts, so a `npm run build` that dies early - `concurrently --kill-others-on-fail` stopping
    // the rest when one leg fails - leaves a new id on disk beside a COMPLETE set of manifests all
    // still carrying the previous one. Five files, none empty, one stamp, cache cold: every other
    // guard passes, and the document is written from a graph that has since been superseded.
    const pkg = writePackage('node_modules/dep', 'dep', '1.0.0');
    writeStampedManifest('main', 'build-a', [path.join(pkg, 'index.js')]);
    writeStampedManifest('renderer', 'build-a', [path.join(pkg, 'index.js')]);
    writeBuildId('build-b');

    expect(() =>
      collectShippedPackages({ manifestDir: path.join(repo, '.notices', 'modules'), repo }),
    ).toThrow(/written by build build-a[\s\S]*build-b[\s\S]*npm run build/);
  });

  it('accepts a coherent set that matches the build id on disk', () => {
    const pkg = writePackage('node_modules/dep', 'dep', '1.0.0');
    writeStampedManifest('main', 'build-a', [path.join(pkg, 'index.js')]);
    writeStampedManifest('renderer', 'build-a', [path.join(pkg, 'index.js')]);
    writeBuildId('build-a');
...
describe('a manifest built against a warm filesystem cache is refused', () => {
  ...
  it('reports the warm bundles instead of throwing when asked to', () => {
    ...
    const { packages, warmBundles } = collectShippedPackages({
      manifestDir: path.join(repo, '.notices', 'modules'),
      repo,
      warmCache: 'report',
    });
    expect(warmBundles).toEqual(['renderer']);
    expect(packages.map((entry) => entry.name)).toEqual(['dep']);
  });
...
describe('a repository reached through a symlink', () => {
  // webpack resolves symlinks, so every module it reports is a REAL path, while everything else is
  // expressed against the repository path as the caller spells it. Where the repository itself is
  // reached through a symlink those disagree and every module reads as living outside the
  // repository - the run then dies claiming the dependency tree is incomplete when nothing is wrong
  // with it. macOS reaches this by default (`/var` is a symlink to `/private/var`, and
  // `os.tmpdir()` returns the former), so it is reproduced here with an explicit symlink rather
  // than left to one platform's CI leg to discover.

  it('resolves modules reported under the resolved path', () => {
    const linkedRepo = path.join(os.tmpdir(), `notices-symlinked-${process.pid}-${Date.now()}`);
    fs.symlinkSync(repo, linkedRepo, 'junction');
    try {
      const dir = writePackage('node_modules/real-pkg', 'real-pkg', '1.0.0');
      // What webpack emits: the module under the RESOLVED repository path.
      writeManifest('main', [path.join(fs.realpathSync(dir), 'index.js')]);

      // Asked about the repository by its SYMLINKED path, which is the mismatch.
      const { packages } = collectShippedPackages({
        manifestDir: path.join(linkedRepo, '.notices', 'modules'),
        repo: linkedRepo,
      });

      expect(packages.map((pkg) => pkg.name)).toEqual(['real-pkg']);
      // Respelled against the repository as asked about, so the directory that reaches the lockfile
      // lookup and licensee is the one the caller would recognise.
      expect(packages[0].dir.startsWith(linkedRepo)).toBe(true);
    } finally {
      // A directory symlink on Windows is a junction, which `unlinkSync` does not always remove.
      try {
        fs.unlinkSync(linkedRepo);
      } catch {
        fs.rmdirSync(linkedRepo);
      }
    }
  });

  it('leaves a path that is already spelled the same way alone', () => {
    // The no-symlink case, which is every other machine: the respelling must be a no-op there
    // rather than something that rewrites paths on every run.
    expect(inRepoSpelling('/repo/node_modules/x/i.js', '/repo', '/repo')).toBe(
      '/repo/node_modules/x/i.js',
    );
    // And a path outside the resolved repository is not rewritten INTO it.
    expect(inRepoSpelling('/elsewhere/x.js', '/repo', '/private/repo')).toBe('/elsewhere/x.js');
  });
});

describe('packages only another platform installs', () => {
  // npm honours `os`/`cpu`, so a platform-restricted optional dependency is absent from the tree
  // everywhere else - and this artifact is generated on Linux. `fsevents` is the live case: chokidar
  // pulls it into the `main` and `extension-host` bundles on macOS, and no Linux run can observe it,
  // so the macOS installer redistributes a package the document does not list. Listed in the policy
  // and described from `package-lock.json`, the way a dev link is.

  function writeRootLock(packages: Record<string, object>) {
    fs.writeFileSync(
      path.join(repo, 'package-lock.json'),
      JSON.stringify({ lockfileVersion: 3, packages }),
    );
  ...
  it('describes a listed package from the lockfile, marked as unread', () => {
    writeRootLock({
      'node_modules/fsevents': { version: '2.3.3', license: 'MIT', os: ['darwin'], optional: true },
    });
    const [pkg] = collectPlatformOnlyPackages(repo, ['fsevents']);
    ...
    // The constraint that makes it invisible here is recorded, so a reader of the derived set can
    // tell why it was described rather than observed.
    expect(pkg.reachedVia).toEqual(['platform-only:darwin']);
  });

  it('refuses a listed package the lockfile does not record', () => {
    // The dependency is gone, or the lockfile is stale. Either way the entry cannot be honoured, and
    // dropping it silently takes a package out of the document that another platform ships.
    writeRootLock({ 'node_modules/other': { version: '1.0.0', license: 'MIT' } });
    expect(() => collectPlatformOnlyPackages(repo, ['fsevents'])).toThrow(
      /records no entry for it/,
    );
  });

  it('refuses a listed package that npm installs everywhere', () => {
    // With no os/cpu constraint, THIS platform's tree is the authority on whether it ships and the
    // module graph is where it belongs. Listing it here would put an unobserved row in the document.
    writeRootLock({ 'node_modules/everywhere': { version: '1.0.0', license: 'MIT' } });
    expect(() => collectPlatformOnlyPackages(repo, ['everywhere'])).toThrow(
      /no os\/cpu constraint/,
    );
  });

  it('adds nothing when the policy lists nothing', () => {
    // Reads no lockfile either: a fixture repo without one stays usable.
    expect(collectPlatformOnlyPackages(repo, [])).toEqual([]);
  });
});
...
describe('the shipping set is ordered by byte comparison, not ICU collation', () => {
  it('orders 9.0.0 before 10.0.0, which localeCompare does not', () => {
    // `compare.ts` exists because this pipeline's output is committed and byte-compared. Both
...
describe('acknowledging a shrink', () => {
  // `!!process.env[ACCEPT_SHRINK_ENV]` is TRUE for '0', 'false', 'off' and 'no', so the single
  // documented way to turn this gate off also turned it off when spelled as an attempt to turn it
  // ON - while `assertNpmNotShrunk`'s docstring calls the escape "impossible to do by accident".
  // With it wrongly on, only the frozen NPM_MIN_PACKAGES (120, against a closure of 218) stands
  // between a short set and the committed artifact.
  it.each(['1', 'true', 'TRUE', ' True '])('accepts %j', (value) => {
    expect(acceptShrinkFromEnv({ [ACCEPT_SHRINK_ENV]: value })).toEqual({ accepted: true });
  });

  it.each(['0', 'false', 'off', 'no', 'yes', 'ok'])('does not accept %j, and says so', (value) => {
    expect(acceptShrinkFromEnv({ [ACCEPT_SHRINK_ENV]: value })).toEqual({
      accepted: false,
      ignored: value,
    });
  });

  // Absent and empty are not attempts to reach the escape, so they are silent - reporting them
  // would put a warning in front of every ordinary run.
  it.each([undefined, '', '   '])('is silently off for %j', (value) => {
    const env = value === undefined ? {} : { [ACCEPT_SHRINK_ENV]: value };
    expect(acceptShrinkFromEnv(env)).toEqual({ accepted: false });
  });
});

describe('readDirectDependencies', () => {
  /** Writes a first-party manifest with the given `dependencies` block. */
  function writeFirstPartyManifest(rel: string, dependencies: Record<string, string>) {
    const file = path.join(repo, rel);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(
      file,
  ...
  it('reads every manifest that describes something this repository ships', () => {
    writeFirstPartyManifest('package.json', { root: '^1.0.0' });
    writeFirstPartyManifest('release/app/package.json', { unbundled: '^1.0.0' });
    writeFirstPartyManifest('extensions/package.json', { aggregate: '^1.0.0' });
    writeFirstPartyManifest('extensions/src/quick-verse/package.json', { extension: '^1.0.0' });
    writeFirstPartyManifest('lib/platform-bible-react/package.json', { component: '^1.0.0' });

    expect(
      readDirectDependencies(repo)
        .map((d) => d.name)
        .sort(),
    ).toEqual(['aggregate', 'component', 'extension', 'root', 'unbundled']);
  });

  it('reads dependencies only, not devDependencies or peerDependencies', () => {
    // A devDependency is not distributed, and a peerDependency is a requirement on the host rather
    // than this package's statement that it carries something.
    ...
    expect(readDirectDependencies(repo).map((d) => d.name)).toEqual(['shipped']);
  });

  it('records which manifest declared each one', () => {
    writeFirstPartyManifest('extensions/src/quick-verse/package.json', { thing: '^1.0.0' });

    expect(readDirectDependencies(repo)).toEqual([
      { name: 'thing', range: '^1.0.0', declaredIn: 'extensions/src/quick-verse/package.json' },
    ]);
  });
});

describe('missingDirectDependencies', () => {
  const declared = (name: string, range = '^1.0.0', declaredIn = 'package.json') => ({
    name,
    range,
    declaredIn,
  });

  it('reports a declared dependency the derived set does not contain', () => {
    // The failure this exists for: the module manifests are short by one package, which neither
    // `assertNpmFloor` nor `assertNpmNotShrunk` can see, and the artifact silently loses a row.
    expect(
      missingDirectDependencies([{ name: 'present' }], [declared('present'), declared('dropped')]),
    ).toEqual([declared('dropped')]);
  });

  it('discounts a file: range, which links first-party code rather than a package', () => {
    expect(
      missingDirectDependencies(
        [],
        [declared('platform-bible-utils', 'file:../platform-bible-utils')],
      ),
    ).toEqual([]);
  });

  it('accepts a dependency the policy records as reaching no bundle', () => {
    expect(
      missingDirectDependencies([], [declared('declared-but-unused')], ['declared-but-unused']),
    ).toEqual([]);
  });

  it('reports one entry per name however many manifests declare it', () => {
    // Fourteen manifests declare `platform-bible-utils`; a failure naming it fourteen times buries
    // whatever else is wrong beside it.
    const missing = missingDirectDependencies(
      [],
      [
        declared('shared', '^1.0.0', 'package.json'),
        declared('shared', '^1.0.0', 'extensions/src/a/package.json'),
        declared('shared', '^1.0.0', 'lib/b/package.json'),
      ],
    );
    expect(missing).toHaveLength(1);
    expect(missing[0].declaredIn).toBe('package.json');
  });
});
...
describe('the committed policy against this repository', () => {
  ...
  it('records every declared runtime dependency that reaches no bundle today', () => {
    // The live check, against the real manifests: a new production dependency whose modules never
    // get recorded fails here rather than quietly shortening the document.
    const committed = readJsonFile<{ packages: { ecosystem: string; name: string }[] }>(
      path.join(REPO_ROOT, 'THIRD-PARTY-NOTICES.lock.json'),
      'the committed notices lock',
    );

    expect(
      missingDirectDependencies(
        committed.packages.filter((pkg) => pkg.ecosystem === 'npm'),
        readDirectDependencies(REPO_ROOT),
        Object.keys(policy.unbundledDependencies || {}),
      ),
    ).toEqual([]);
  });

  it('gives every recorded entry a reason', () => {
    // The entry IS the record that somebody looked, so one without a reason says only that a name
    // was added to the exemption list.
    const withoutReason = Object.entries(policy.unbundledDependencies || {})
