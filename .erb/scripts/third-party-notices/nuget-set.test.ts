...
// normalizeValidationErrors, which strips it down to the string policy.js's contract expects.
...
describe('mergeRidResults', () => {
  ...
  it('orders by byte comparison and NUMERIC version, not ICU collation', () => {
    // `compare.js` exists because this pipeline's output is committed and byte-compared: ICU
    ...
    // this term is exercised in production. `render.js` re-sorts before anything is written, so
  ...
  it('carries LicenseInformationOrigin Unknown through as an undeclared license', () => {
    ...
    // permissive result; policy.js blocks on an undeclared field with no text. nuget-license
...
describe('collectNugetPackages checks each RID before merging them', () => {
  ...
  // reason the four-RID union exists (ADR-0023). Silently, with a healthy-looking count.
  //
  // Driven in a child process because `nuget-set.js` destructures `execFileSync` at module load, so
  ...
  function runCollect(shortRidCount: number) {
    const script = `
      ...
      const { collectNugetPackages } = require(
        path.join(${JSON.stringify(__dirname)}, 'nuget-set.js'),
      );
      try {
...
// ships" - the two filters the old project.assets.json walker's collectDotnetPackages applied
// (shipsSomething, isFrameworkPackage), now read from whichever RID's assets file the caller just
// restored.
...
describe('parseShippingSet', () => {
  ...
  it('records assemblies a package ships under a name other than its own id', () => {
    // The real case the old walker's comment names: SharpZipLib ships ICSharpCode.SharpZipLib.dll.
    const assets = assetsWithTarget({
      'SharpZipLib/1.4.2': {
        runtime: {
...
describe('normalizeValidationErrors', () => {
  ...
  it('returns an empty array for a missing or malformed value', () => {
    expect(normalizeValidationErrors(undefined)).toEqual([]);
    expect(normalizeValidationErrors('not an array')).toEqual([]);
  });
});
...
describe('readDirectPackageReferences', () => {
  ...
  it('reads each reference regardless of attribute order', () => {
    ...
    expect(readDirectPackageReferences(file)).toEqual([
      { id: 'icu.net', version: '3.0.1' },
      { id: 'StreamJsonRpc', version: '2.22.11' },
    ]);
  });
  ...
  it('reports an MSBuild property reference as no version at all', () => {
    ...
    expect(readDirectPackageReferences(file)).toEqual([{ id: 'ParatextData', version: undefined }]);
  });
...
describe('bundled license files', () => {
  ...
  describe('attachLicenseFiles', () => {
    ...
    it('attaches a NOTICE, so the NuGet half can discharge Apache-2.0 section 4(d)', () => {
      // Notices have to be read on the NuGet side too, or `render.js`'s NOTICE section can never
