...
import { buildLock, diffLock, diffShippingSet, readLock, writeLock } from './lock';
...
const lock = (...v: ReturnType<typeof verdict>[]) =>
  buildLock({ verdicts: v, licenseeVersion: '9.18.0', corpusVersion: '6.6.0' });
...
describe('writeLock / readLock', () => {
  it('round-trips without diffLock seeing a difference, even with undefined fields', () => {
    ...
    // expected, see policy.js), so this is the realistic case, not an edge case. The round-trip
