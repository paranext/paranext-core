...
import type { DetectedFile, Election, Policy, Verdict } from './types';
 ...
 *
 *   Where a package publishes NO licence file in its tarball - the `@radix-ui/*` family, which
 *   publishes dozens of packages from one monorepo, is the bulk of it - the notice is read from the
 *   licence file in the repository that package's own `package.json` names, which is the same
 *   document by a different route. Two packages carry their licence text inside `README.md` and the
...
function applyException(
  ...
  if (unsigned.length)
    return blocked(
      `the reviewed exception for ${key}@${version} records no ${unsigned.join(' and no ')}. An ` +
        "exception is a determination someone made by reading the package's licence file; " +
  ...
  // is a human's determination about the same package and buys no licence to state something
  ...
  // would clear the gate and then describe a licence the package is not under.
...
function resolveDeclaredPrefix({
  ...
  // as dropping a licence exception and is refused for the same reason.
 ...
 * `CC-BY-SA-*` and `CC-BY-NC-*` all read as "not copyleft" and passed, while their text was
 * reproduced into the document either way. An id that is simply unlisted must not default to
 * admissible because it is absent from the copyleft list.
 *
...
export function classify({
  ...
  // FIRST wins, not last. `new Map(usable.map(...))` let a later file with the same identifier
  // displace an earlier one, and `detect.rb` orders files by filename score - so a package's own
  // `LICENSE` could be displaced by a bundled `LICENSE.thirdparty` identifying as the same licence.
  // The verdict then names the bundled file, and `buildLock` pins its hash: after which `diffLock`'s
  ...
  // that ARE usable: `copyleft.has(id) || !allowed.has(id)`. Testing the copyleft list alone made
  // this the one admission path in the file shaped as a denylist, so it saw only the 21 ids somebody
  // had already thought of - `GPL-1.0-*`, `AGPL-1.0-*`, `LGPL-2.0-*`, `MPL-1.0`, `OSL-*`, `CPL-1.0`,
  // `QPL-1.0`, `RPL-*`, `Sleepycat`, `EUPL-1.0/1.1`, `BUSL-1.1`, `Elastic-2.0` and `CC-BY-SA-*` all
  // read as "not copyleft" and passed, while their text was reproduced into the document either
  // way. An id that is simply unlisted must not default to allowed just because it is absent from
  // the copyleft list; the same sentence is already written about the text-derived path below.
  //
  // Sentinels are excluded explicitly. `copyleft.has('NOASSERTION')` was false, so the denylist
  // form skipped an unidentified file for free; `!allowed.has('NOASSERTION')` is TRUE, so without
  // this every package shipping a file licensee could not identify would block - which is case (2)
  // below's job, decided there with the declaration in hand rather than here.
  ...
  // Testing copyleft alone left this the one admission path in the file still shaped as a denylist:
  // a text identifying as `CC-BY-NC-4.0` or `BUSL-1.1` at 100% was cleared by an exception recording
  // `MIT`, and its text was reproduced under a row labelled MIT.
  //
  ...
  const withException = (result: Verdict): Verdict => {
  ...
  // one licence text), so it is consulted first and an override never rescues a package whose
  // exception failed its own pinning.
  const override = (policy.overrides || {})[key];
  // "No licence text was identified" is a statement about the FILES, not about `best`. `best` is
  ...
  // keyed by name, pinned to no licence text - so it must not be the more permissive one.
  //
  ...
  const overridable = !!override && !declared.ok && !identifiedText;
  ...
  const settle = (result: Verdict): Verdict => {
    ...
    // An override is keyed by NAME, and is pinned to neither a version nor a licence text - so a
    // determination someone made by reading one version's nuspec kept applying, unreviewed, to
    // whatever came back under that name. `version` is optional (most entries are about a package
      ...
      return {
        ...
        ...blocked(
          `the "overrides" entry for "${key}" records a determination made against version ` +
            `${override.version}, and this is ${version}. An override is not pinned to a licence ` +
      ...
      return {
        ...
        ...blocked(
          `the "overrides" entry for "${key}" records neither the "version" the determination was ` +
            ...
            'to no licence text, so without one of the two it applies unreviewed to whatever ' +
    ...
    // `recorded.ok &&` short-circuited the copyleft test to `false`, so the check below applied
    // ONLY to an override that happened to be spelled as an SPDX expression - and free text is what
    // this field is designed to hold (five of the seven live entries use it). An entry reading
    // "GNU General Public License v3" therefore returned `overridden` while the same determination
    // spelled `GPL-3.0-only` blocked: the gate was strictest on the input class it could actually
    // check, and silent on the one it could not.
    //
    ...
    // made this a denylist over 21 ids, so an override spelled as a real SPDX identifier the policy
    // has never admitted - `CC-BY-NC-4.0`, `BUSL-1.1` - was recorded as the licence of a shipped
    // package with nothing checking it. An override still bypasses the allow list where its value
