...
describe('readPackageLicenseFiles', () => {
  // EVERY file, not the first: a dual-licensed package ships one per branch, and reproducing only
  // one of them satisfies neither license.
  it('reads every license file a package ships, in filename order', () => {
    ...
    expect(readPackageLicenseFiles(dir)).toEqual([
      { name: 'LICENSE-APACHE', text: 'apache text' },
      { name: 'LICENSE-MIT', text: 'mit text' },
    ]);
  });

  it('reports nothing for a package that bundles none', () => {
    expect(readPackageLicenseFiles(packageDir({ 'README.md': 'hi' }))).toEqual([]);
    expect(readPackageLicenseFiles(undefined)).toEqual([]);
  });
});
...
describe('isNugetLicenseFileName', () => {
  ...
  it('stays stricter for npm, where the file is a verdict signal rather than a reproduction', () => {
    // For npm the licence file is the second signal `policy.js` reconciles the manifest against, so
