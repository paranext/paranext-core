 ...
 * installs. A completed packaging run also leaves nothing at the staged path -- observed, not
 * explained: electron-builder itself does not delete it (`snap.js` takes a raw path from
 * `createStageDirPath` and registers no cleanup), so whatever does lives further down.
 */
 ...
 *
 * `base` is read from this same metadata rather than from `electron-builder.json5`, so the artifact
 * is checked against itself rather than against an input the sibling guard already covers. See
 * `assert-generated-snap-metadata.ts` for why reading the artifact is the point.
 */
  ...
  if (key !== expected)
    problems.push(
      `The generated snap mounts \`${key}\` at ${GNOME_PLATFORM_TARGET}, but base \`${base}\` ` +
        `pairs with \`${expected}\`. Usually this means the app-builder-lib patch stopped applying: ` +
        'the template pins its own default regardless of `base`. It matters because snapd matches ' +
        'stored connections by plug name across a refresh, so a stale name revives the stale ' +
        'connection on every existing install.',
    );
