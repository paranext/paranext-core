 ...
 * (`.erb/scripts/notices-build-id.js`) before any webpack starts.
 *
 ...
 * under-report of what ships. `shipping-set.js` refuses a set whose stamps disagree.
 *
 ...
 * `shipping-set.js`'s refusal message and its test coverage), and directory presence is the
 * mechanism that is trivial to prove both ways.
 *
 * Only `renderer` (`.erb/configs/webpack.config.renderer.prod.ts`) and `extensions`
 * (`extensions/webpack/webpack.config.base.ts`) configure a filesystem cache at all - `main` and
 * `extension-host` set none, so `compiler.options.cache` there is never `{ type: 'filesystem' }`
 * and this is always `false` for them; a single `npm run build` invocation is one process with no
 * cross-process memory cache to be warm from either. CI is unaffected for a different reason: its
 * `setup-node` step caches `~/.npm`, not `node_modules/.cache/webpack-*`, and there is no
 * `actions/cache` step for the latter, so every CI build is cold. This guard exists for the
 * developer who regenerates notices locally on an already-built tree.
 *
 * One over-flagging wrinkle: `extension-main` (`extensions/webpack/webpack.config.main.ts`)
 * declares `dependencies: ['webView']`, so it starts only after `extension-web-view` finishes - and
 * both share the SAME `webpack-extensions` cache directory. On the very first-ever build of a fresh
 * checkout, `extension-web-view` writes its cache entries as it compiles, and by the time
 * `extension-main` starts moments later that directory already holds content - so `extension-main`
 * can be stamped `cacheWarm: true` on a build that is, in every other sense, genuinely cold. This
 * is still the safe direction: it costs an extra cache-clear-and-rebuild, never a silently short
 * artifact, so it is left as-is rather than special-cased.
 */
export function isWarmFilesystemCache(cache: webpack.Configuration['cache']): boolean {
  ...
  const dir = cache.cacheDirectory;
  if (!dir) return false;
  try {
    return fs.readdirSync(dir).length > 0;
  } catch {
    // ENOENT (never built here before) or any other reason the directory can't be read: nothing to
    // have been served from, so this is cold.
    return false;
  }
}
 ...
 * entry.
 *
 * Also stamps `cacheWarm` (see `isWarmFilesystemCache`) so `shipping-set.js` can refuse a manifest
...
export class EmitShippedModulesPlugin {
  ...
  apply(compiler: webpack.Compiler) {
    ...
    compiler.hooks.done.tap(NAME, (stats) => {
      ...
      // Staged and renamed, like the artifact and lock `main.js` writes: five of these are written
      ...
      fs.writeFileSync(
        tmp,
        `${JSON.stringify(
          { bundle: bundleName, buildId: build, cacheWarm, modules: [...resources].sort() },
          undefined,
          2,
        )}\n`,
      );
