...
| Standard View           | [Standard-View-Invariants.md](.context/standards/Standard-View-Invariants.md) | **Read before touching the Standard-view editing path** (marker-edit engine, settle, display runs, USFM tokenizer, USJ/USFM writers): the invariants those layers keep with each other, the ratified behaviors not to "fix", and the C# serialization approval gate |
