...
Any new C# code path that mutates project data must wrap the mutation in a
`SendReceiveWriteLock.EnterWrite(projectId)` scope. Full rules, threading semantics, and the
coverage-test exemption forms load automatically when working under `c-sharp/` — see
[`.claude/rules/architecture/send-receive-write-gate.md`](.claude/rules/architecture/send-receive-write-gate.md).
