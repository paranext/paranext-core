...
(async () => {
  ...
  try {
    await execCommand(bumpVersionCommand, { cwd: path.join(__dirname, '..') });
  } catch (e) {
  ...
  for (const ext of extensions) {
    ...
    try {
      await fs.promises.access(path.join(sourceFolder, ext.dirName, 'package.json'));
    } catch (e) {
      console.log(
    ...
    if (packageJsonExists) {
      try {
        await execCommand(bumpVersionCommand, {
          cwd: path.join(__dirname, '..', sourceFolder, ext.dirName),
        });
      } catch (e) {
