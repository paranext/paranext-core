...
export function ResourceTextPanel({
  ...
  //
  // The data layer preserves its value across a source change, and the `ChapterUSJ` selector does
  // not change when only the resource does — so after a switch the PREVIOUS resource's chapter is
  // still here, `isUsjLoading` never rises, and a valid USJ carries no book or project of its own to
  ...
  if (resourceProjectId && answeredForRef.current.value !== usjPossiblyError)
