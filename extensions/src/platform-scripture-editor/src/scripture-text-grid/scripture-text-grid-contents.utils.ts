 ...
 * `isResourceShownForUser`). Preserving newer versions mirrors the C# project-setting path —
 * stamping `CURRENT_DATA_VERSION` unconditionally would make the write a downgrade the validator
 * rejects once a future build has stored a higher version. A malformed incoming version is also
 * lifted.
 */
