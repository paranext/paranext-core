...
describe('ALIGNED_GRID_STYLESHEET', () => {
  ...
  it('flattens the resource’s content-zoom marker and pins its own zoom, so the level lands once', () => {
    // The marker sits between the content wrapper and the verse blocks: generating a box, it would
    // cut them off from the shared rows; keeping the platform's `zoom` for its area, it would square
    // the level the verse blocks already take.
    expect(ALIGNED_GRID_STYLESHEET).toMatch(
      new RegExp(
        `\\.${ALIGNED_GRID_CLASS} \\[data-platform-content-zoom-root\\]\\{[^}]*display:contents;[^}]*zoom:1;`,
      ),
    );
  });
...
describe('buildAlignedZoomStyle', () => {
  it('hands the verse blocks the platform’s level for the column’s own area, else the pane default', () => {
    // The same fallback chain as the platform's rule for a marker, so a column shows the level its
    // zoom menu reports whether or not the resource has a level of its own.
    expect(buildAlignedZoomStyle('resource-web')).toEqual({
      [ALIGNED_ZOOM_PROPERTY]:
        'var(--platform-content-zoom-resource-web,var(--platform-content-zoom-default,1))',
    });
  });
});
