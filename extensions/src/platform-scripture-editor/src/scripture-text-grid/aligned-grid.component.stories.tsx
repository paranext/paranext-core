 ...
 * stylesheet keys on, including the content-zoom marker around the text. Alignment is native CSS,
 * so a story that reproduces the markup demonstrates the real thing rather than a picture of it,
 * and Chromatic catches a break in the chain that no unit test can see (jsdom lays nothing out).
 */
...
function StubColumn({
  ...
  const zoomArea = resourceZoomAreaOf(label);
  return (
    <div
      role="region"
      aria-label={label}
    ...
    >
      <div data-cell-root className="tw:group tw:flex tw:min-w-0 tw:flex-col">
        ...
        <div
          ...
          dir={textDirection}
          style={buildAlignedZoomStyle(zoomArea)}
        >
          <div data-cell-pad className="tw:p-2">
            <div
              data-platform-content-zoom-root={zoomArea}
              data-platform-content-zoom-label={label}
            >
 ...
 *
 * The level and the marker rule stand in for what the platform injects into a web view: the GRK
 * area's variable, and the rule that zooms that area's marker. The grid pins the marker's own zoom
 * to 1, so the column shows the level once, not squared.
 */
const GRK_ZOOMED_BOX_STYLE: React.CSSProperties & {
  '--platform-content-zoom-resource-grk': number;
} = { ...GRID_BOX_STYLE, '--platform-content-zoom-resource-grk': 1.5 };

export const MixedZoom: Story = {
  render: () => (
    <div style={GRK_ZOOMED_BOX_STYLE}>
      <style>
        {
          '[data-platform-content-zoom-root="resource-grk"]{zoom:var(--platform-content-zoom-resource-grk,1)}'
        }
      </style>
      <AlignedGrid scrRef={scrRef} ariaLabel="Text Collection">
        <StubColumn label="GRK" verses={versePerRow} />
