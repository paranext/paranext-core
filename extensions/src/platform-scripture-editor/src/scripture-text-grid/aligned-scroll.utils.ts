 ...
 *
 * Bounding rects are in viewport pixels and include every ancestor's CSS `zoom`, while `scrollTop`,
 * `clientTop` and `offsetHeight` are in the port's unzoomed layout pixels. The content zoom this
 * view applies sits below the port, on the verse blocks, so it never enters the ratio; measuring
 * the ratio off the port itself covers zoom from any ancestor a host puts above it. Falls back to 1
 * when the port has no layout height, where there is no ratio to measure.
 *
