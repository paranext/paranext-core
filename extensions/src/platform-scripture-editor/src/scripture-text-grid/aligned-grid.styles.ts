 ...
 *
 * In the other views the platform scales a resource by putting `zoom` on the element that marks its
 * text as a content-zoom area (`ContentZoomRoot`). That cannot work here: the marker sits between
 * the column's content wrapper and the verse blocks, so it must generate no box or the blocks stop
 * reaching the shared rows, and the wrapper itself is a `grid-template-rows: subgrid` box, where
 * `zoom` would scale the row tracks it inherits along with the text — a zoomed column would measure
 * its rows against a different scale than its neighbours. So this view pins the marker's own zoom
 * to 1 and zooms the verse blocks instead, which leaves the tracks alone and keeps the row model
 * intact: a row is still as tall as its tallest cell, that cell is simply zoomed.
 */
 ...
 */
export type AlignedZoomStyle = CSSProperties & { [K in typeof ALIGNED_ZOOM_PROPERTY]?: string };

/**
 * The platform's per-area zoom variables. `@papi/core` is types-only, so extension code writes
 * these literals itself and keeps them equal to `CONTENT_ZOOM_CSS_VARIABLE_PREFIX` and
 * `CONTENT_ZOOM_DEFAULT_CSS_VARIABLE`.
 */
const PLATFORM_ZOOM_VARIABLE_PREFIX = '--platform-content-zoom-';
const PLATFORM_DEFAULT_ZOOM_VARIABLE = '--platform-content-zoom-default';

/**
 * The style that hands one column's zoom level to its verse blocks: the platform's variable for the
 * column's zoom area, else the pane default — the same fallback the platform's own rule for a
 * marker uses, so a column shows the level its zoom menu reports.
 *
 * @param zoomArea The resource's content zoom area id.
 * @returns The style to set on the column's content wrapper.
 */
export function buildAlignedZoomStyle(zoomArea: string): AlignedZoomStyle {
  return {
    [ALIGNED_ZOOM_PROPERTY]: `var(${PLATFORM_ZOOM_VARIABLE_PREFIX}${zoomArea},var(${PLATFORM_DEFAULT_ZOOM_VARIABLE},1))`,
  };
}
...
/* The resource's content-zoom marker. It stays in the DOM, since it is what makes the resource a
   zoom area and what the platform resolves a click or Ctrl+wheel over the text to, but its zoom is
   pinned to 1 and the verse blocks take the level instead (see ALIGNED_ZOOM_PROPERTY). Out-specifies
   the platform's own [attr="<area>"] rule for it. */
.${ALIGNED_GRID_CLASS} [data-platform-content-zoom-root]{
  display:contents;
  zoom:1;
}
