 ...
 */
function createColumn3PanelsMockPapi(openDefs: Array<Partial<SavedWebViewDefinition>> = []) {
  const mockSendCommand = vi.fn().mockResolvedValue(undefined);
...
describe('updateRelatedTextCollectionPanel', () => {
  it('reloads the open panel with the incoming project', async () => {
    const { papi, mockReloadWebView } = createColumn3PanelsMockPapi([gridDef('proj-a')]);
  ...
  it('re-points a panel that has no project yet', async () => {
    ...
    const { papi, mockReloadWebView } = createColumn3PanelsMockPapi([gridDef(undefined)]);
  ...
  it('does not open a panel when none is open', async () => {
    ...
    const { papi, mockReloadWebView, mockOpenWebView } = createColumn3PanelsMockPapi([
  ...
  it('resolves without throwing and warns when the reload fails', async () => {
    const { papi, mockReloadWebView, mockWarn } = createColumn3PanelsMockPapi([gridDef('proj-a')]);
  ...
  it('resolves without throwing and warns when the open-web-view probe fails', async () => {
    const { papi, mockGetAllOpenWebViewDefinitions, mockWarn } = createColumn3PanelsMockPapi();
...
describe('openOrUpdateRelatedPanels', () => {
  ...
  it('still re-points the Text Collection when an earlier panel fails', async () => {
    const { papi, mockSendCommand, mockReloadWebView } = createColumn3PanelsMockPapi([
      gridDef('proj-a'),
    ]);
  ...
  it('still opens the four command-driven panels', async () => {
    const { papi, mockSendCommand } = createColumn3PanelsMockPapi([gridDef('proj-a')]);
