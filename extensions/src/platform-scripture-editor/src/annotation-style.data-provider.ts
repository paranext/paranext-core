...
const styleEntriesByName = new Map<string, AnnotationStyleEntry>();
let latestStylesheet: string | undefined;
  ...
  implements IDataProviderEngine<AnnotationStyleDataProviderDataTypes>
{
  ...
  async getAnnotationStylesheet(): Promise<string> {
    if (latestStylesheet) return latestStylesheet;

    const cssRules: string[] = [];

    // Generate CSS rules for each registered annotation style
    [...styleEntriesByName.values()].forEach((entry) => {
    ...
    latestStylesheet = cssRules.join('\n\n');
    return latestStylesheet;
  }
