...
/**
 * The strip across the top of a resource pane that tells the reader about a text's licence.
 * Biblica's traditionally licensed texts get Biblica's reference-only notice, and "More info…"
 * opens Biblica's terms for use outside Paratext. A text whose copyright starts with
 * "Notification:" (the ESV) gets its own wording, as in Paratext 9.
 *
 * The message is clamped to two lines in a narrow pane; **Show more** appears only when it does not
 * fit. The X dismisses it for that text in that pane.
 */
const meta: Meta<typeof CopyrightNoticeBanner> = {
  ...
  tags: ['autodocs'],
  args: {
    notice: { kind: 'restrictedLicense', copyrightYears: '1973, 1978, 1984, 2011' },
    name: 'NIV11',
    fullName: 'New International Version 2011',
    localizedStrings: getLocalizedStrings([...COPYRIGHT_NOTICE_STRING_KEYS]),
    onDismiss: fn(),
    // Clicking the permissions link shows up in the Actions panel
    onOpenUrl: fn(),
  },
...
export const BiblicaTermsOpenWithoutYears: Story = {
  args: { notice: { kind: 'restrictedLicense', copyrightYears: '' } },
...
export const RightToLeftName: Story = {
  args: {
    notice: { kind: 'restrictedLicense', copyrightYears: '1988, 1997, 2012' },
    name: 'كتاب الحياة',
