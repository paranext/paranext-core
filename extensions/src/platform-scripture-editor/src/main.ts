...
class ScriptureEditorWebViewFactory extends WebViewFactory<typeof scriptureEditorWebViewType> {
  ...
  override async createWebViewController(
    ...
    return {
      ...
      async setAnnotation(range, annotationType, annotationId, interactionCommand, threadId) {
        try {
          logger.debug(
            `Platform Scripture Editor WebView Controller ${currentWebViewDefinition.id} received request to setAnnotation ${serialize({ annotationType, annotationId, interactionCommand, threadId })}`,
          );
          if (!currentWebViewDefinition.projectId)
            throw new Error(`webViewDefinition.projectId is empty!`);
      ...
      async createComment(range, contents, assignedUser) {
        const { projectId } = currentWebViewDefinition;
        if (!projectId) throw new Error('Cannot create comment: No project ID');

        try {
          logger.debug(
            `Platform Scripture Editor WebView Controller ${currentWebViewDefinition.id} received request to createComment`,
          );

          // Convert the range and get the UsjReaderWriter for text extraction
          const { scrRef, editorRange, usjReaderWriter, startLocation, endLocation } =
            await convertScriptureRangeToEditorRange(range, projectId);

          // Extract the selected text (up to 500 characters)
          const selectedText = usjReaderWriter.extractTextBetweenPoints(
            startLocation,
            endLocation,
            500,
          );

          // Extract context before (up to 50 characters)
          // We need to find a location before the start to extract context
          let contextBefore = '';
          try {
            // Get text from the beginning of the verse to the start of selection
            const verseStartLocation = usjReaderWriter.usfmVerseLocationToNextTextLocation(scrRef);
            const verseText = usjReaderWriter.extractText(verseStartLocation, 500);
            const selectionIndex = verseText.indexOf(selectedText);
            if (selectionIndex > 0) {
              contextBefore = verseText.substring(Math.max(0, selectionIndex - 50), selectionIndex);
            }
          } catch (contextError) {
            logger.debug(`Failed to extract context before: ${getErrorMessage(contextError)}`);
          }

          // Extract context after (up to 50 characters)
          let contextAfter = '';
          try {
            contextAfter = usjReaderWriter.extractText(endLocation, 50);
          } catch (contextError) {
            logger.debug(`Failed to extract context after: ${getErrorMessage(contextError)}`);
          }

          // Get the comments PDP and create the comment
          ...
          const threadId = await legacyCommentManagerPdp.createComment({
            contents,
            assignedUser,
          ...
          // Add an annotation to highlight the commented text
          const annotationMessage: EditorWebViewMessage = {
            method: 'setAnnotation',
            scrRef,
            annotationRange: editorRange,
            annotationType: 'translatorComment',
            annotationId: threadId,
            // TODO Rolf: How to implement interactionCommand?
          };
          await papi.webViewProviders.postMessageToWebView(
            currentWebViewDefinition.id,
            webViewNonce,
          ...
          const errorMessage = `Platform Scripture Editor WebView Controller ${currentWebViewDefinition.id} threw while running createComment! ${getErrorMessage(e)}`;
          logger.warn(errorMessage);
          throw new Error(errorMessage);
        }
      },
      async focusComment(threadId) {
        try {
          ...
          // Open the comment list for this editor's project
          ...
          if (!commentListController) {
            logger.warn(
              `Failed to get comment list controller for web view ${commentListWebViewId}`,
            );
            return;
          }
