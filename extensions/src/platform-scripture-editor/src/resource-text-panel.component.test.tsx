...
describe('ResourceTextPanel content that cannot be shown', () => {
  ...
  it('names the failure once the read settles with nothing delivered', () => {
    // The other half of the distinction above. A settled read holding no USJ is terminal — nothing
    // re-emits until the data provider does — so this must NOT be the spinner, which would claim
    // progress that never arrives. Only reachable because the subscription is seeded with
    // `undefined`; a blank-USJ seed reports this state as a chapter that genuinely has no content.
