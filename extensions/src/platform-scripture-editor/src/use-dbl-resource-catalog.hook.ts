...
export function useDblResourceCatalog(): DblResourceCatalogState {
  ...
  const [resourcesPossiblyUndefined, isLoadingResources] = usePromise(
    useCallback(async () => {
      ...
      //
      // The DBL rows wait, bounded, for the installed-flag reconciliation: a panel acts on those
      // flags, and a row cached before the C# project factory registered reads not-installed.
