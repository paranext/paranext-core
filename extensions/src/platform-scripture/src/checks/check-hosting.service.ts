  ...
  implements IDataProviderEngine<CheckRunnerDataTypes>
{
  ...
  enabledChecksByProjectId = new Map<
    string,
    {
      checkId: string;
      check: Check;
  ...
  async getAvailableChecks(): Promise<CheckRunnerCheckDetails[]> {
    ...
    Object.getOwnPropertyNames(registeredChecksById).forEach((registeredCheckId) => {
      const checkDetails = registeredChecksById.get(registeredCheckId);
  ...
  async enableCheck(checkId: string, projectId: string): Promise<string[]> {
    ...
    const retVal = await lock.runExclusive(async () => {
      ...
      // Make sure there is an entry in the map to store the check details
      if (!checksForProject) {
        checksForProject = [];
      ...
      const warnings = await checkObj.initialize(projectId);
      checksForProject.push({ checkId, check: checkObj, latestResults: [] });
  ...
  async rebuildResults(): Promise<void> {
...
const unregisterCheck: UnregisterCheckFunction = async (checkId: string): Promise<boolean> => {
...
const registerCheck: RegisterCheckFunction = async (
...
papi.commands.registerCommand('platformScripture.registerCheck', registerCheck);
