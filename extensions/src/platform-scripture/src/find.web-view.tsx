...
global.webViewComponent = function FindWebView({
  ...
  const getScopeSummaryText = () => {
    if (!submittedScope || !submittedVerseRef) return 'scope undetermined'; // todo

    const localizedBookName = localizedBookData.get(submittedVerseRef.book)?.localizedId;

    switch (submittedScope) {
      case 'selectedText':
        return scopeSelectorLocalizedStrings['%webView_scope_selector_selected_text%'];
      case 'verse':
        return `${localizedBookName ?? submittedVerseRef.book} ${submittedVerseRef.chapterNum}:${submittedVerseRef.verseNum}`;
      case 'chapter':
        return `${localizedBookName ?? submittedVerseRef.book} ${submittedVerseRef.chapterNum}`;
      case 'book':
        return localizedBookName ?? submittedVerseRef.book;
      case 'selectedBooks':
  ...
  const handleScroll = (event: React.UIEvent<HTMLDivElement>) => {
    ...
    if (scrollPercentage > 0.8 && activeJobId && results.length < totalNumberOfResults) {
      loadMoreResults();
    }
  };
  ...
  const [currentProjectVersePossiblyError, , isFocusedVerseTextLoading] = useProjectData(
    ...
    projectId ?? undefined,
  ).VerseUSFM(focusedResultVerseRef, 'loading verse text...');
  ...
  const getFocusedVerseText = (resultText: string) => {
    if (!focusedVerseText || !resultText || focusedResultIndex === undefined)
      return focusedVerseText;

    const focusedResult = results[focusedResultIndex];
    let occurrenceIndex = 0;

    for (let i = focusedResultIndex - 1; i >= 0; i--) {
      const prevResult = results[i];

      if (
        prevResult.verseRef.book !== focusedResult.verseRef.book ||
        prevResult.verseRef.chapterNum !== focusedResult.verseRef.chapterNum ||
        prevResult.verseRef.verseNum !== focusedResult.verseRef.verseNum
      ) {
        break;
      }
    ...
    const targetOccurrenceIndex =
      occurrenceIndex < occurrences.length ? occurrences[occurrenceIndex] : occurrences[0];
                          ...
                          onClick={(e) => {
                            ...
                            setResults((prevResults) =>
                              prevResults.map((prevResult, i) =>
                                i === focusedResultIndex
                                  ? { ...prevResult, isHidden: true }
                                  : prevResult,
                              ),
                            );
          ...
          {(searchStatus === 'completed' ||
            ...
            results && (
              <p className="tw-font-light">
                {results.length > 0
                  ? `Showing ${results.length - numberOfHiddenResults} of ${totalNumberOfResults} results`
                  : 'No results found'}
              </p>
            )}
          {searchStatus === 'exceeded' && (
