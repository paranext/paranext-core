...
global.webViewComponent = function FindWebView({
  ...
  const handleStartSearch = async () => {
    ...
    // Add the current search term to recent searches
  ...
  const clearSearchResults = async () => {
    ...
    if (searchTerm === submittedSearchTerm) setSearchTerm('');
          ...
          {searchStatus === 'running' && (
            <div className="tw-flex tw-items-center tw-gap-4">
              ...
              <Button onClick={() => handleStopSearch(false)}>
                {localizedStrings['%webView_find_abortSearch%']}
              </Button>
            </div>
          )}
