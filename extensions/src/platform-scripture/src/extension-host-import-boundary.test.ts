 ...
 * Activation fails and no Find, no checks, and no inventories open — with no build error and no lint error to warn
...
const ALLOWED_MODULES = new Set([
  ...
  'crypto',
  // This extension's own ambient module declaration (`src/types/platform-scripture.d.ts`).
  ...
  // external of its own — no react, no UI package. The data providers have loaded it in the host
...
describe('extension host import boundary', () => {
  it('reaches main.ts and the modules it imports', () => {
    ...
    expect(filesInGraph).toContain('find/find.utils.ts');
  });
  ...
  it('value-imports only modules the extension host supplies', () => {
    ...
    //      `find/extra-material.utils.ts`, which exists for exactly this reason;
