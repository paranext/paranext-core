...
type ParagraphRowProps = {
  ...
  /**
   * Owning cell's anchor reference (e.g. `LUK 1:0`) — provides the book + chapter context for
   * wrapping individual verse-number items into goto links (UX-2 finding #16).
   */
  cellReference: string;
  /**
   * Optional goto callback. When provided, verse-number items render as click-to-navigate buttons.
   * When absent (read-only contexts), verse numbers render as plain superscripts.
   */
  onGotoVerseClick?: (verseRef: string) => void;
};
...
function ParagraphRow({
  ...
  // UX-2 finding #16: derive the book+chapter prefix from the owning cell's reference so each
  // verse-item button can announce/navigate to its own `{book} {chapter}:{verseNumber}`. The
  // cell's reference is anchored at the cell's first verse (e.g. `LUK 1:0`), but only the
  // `book chapter:` prefix is needed here. If the reference is malformed (no colon), fall back
  // to the raw value so the button still receives some context.
  const cellRefPrefix = cellReference.includes(':')
    ? cellReference.slice(0, cellReference.indexOf(':'))
    : cellReference;
  return (
    <div
    ...
    >
      ...
      {showVerseText &&
        paragraph.items.map((item, itemIndex) => {
          ...
          if (item.type === 'verse') {
            // UX-2 finding #16: when a goto handler is provided, render each verse number as a
            // click-to-navigate link. Without a handler we keep the plain superscript fallback
            // (read-only contexts, e.g. Storybook stories without a goto callback wired).
            const verseRef = `${cellRefPrefix}:${item.verseNumber}`;
            if (onGotoVerseClick) {
              return (
                <button
                  key={itemKey}
