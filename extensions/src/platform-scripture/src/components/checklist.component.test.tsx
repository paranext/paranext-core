...
// Structured `ScriptureRange` mirroring how the wiring layer constructs cell/row references — see
// `checklist.types.ts` (`reference?: ScriptureRange`; `firstRef?: ScriptureRange`). The component
// derives the per-verse goto target by inheriting the start's book/chapter and substituting the
// item's verseNumber, so a cell anchored at GEN 2:8 + a `verse 5` item yields GEN 2:5.
const genRefRange = {
  start: { book: 'GEN', chapterNum: 2, verseNum: 8 },
  end: { book: 'GEN', chapterNum: 2, verseNum: 8 },
};
...
describe('ChecklistTool — verse-number goto links (UX-2 #16)', () => {
  ...
  it('calls onGotoLinkClick with the reconstructed GEN 2:5 SerializedVerseRef when clicked', async () => {
    const onGotoLinkClick = vi.fn();
    ...
    // The component reconstructs the per-verse SerializedVerseRef by inheriting the cell-start's
    // book + chapter and substituting the item's parsed verseNumber.
    expect(onGotoLinkClick.mock.calls[0][1]).toMatchObject({
      book: 'GEN',
      chapterNum: 2,
      verseNum: 5,
    });
