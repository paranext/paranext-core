declare module 'platform-scripture' {
  ...
  export type ItemizedInventoryItem = {
    ...
    /**
     * The text of the inventory item itself (e.g., single character in character inventory). This
     * is generally the same as the key used to identify the inventory item. However, it does not
     * have to be the same.
     */
  ...
  export type ItemizedInventoryJobStatus = 'queued' | 'running' | 'errored' | 'completed';
  ...
  export type IInventoryDataProvider = IDataProvider<InventoryDataTypes> & {
     ...
     * The operation runs asynchronously and returns a the summarized inventory data, including a
     * unique ID that can be used to retrieve itemized inventory data later.
     *
