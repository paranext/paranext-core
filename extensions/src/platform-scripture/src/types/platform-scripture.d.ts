declare module 'platform-scripture' {
  ...
  // These types are defined in platform-bible-utils (the lowest layer that both extensions and
  // platform-bible-react can depend on) so that shared code such as platform-bible-react's
  // experimental `useEffectiveResourceReferenceList` hook can reference them without depending on
  // this extension's type declarations. They are re-exported here because this extension owns the
  // settings they describe and existing consumers import them from 'platform-scripture'.
  export type {
