...
function ChecklistContent({
  ...
  const [equivalentMarkers, setEquivalentMarkers] = useWebViewState<string>(
    ...
    persistedDefaults.equivalentMarkers,
  );
  const [markerFilter, setMarkerFilter] = useWebViewState<string>(
    'checklistMarkerFilter',
    persistedDefaults.markerFilter,
  );
  ...
  // setting's `comparativeTextIds` (string[]) by lifting each id into the
  // `ChecklistComparativeTextRef` shape the component uses internally.
  ...
  // checklist tab — or a fresh app session — inherits the latest committed defaults. Scope and
  // verse range are intentionally NOT mirrored: PT9 reset both per-open (FirstVerseRef /
  // LastVerseRef were rewritten from the live scroll-group ref on each open) and the reviewer
  // agreed PT10 should match. The mirror also fires on the very first render to (idempotently)
  // re-write the same value back — harmless, and means a user who never changes the toggles
  // still gets a persisted record once they save the equivalent-markers dialog.
  useEffect(() => {
    writeChecklistDefaults({
