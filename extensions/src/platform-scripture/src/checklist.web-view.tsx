...
function buildClipboardText(columnHeaders: string[], includedRows: ChecklistRow[]): string {
  ...
  const bodyLines = includedRows.map((row) => {
    ...
    // `firstRef` is a structured `ScriptureRange` after PR #2276; format the anchor verse for the
    // clipboard column. Empty when the row carries no reference (e.g. paragraph-only rows).
