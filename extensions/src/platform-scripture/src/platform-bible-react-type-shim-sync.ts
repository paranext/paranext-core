 ...
 * visible and fail this extension's typecheck when they drift (within the limits each assertion's
 * doc comment describes). The resource-reference DATA types need no checking: they live in
 * platform-bible-utils and are re-exported by this extension, so there is a single source of
 * truth.
 *
 ...
 * assignable to it. This is a ONE-DIRECTIONAL guard: it catches renamed/removed methods and
 * incompatible callback types, but method-parameter bivariance means it will NOT catch the real
 * interface loosening or dropping a parameter the shim still declares — do not read it as full
 * equivalence. If this fails, update
