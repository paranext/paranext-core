...
const testExtensionDataScope = {
  extensionName: 'helloRock3',
  dataQualifier: 'webViewTestExtensionData',
};

globalThis.webViewComponent = function HelloRock3ProjectWebView({
  projectId,
  useWebViewState,
}: WebViewProps) {
  ...
  const pdp = useProjectDataProvider('helloRock3', projectId);

  const [numPossiblyError, setNum] = useProjectData('helloRock3', pdp).RandomNumber(
    max,
    numDefault,
  );
  ...
  const [namesPossiblyError] = useProjectData('helloRock3', pdp).Names(undefined, namesDefault);
  ...
  const helloRock3ProjectSettings = useHelloRock3ProjectSettings(pdp);
  const { headerStyle, headerColor } = helloRock3ProjectSettings;
