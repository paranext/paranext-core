 ...
 * - Click "Maximize" inside the drawer to promote the map into a centered MediaViewer Dialog.
 ...
 *   skeletons.
 * - Cycle the row count via the "Items" selector to demonstrate singular/plural count labels.
 */
function InteractiveMediaMapsTabDemo() {
  const [itemCount, setItemCount] = useState<number>(MOCK_MEDIA_MAPS.length);
  const [loaded, setLoaded] = useState(true);
  const [selectedItemId, setSelectedItemId] = useState<string | undefined>();
  const [maximizedId, setMaximizedId] = useState<string | undefined>();

  const items = MOCK_MEDIA_MAPS.slice(0, itemCount);
...
export const Default: Story = {
  ...
  render: () => <InteractiveMediaMapsTabDemo />,
};
...
/** Empty - "No maps for {scope}." rendered when no entries match (BHV-352). */
