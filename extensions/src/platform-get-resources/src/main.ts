 ...
 *   model-text restrictions. A caller that needs it may still join an in-flight sync that does not
 *   refresh them, so ask for it through {@link refreshResourceFlags}, which never settles for one of
 *   those.
 */
