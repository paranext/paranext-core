 ...
 * `projectId` match or by the `startsWith(dblEntryUid)` convention — guarded by `installed` to
 * avoid false exclusions when a UID has been reassigned to a different resource). Maps the
 * remaining metadata to {@link DblResourceData} with `dblEntryUid === projectId` as a synthetic
 * marker so callers can create a {@link ProjectReference} instead of a {@link DblResourceReference}.
 *
