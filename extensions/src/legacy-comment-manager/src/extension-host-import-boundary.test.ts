 ...
 * Activation fails and neither the comment list nor the Comments panel registers its commands or
 * web views — with no build error and no lint error to warn you. This test is that missing check.
 *
...
const ALLOWED_MODULES = new Set([
  ...
  'crypto',
  // This extension's own ambient module declaration (`src/types/legacy-comment-manager.d.ts`).
  // It declares types only, so TypeScript erases these imports and no `require` is emitted.
  'legacy-comment-manager',
  // Not a webpack external, so it is bundled rather than required at runtime, and it pulls in no
  // external of its own — no react, no UI package. The USJ data provider has loaded it in the host
...
describe('extension host import boundary', () => {
  it('reaches main.ts and the modules it imports', () => {
    ...
    expect(filesInGraph).toContain('comment-list-filters.model.ts');
  });
  ...
  it('value-imports only modules the extension host supplies', () => {
    ...
    //   2. take the runtime value as a parameter and let the web-view caller supply it, the way
    //      `comment-list-filters.model.ts` takes a `WarnFn` instead of importing a logger — or move
    //      the code into a module the host never reaches at all;
