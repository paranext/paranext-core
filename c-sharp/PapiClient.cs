...
internal class PapiClient : IDisposable
{
    ...
    )
    {
        ...
        {
            var result = handler.DynamicInvoke(requestContents);
            if (result is null)
            {
    ...
    /// <param name="timeout">Amount of time to wait for the registration response to be received</param>
    ...
    )
    {
        ...
        if (!await ThreadingUtils.IsTaskCompletedAsync(registerTask, timeout, _cancellationToken))
        {
