...
using PapiEventHandler = Func<dynamic?, Message?>;
...
{
    // Lock during register/unregister because there are multiple steps required.
    ...
    public IEnumerable<Message>? HandleMessage(Message message)
    {
        ...
        if (_handlers.TryGetValue(messageEvent.EventType, out var handlersForEventType))
        {
            ...
            {
                var msg = handler.DynamicInvoke(messageEvent.Event);
                if (msg != null)
                    messages.Add(msg);
            }
        }
