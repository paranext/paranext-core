    ...
    public override ResponseToRequest SetProjectData(ProjectDataScope scope, string data)
    {
        ...
        switch (scope.DataType)
        {
            ...
            case ProjectDataType.SETTINGS:
                var currentValueResponse = GetProjectData(scope);
                ...
                var paratextSettingName =
                    ProjectSettings.GetParatextSettingNameFromPlatformBibleSettingName(
                        scope.DataQualifier);
                if (paratextSettingName == null)
                    return ResponseToRequest.Failed(
                        $"No known Paratext setting for {scope.DataQualifier}");
                RunWithinLock(
                    WriteScope.AllSettingsFiles(),
                    _ => scrText.Settings.SetSetting(paratextSettingName,
                        JToken.Parse(data).ToString()));
                return ResponseToRequest.Succeeded(ProjectDataType.SETTINGS);
            default:
