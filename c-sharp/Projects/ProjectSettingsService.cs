...
namespace Paranext.DataProvider.Projects
{
    internal static class ProjectSettingsService
    {
        ...
        /// changes)</param>
        /// <param name="projectInterfaces">The `projectInterfaces` to get default setting value for</param>
        ...
        public static string? GetDefault(PapiClient papiClient, string key, List<string> projectInterfaces)
        {
            ...
            papiClient.SendRequest(
                PROJECT_SETTINGS_SERVICE,
                new object[] { "getDefault", key, projectInterfaces },
                (bool success, object? returnValue) =>
                {
                    try
                    {
            ...
            ResponseToRequest requestHandler(JsonElement jsonElement)
            {
                ...
                )
                {
                    return ResponseToRequest.Failed(
                        $"Validator for {key} expected a JSON array with 4 items: newValueJson"
                            + "currentValueJson, allChangesJson, projectInterfaces."
                    );
                }
                ...
                List<string> projectInterfacesJson = jsonElement[3].Deserialize<List<string>>() ?? [""];

                var validationResponse = validatorCallback(
                    (newValueJson, currentValueJson, allChangesJson, projectInterfacesJson)
                );
