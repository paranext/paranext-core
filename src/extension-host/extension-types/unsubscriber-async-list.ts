...
export default class UnsubscriberAsyncList {
  readonly unsubscribers: Set<UnsubscriberAsync> = new Set<UnsubscriberAsync>();

  /**
   * Add unsubscribers to the list. Note that duplicates are not added twice.
   * @param unsubscribers Objects that were returned from a registration process
   */
  add(...unsubscribers: (UnsubscriberAsync | Dispose)[]) {
  ...
  async runAllUnsubscribers(): Promise<boolean> {
    const unsubs = [...this.unsubscribers].map((unsubscriber) => {
      return unsubscriber();
    });
