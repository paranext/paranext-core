...
export function validatePopoverRequest(request: PopoverRequest): void {
  ...
  if (content.type === 'card') {
    ...
    if (!content.items || content.items.length === 0) {
      throwValidationError('List content must have at least one item');
    }
  } else if (content.type === 'richText') {
    if (!content.body || content.body.length === 0) {
      throwValidationError('Rich text content must have at least one text run');
    }
  } else if (content.type === 'description') {
    if (!content.entries || content.entries.length === 0) {
      throwValidationError('Description content must have at least one entry');
    }
  }
