 ...
 * filtering `filterResourcesByType` (in the platform-scripture-editor extension's
 * `resource-reference.utils.ts`) already applies for display: `dblResource` items are typed via the
 * cached DBL resource catalog, `project` items always belong to the Scripture tab, and any other
 * reference type — including a `dblResource` item whose id isn't (currently) found in the catalog —
 * is routed into `otherResources` instead of being dropped. `src/renderer` cannot import across the
 * extension boundary, so this comment is the only thing keeping the two in step: edit them
 * together. The dialog doesn't display or let the admin edit `otherResources`, but callers must
 * round-trip it unchanged when writing the setting back out, or those references are permanently
 * lost.
 */
