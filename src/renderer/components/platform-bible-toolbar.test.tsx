...
describe('PlatformBibleToolbar — sync button accessible name', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useSetting).mockReturnValue(['simple', vi.fn(), vi.fn(), false]);
    mockSendCommand(true);
  ...
  it('names the sync button independently of its visible label', async () => {
    render(<PlatformBibleToolbar />);
    // The label is visible at every width today, so this is redundant *today*. It is asserted
    // because the button is expected to collapse toward icon-only (PT-4344 / ADR-0016), and at that
    // point the visible text stops being its accessible name. A button whose name depends on its
    // width is the failure this pins.
