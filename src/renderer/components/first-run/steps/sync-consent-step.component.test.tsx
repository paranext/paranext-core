...
describe('SyncConsentStep', () => {
  ...
  // ADDED for PT-4260: setCanProceed(false) on mount prevents shell Next from being clickable.
  it('calls setCanProceed(false) on mount to disable the shell Next button', () => {
    const setCanProceed = vi.fn();
    render(
      <SyncConsentStep onNext={vi.fn()} onSync={makeOnSync()} setCanProceed={setCanProceed} />,
    );
