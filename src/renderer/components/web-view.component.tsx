    ...
    useCallback(
      (callback: (args: Parameters<WebViewMessageRequestHandler>) => void) => {
        return registerRequestHandler(
          ...
          {
            description: `Post a message to a WebView with id "${id}". Expected to be used only by the Web View Provider that created the web view or the Web View Controller that represents the web view created by the Web View Provider.`,
            params: [
              {
                name: 'webViewNonce',
                required: true,
