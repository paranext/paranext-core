...
/** Name of the request to register on the network to determine if a network object exists */
const EXISTS = 'exists';
...
const getNetworkObjectRequestType = (id: string, functionName?: string) =>
  serializeRequestType(CATEGORY_NETWORK_OBJECT, `${id}.${functionName}`);
...
const getRemoteNetworkObjectFunctions = async (id: string): Promise<string[] | undefined> => {
  try {
    return await networkService.request<[], string[]>(getNetworkObjectRequestType(id, EXISTS));
  } catch (e) {
...
const set = async <T extends NetworkableObject>(
  ...
  return lock.runExclusive(async () => {
    ...
    const unsubPromises = [
      networkService.registerRequestHandler(getNetworkObjectRequestType(id, EXISTS), () =>
