...
import { Mutex, MutexMap } from 'platform-bible-utils';
...
export default class ClientWebSocketProtocolHandler implements INetworkProtocolHandler {
  ...
  async connect(localEventHandler: InternalNetworkEventHandler): Promise<boolean> {
    return this.connectionMutex.runExclusive(async () => {
      ...
      try {
        ...
        ClientWebSocketProtocolHandler.handleError('Client websocket could not connect', error);
  ...
  private async onMessage(ev: MessageEvent) {
    try {
      await this.rpcClientServer.receiveAndSend(ev.data, { origin: ev.origin }, undefined);
    } catch (error) {
