 ...
 *
 * "jsonRpcServer" refers to the current process that created this object (i.e., not main)
 *
 * "jsonRpcClient" refers to main
 */
export default class RpcClient implements IRpcMethodRegistrar {
  ...
  async connect(localEventHandler: EventHandler): Promise<boolean> {
    return this.connectionMutex.runExclusive(async () => {
      ...
      // Incoming events from other parts of the network
  ...
  async request(
    ...
    if (response.error) return createErrorResponse(response.error.message, response.error.code);
  ...
  async unregisterMethod(methodName: string): Promise<boolean> {
    ...
    return mutex.runExclusive(async () => {
      ...
      if (successful) {
        this.jsonRpcServer.removeMethod(methodName);
