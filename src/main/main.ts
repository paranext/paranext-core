    ...
    newWindow.on('close', async (event) => {
      ...
      try {
        ...
        if (isAppGoingDown) {
          ...
          // Flush the window-layouts structure with every still-tracked window in it — a window
          // that is not live at save time is not written, so this is what preserves the closing
          // windows' entries across a quit (and the final window's entry on a last-window close).
          // Capture this window's placement first so the flush holds its freshest bounds.
          //
