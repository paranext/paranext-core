...
async function getWindowServiceForWindow(winId: number): Promise<IWindowService | undefined> {
  return getDataProviderByType<IWindowService>(`${windowServiceProviderName}-${winId}`);
}
...
async function main() {
  ...
  const createWindow = async () => {
    ...
    const setWindowFocus = async (
      ...
      else logger.warn(`Window service for window ${windowId} not available yet`);
    };
    ...
    newWindow.on('closed', async () => {
      const idx = windows.indexOf(newWindow);
  ...
  app
    ...
    .then(async () => {
      ...
      if (process.platform === 'darwin') {
        subscribeCurrentMacosMenubar().catch((error) => {
