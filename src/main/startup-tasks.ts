 ...
 * ADR-startup-sync-readiness-gate for that trade-off, and for why having the S/R extension
 * self-trigger at the end of its own activation — once floated here as the cleaner long-term shape
 * — was rejected: extensions activate sequentially, so nothing guarantees it activates after the
 * scripture extension, and if it goes first its self-trigger starves the same factory.
 *
