...
export default class ServerWebSocketProtocolHandler implements INetworkProtocolHandler {
  ...
  private async onMessage(ev: MessageEvent) {
    ...
    if (isJSONRPCResponse(payload) || isJSONRPCResponses(payload)) {
      ...
      const requests: JSONRPCRequest[] = isJSONRPCRequest(payload) ? [payload] : payload;
      ...
      ServerWebSocketProtocolHandler.handleError(message, payload);
