 ...
 *
 * This is the ownership fan-out `web-view-routing.service.ts` does with `findOwner`, collapsed into
 ...
 * for an id it does not know (see `notification.service-host.ts`, which only touches ids in its own
 * tracking map). So dismissing in every window IS the owner lookup, without the extra query hop.
 *
