 ...
 * connects, a RpcWebSocketServer is created in this same process to service that connection.
 *
 * Also owns a map of all registered methods tied to the INetworkProtocolHandler that knows how to
 * respond to those methods. Methods registered within "main" will be tied to this class. Methods
 * registered by other processes will be tied to the RpcWebSocketServer that services the connection
 * for that other process.
 *
...
export default class RpcWebSocketListener implements IRpcMethodRegistrar {
  ...
  private readonly protocolHandlerByMethodName = new Map<string, IRpcHandler>();
  ...
  async registerMethod(methodName: string, method: InternalRequestHandler): Promise<boolean> {
    if (
      this.protocolHandlerByMethodName.has(methodName) ||
  ...
  async unregisterMethod(methodName: string): Promise<boolean> {
    ...
    return (
      this.protocolHandlerByMethodName.delete(methodName) &&
  ...
  private onClientConnect(webSocket: WebSocket): void {
    const rpcServer = new RpcServer(
      ...
      webSocket,
      this.propagateEvent,
      this.protocolHandlerByMethodName,
    );
    rpcServer.connect();
