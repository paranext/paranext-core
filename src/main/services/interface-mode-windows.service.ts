...
export type ModeSwitchDependencies = {
   ...
   */
  isWindowAbandoned: (windowId: string) => boolean;
  ...
  unmarkWindowClosing: (windowId: string) => void;
  /**
  ...
  getPreservedEntryWindowIds: () => string[];
  /** Create a window restoring the persisted entry this durable window id names */
  createWindowForEntry: (windowId: string) => Promise<void>;
  /**
 ...
 */
export function isClosingForModeSwitch(windowId: string): boolean {
 ...
 */
export function clearModeSwitchClose(windowId: string): void {
 ...
 */
export function undoModeSwitchClose(windowId: string): void {
  const deps = dependencies;
  if (!deps) {
    logger.warn(
 ...
 * @param windowId Window whose close is not going to happen after all
 */
function undoModeSwitchCloseWith(deps: ModeSwitchDependencies, windowId: string): void {
  if (!modeSwitchClosingWindowIds.delete(windowId)) return;
 ...
 *   fit to leave
 */
function closeSecondaryWindows(deps: ModeSwitchDependencies): string | undefined {
 ...
 *
 * Entries are named by their durable window id, never by position: creating a window takes a full
 * renderer start, and a window the user closes during one splices its entry out of the list,
 * shifting every later position down. An index captured before that await would name a different
 * entry afterwards.
 *
...
async function reopenPreservedWindows(
  ...
  const reopenedWindowIds = new Set<string>();
  let remaining = deps
    .getPreservedEntryWindowIds()
    .filter((windowId) => !reopenedWindowIds.has(windowId));
  while (remaining.length > 0) {
    if (generation !== switchGeneration) return;
    const windowId = remaining[0];
    reopenedWindowIds.add(windowId);
    try {
      ...
      await deps.createWindowForEntry(windowId);
    } catch (e) {
      ...
      // failing together
      logger.warn(`Could not reopen the window for entry ${windowId}: ${getErrorMessage(e)}`);
    }
    remaining = deps
      .getPreservedEntryWindowIds()
      .filter((candidate) => !reopenedWindowIds.has(candidate));
  }
}
