...
export type ModeSwitchDependencies = {
  ...
  getPreservedEntryIds: () => string[];
  /** Create a window restoring the persisted entry this durable id names */
  createWindowForEntry: (entryWindowId: string) => Promise<void>;
  /**
 ...
 *
 * Entries are named by their own durable id, never by position: creating a window takes a full
...
async function reopenPreservedWindows(
  ...
  const reopenedEntryIds = new Set<string>();
  let remaining = deps.getPreservedEntryIds().filter((entryId) => !reopenedEntryIds.has(entryId));
  while (remaining.length > 0) {
    if (generation !== switchGeneration) return;
    const entryId = remaining[0];
    reopenedEntryIds.add(entryId);
    try {
      ...
      await deps.createWindowForEntry(entryId);
    } catch (e) {
