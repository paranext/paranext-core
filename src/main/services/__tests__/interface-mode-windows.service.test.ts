 ...
 * Window 'w1' is the primary; 'w2' and 'w3' are secondaries.
 */
function makeDeps(overrides: Partial<ModeSwitchDependencies> = {}): ModeSwitchDependencies {
  return {
    getTrackedWindowIds: () => ['w1', 'w2', 'w3'],
    isPrimaryWindow: (windowId) => windowId === 'w1',
...
describe('reacting to an interface-mode change', () => {
  ...
  test('a write of an unrelated setting does nothing', async () => {
    ...
    const deps = makeDeps({ getPreservedEntryWindowIds: () => ['w1', 'w2'] });
  ...
  test('an ordinary simple launch does not run the switch reaction', async () => {
    ...
    const deps = makeDeps({ getTrackedWindowIds: () => ['w1'] });
  ...
  test('the first mode seen after an unreadable startup read is adopted, not acted on', async () => {
    ...
    const deps = makeDeps({ getPreservedEntryWindowIds: () => ['w11', 'w12'] });
    initializeModeSwitchOrchestration(deps, undefined);
  ...
  test('a switch to power with no window open creates nothing', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => [],
      getPreservedEntryWindowIds: () => ['w11', 'w12'],
    });
  ...
  test('a switch to power with no window counts as one too', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => [],
      getPreservedEntryWindowIds: () => ['w11'],
    });
    initializeModeSwitchOrchestration(deps, 'simple');
    const generationBeforeRestore = getSwitchGeneration();
  ...
  test('a seed records the mode without acting on it', async () => {
    ...
    const deps = makeDeps({ getPreservedEntryWindowIds: () => ['w11'] });
    initializeModeSwitchOrchestration(deps, undefined);
  ...
  test('switching to simple closes every window but the primary', async () => {
    ...
    expect(deps.closeWindow).toHaveBeenCalledWith('w2');
    expect(deps.closeWindow).toHaveBeenCalledWith('w3');
    expect(deps.closeWindow).not.toHaveBeenCalledWith('w1');
  });
  ...
  test('a window is marked closing before it is told to close', async () => {
    ...
    const calls: string[] = [];
    const deps = makeDeps({
      markWindowClosing: vi.fn((windowId: string) => {
    ...
    expect(calls).toEqual(['markw2', 'closew2', 'markw3', 'closew3']);
  });
  ...
  test('a window closed for the switch is reported as such, and the survivor is not', async () => {
    ...
    expect(isClosingForModeSwitch('w2')).toBe(true);
    expect(isClosingForModeSwitch('w1')).toBe(false);
  });
  ...
  test('a window that has gone is forgotten, so the set never outlives its windows', async () => {
    ...
    clearModeSwitchClose('w2');

    expect(isClosingForModeSwitch('w2')).toBe(false);
    ...
    expect(isClosingForModeSwitch('w3')).toBe(true);
  });
  ...
  test('a window the user already closed is left alone, and not claimed by the switch', async () => {
    const deps = makeDeps({ isWindowClosing: (windowId) => windowId === 'w2' });
    initializeModeSwitchOrchestration(deps, 'power');

    await handleInterfaceModeChanged('simple');

    expect(deps.closeWindow).not.toHaveBeenCalledWith('w2');
    expect(deps.markWindowClosing).not.toHaveBeenCalledWith('w2');
  ...
  test('a window that is already gone is not left recorded as closing for the switch', async () => {
    ...
    const deps = makeDeps({ closeWindow: vi.fn((windowId: string) => windowId !== 'w2') });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('a window that fails to close does not take the rest of the batch with it', async () => {
    ...
    const deps = makeDeps({
      closeWindow: vi.fn((windowId: string) => {
        if (windowId === 'w2') throw new Error('window vanished');
        return true;
      }),
    });
    initializeModeSwitchOrchestration(deps, 'power');
    ...
    expect(deps.markWindowClosing).toHaveBeenCalledWith('w3');
    expect(deps.hideWindow).toHaveBeenCalledWith('w3');
    expect(deps.closeWindow).toHaveBeenCalledWith('w3');
  ...
  test('a window that fails to close is put back on screen instead of left hidden and claimed', async () => {
    ...
    expect(isClosingForModeSwitch('w2')).toBe(false);
    expect(deps.showWindow).toHaveBeenCalledWith('w2');
    expect(deps.unmarkWindowClosing).toHaveBeenCalledWith('w2');
    // Only the window that failed. Window 3 closed cleanly and must not be brought back.
    expect(deps.showWindow).not.toHaveBeenCalledWith('w3');
    expect(deps.unmarkWindowClosing).not.toHaveBeenCalledWith('w3');
  });
  ...
  test('a recovery that itself fails does not abandon the rest of the batch', async () => {
    ...
    const deps = makeDeps({
      ...
      showWindow: vi.fn((windowId: string) => {
        if (windowId === 'w2') throw new Error('window already destroyed');
      }),
    });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('a partial switch to simple leaves the mode where the user left it, so switching back still works', async () => {
    ...
    const deps = makeDeps({
      ...
      // Window 'w2' is alive again after the recovery, so its entry is not among the preserved
      // ones; only entry 'w7' is. Which entries count as preserved is
      // `getPreservedEntryWindowIds`' own decision, pinned in the persistence suite — this
      // asserts the reopen honours it.
      getPreservedEntryWindowIds: () => ['w7'],
    });
    initializeModeSwitchOrchestration(deps, 'power');
    ...
    expect(deps.createWindowForEntry).toHaveBeenCalledWith('w7');
  });
  ...
  test('a window is taken off screen as it is marked, before its close is requested', async () => {
    ...
    const calls: string[] = [];
    const deps = makeDeps({
      hideWindow: vi.fn((windowId: string) => {
    ...
    expect(calls).toEqual(['hidew2', 'closew2', 'hidew3', 'closew3']);
  });
  ...
  test('a window that has been given up on cannot be the window a switch leaves behind', async () => {
    ...
    const deps = makeDeps({ isWindowAbandoned: (windowId) => windowId === 'w1' });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('a failed write-back is reported rather than swallowed', async () => {
    const deps = makeDeps({
      isWindowAbandoned: (windowId) => windowId === 'w1',
      writeInterfaceModeSetting: vi.fn(async () => {
  ...
  test('a window already on its way out cannot be the window a switch leaves behind', async () => {
    ...
    const deps = makeDeps({
      isWindowClosing: (windowId) => windowId === 'w1',
    });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('a window that is alive is still the survivor when another has been given up on', async () => {
    ...
    // above alone.
    const deps = makeDeps({ isWindowAbandoned: (windowId) => windowId === 'w3' });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('a window that fails to come back does not take the rest of the reopen with it', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => ['w1'],
      getPreservedEntryWindowIds: () => ['w11', 'w12', 'w13'],
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
        if (windowId === 'w12') throw new Error('renderer never started');
      }),
    });
    initializeModeSwitchOrchestration(deps, 'simple');
    ...
    expect(created).toEqual(['w11', 'w12', 'w13']);
  ...
  test('an entry freed while the reopen is running is still picked up', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => ['w1'],
      getPreservedEntryWindowIds: () => preserved,
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
        // The secondary's close settles while the first window is being created
        if (windowId === 'w11') preserved = ['w11', 'w12'];
      }),
    });
    initializeModeSwitchOrchestration(deps, 'simple');
    ...
    expect(created).toEqual(['w11', 'w12']);
  });
  ...
  test('a close reporting in during a reopen does not start a second one', async () => {
    ...
    const deps = makeDeps({
      ...
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
        if (windowId === 'w33') await firstWindowCreated;
      }),
    });
    initializeModeSwitchOrchestration(deps, 'power');
    ...
    expect(created).toEqual(['w33', 'w22']);
  });
  ...
  test('a second switch to power waits for the reopen the first one left running', async () => {
    ...
    const deps = makeDeps({
      ...
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
        if (windowId === 'w11') await firstWindowCreated;
        // A window exists now, so its entry stops being one with nothing on screen
        preserved = preserved.filter((candidate) => candidate !== windowId);
      }),
    });
    initializeModeSwitchOrchestration(deps, 'simple');
  ...
  test('a close reporting in while the mode still reads simple reopens nothing', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => ['w1', 'w2'],
      getPreservedEntryWindowIds: () => ['w22'],
    });
    initializeModeSwitchOrchestration(deps, 'power');
  ...
  test('the primary is focused once the others are closing', async () => {
    ...
    expect(deps.focusWindow).toHaveBeenCalledWith('w1');
  });
  ...
  test('switching to power reopens one window per preserved entry, in entry order', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => ['w1'],
      getPreservedEntryWindowIds: () => ['w0', 'w2', 'w3'],
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
      }),
    });
    ...
    expect(created).toEqual(['w0', 'w2', 'w3']);
  });
  ...
  test('a switch back part-way through stops the older run', async () => {
    ...
    const deps = makeDeps({
      getTrackedWindowIds: () => ['w1'],
      getPreservedEntryWindowIds: () => ['w0', 'w1', 'w2'],
      createWindowForEntry: vi.fn(async (windowId: string) => {
        created.push(windowId);
        // The user flips back while the reopen is still working through the entries
        if (windowId === 'w0') await handleInterfaceModeChanged('simple');
      }),
    });
    initializeModeSwitchOrchestration(deps, 'simple');
