...
/**
 * Reads a WebView's saved definition, tolerating the brief early-mount window before the dock layout
 * is registered (`getSavedWebViewDefinitionSync` throws then).
 */
...
```ts
  ...
  it('ignores update events for other webviews', async () => {
    ...
    act(() => lastWebViewUpdateHandler?.({ webView: { id: 'other', webViewType: 'scriptureEditor' } }));
