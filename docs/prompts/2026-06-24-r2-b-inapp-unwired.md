...
- **Seeded fixture (design §9) — encode exactly:**
  - **Plan:** SIL Compact Base Plan (6 stages: Drafting → Team Check & Comprehension Check → Preparing for Consultant → Consultant Check → Community Review → Final Preparation for Publication).
  - **Two scopes:** **Philippians** (4 ch, book-by-book) **and** **Priority "Birth Narrative"** (Matt 1–2 / Luke 1–2, cross-book).
  - **Roles:** Saroj drafts; consultant **Maria** owns Consultant Check (enables the teammate-block).
  - **Per-chapter spread (Philippians):**
    - **Php 1** — advanced to **Community Review**; carries a **stale reopened Check** (demonstrates NN6 progression + content-addressed staleness).
    - **Php 2** — the **dense focal chapter** in Team Check & Comprehension Check: comprehension Review available; a co-translator Review with comments/issues; `spelling.word-list` = 12 issues; `basic.quotations` = no issues; `other.biblical-terms` = 5 issues; plus a later Task reading **"Available after Consultant Check."**
    - **Php 3** — Drafting (plain next).
    - **Php 4** — Drafting that **blocks Maria** (Saroj's unfinished Drafting is the direct prerequisite of Maria's Consultant Check → drives the Current Task "unblock a teammate" precedence, design §4 rule 0).
