...
is deliberately partial per the implementation owner's decision — see **`adr-custom-marker-support-ships-partial`** in
...
the partial state — update these when implementing:** `adr-custom-marker-support-ships-partial`, and this brief (§2a, §3 row 4,
§6 question 4).
...
| NTH-1 invalid markers red | WI-1 (+WI-2) | Already implemented (two-state red, PT9 TagValidator port); lands with the merge; stylesheet-based per §3 row 4 / `adr-custom-marker-support-ships-partial` |
...
- **Custom-marker deferral (2026-07-13)**: recorded as `adr-custom-marker-support-ships-partial`; tracked by backlog item WI-12
