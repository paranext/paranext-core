...
- **Agent file convention:** frontmatter has `name` (kebab-case, **exactly equal to the filename stem**) + double-quoted `description`; read-only agents use `allowed-tools: Bash, Read, Grep, Glob` (no `model` key) and state up front "Read-only … Do NOT use Edit, Write, or any file-modifying tools." Body: H1 + role, `## Inputs`, a "read X first" directive, `## Step N — <Name>` (em-dash), `## Output`, `## Status reporting`. Empty sections get `None.` sentinels. *(Historical note: `allowed-tools:` was later found to be an unrecognized — silently ignored — subagent frontmatter key and renamed to `tools:` on every agent (two follow-up commits, 2026-07; the branch history's hashes die on squash-merge); follow the agent files, not this plan, for frontmatter.)*
