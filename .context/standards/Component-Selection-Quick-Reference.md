...
Reference consumer: `BookNotAvailableView` (`extensions/src/platform-scripture-editor/src/book-not-available-view.component.tsx`). Rationale for `Empty` over extending `EmptyState`: `adr-empty-is-zero-state-primitive` in [Architecture-Decisions.md](Architecture-Decisions.md).
