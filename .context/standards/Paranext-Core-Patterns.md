...
#### A capability not every PDP can serve is its own `projectInterface`

A method on a Project Data Provider is **never optional** — not `method?()` on the engine type, and not "required on the consumer but optional on the engine" (`adr-pdp-methods-are-never-optional`). Every method a `projectInterface` declares is required of every PDP that advertises it and present on every PDP a consumer obtains through it. So when a capability cannot be served by every PDP that serves the interface it would naturally belong to — `listExtensionDataQualifiers` is the shipped example: every base PDP serves `platform.base`, but a PDP over a remote store may be unable to enumerate — give it its **own** `projectInterface` and require the method there. `platform.extensionDataEnumeration` is the model:

- **Declare the method once, engine-side, and reuse it consumer-side.** The method lives on a `With…Methods` type in `src/shared/models/project-data-provider.model.ts` (`WithProjectDataProviderEngineExtensionDataEnumerationMethods`) carrying the full contract in its TSDoc; the consumer type in `src/declarations/papi-shared-types.ts` is `IProjectDataProvider<TheDataTypes> & WithTheMethods` (`IExtensionDataEnumerationProjectDataProvider`), registered under the interface name in `ProjectDataProviderInterfaces`. This is the split `platform.base` already uses for `getExtensionData`. A `list*`-style method has no data type, so its data-types record is `{}` (`ExtensionDataEnumerationProjectDataTypes`); `ExtractDataProviderDataTypes` resolves a provider with no data types to `{}` explicitly, because the generic `infer` branches cannot.
- **C#: one constant, one list entry, one registration.** Add the name to `c-sharp/Projects/ProjectInterfaces.cs`, add it to the published and/or unpublished lists in `LocalParatextProjects.cs` (published projects are readable, so a read-only capability belongs in both), and register the wire method in the PDP's `GetFunctions()` override — not in the abstract `ProjectDataProvider`, which models `platform.base` for every subclass.
- **Engines claim it or leave it off.** A TS engine that serves it adds the name to its interfaces tuple and `implements` the `With…Methods` type alongside `IBaseProjectDataProviderEngine` (`hello-rock3`); one that cannot serve it simply does not claim the interface (`platform-lexical-tools`) — no throwing stub. Never extend the `platform.base` registration guard (`project-data-provider.service.ts`, which checks only `getExtensionData` and `getSetting`).
- **Consumers discover it from metadata, never by probing.** Check `projectInterfaces` on `papi.projectLookup.getMetadataForProject` (or filter with `includeProjectInterfaces`), then `papi.projectDataProviders.get('platform.extensionDataEnumeration', projectId)`, which throws for a project that does not advertise it. Probing for the method cannot work: a PDP reached over the network is a `createRemoteProxy` (`network-object.service.ts`) that fabricates a request function for *any* property name, so `pdp.method?.()` never short-circuits — it calls and then rejects — while a PDP in the calling process is a local proxy where the property is genuinely absent and a direct call throws synchronously. Interface advertisement is the one discovery mechanism that works on both paths, and it lets a consumer shipped separately from the platform fall back cleanly on a platform that predates the capability. See "Drive variant/visibility logic from `projectInterfaces`" below.
...
Rationale and rejected alternatives: `adr-pdp-methods-are-never-optional` and `adr-pdp-enumerates-extension-data-qualifiers` in `Architecture-Decisions.md`.
...
is just a cache. See
`adr-scroll-group-hosted-in-main`
and `adr-theme-hosted-in-main`;
