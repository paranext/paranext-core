...
This applies to **code** (identifiers, types, comments) and to **UI microcopy** (labels, button text, messages, tooltips). It is enforced at review.
...
Domain-standard initialisms (e.g. `USFM`, `RTL`, `DTO`) are fine to use as-is. If an initialism is project-coined (e.g., `PAPI`) or obscure, either spell it out or define it where it is introduced.
