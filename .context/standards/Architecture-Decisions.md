...
- **Decision:** A pure helper, `resolveNoteEditingSurface({ viewType, isReadOnly }) → 'pane' |
  ...
  document. The pane's `listId` changes only when the note count changes, so a content edit keeps
  the same row's inline editor mounted across re-renders; reordering with an unchanged count is a
...
- **Consequences:** The editor-engine change (`scripture-editors`) and the host wiring
  ...
  never discarded when an editing session ends. The "Auto-show footnote pane" toggle and its
  persisted setting are removed: PT9 has no auto-hide, so the pane now hides only via its own close
  control, and a caller click reveals a hidden pane only in Power mode.
