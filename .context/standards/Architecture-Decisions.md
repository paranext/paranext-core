...
- **Consequences:** Reads are authoritative for both derived flags, which makes the `DBL_RESOURCES`
  ...
  disk is out of date".
- **Amended 2026-09-14:** Reads can now opt into waiting. Callers that act on `installed` use
  `getCachedResources({ waitForInstalledFlagsSync })`, a bounded wait for a sync that starts after
  the request; it does not recompute `updateAvailable`, so `refreshResourceFlags` is still the only
  path that does. See `adr-dbl-install-is-idempotent`.
...
- **Context:** The Model Text panel auto-installs a configured resource its cached catalog reports as
  ...
  that case, so a stale flag produced an install-failed view whose **Try again** re-ran the same call
  forever (PT-4588: 14 identical failures in 4 minutes, against a resource that was on disk). The
  flag goes stale in ways no consumer can rule out: C# computes `Installed` at catalog-fetch time, and
  both that fetch and the TypeScript reconciliation can run before the C# project factory has
  finished registering projects.
- **Decision:** Four rules, which build on `adr-dbl-cache-recompute-on-read`.
  1. **Install is idempotent.** An already-installed, up-to-date resource is a no-op success.
  2. **`installed` is a hint.** A retry re-reads the catalog as well as re-attempting the install
     (`useDblResourceAutoInstall`'s `retryInstall`). Callers that act on the flag — both resource
     panels, the text grid and the resource picker — read through
     `getCachedResources({ waitForInstalledFlagsSync })`, a bounded wait for a sync that starts after
     the request. Callers that only display the flag do not wait.
...
- **Alternatives:** *Treat the "already installed" message as success in TypeScript* — rejected: a
  ...
  rejected: a listing would rather show the previous snapshot than block on a metadata read that can
  take many seconds. *An unbounded wait* — rejected: past the 30-second network timeout the command
