...
## ADR-0007: The Simple editor RESERVES a 5em inline-end gutter for the character-marker bar; floating over the right margin is rejected because there is no right margin

- **Date:** 2026-08-04
- **Status:** Accepted
- **Context:** The character-marker control (already shipped, placement-agnostic) needed a persistent
  home in the Simple-mode Scripture editor, pinned to the inline-end edge of the text column and
  tracking the line the caret is on. The hard requirement was that the bar must never overlap project
  text. Two facts about the editor's own CSS decided the shape of the answer, and they are the facts
  worth remembering: **the editor text column has no right margin to float over.**
  `extensions/src/platform-scripture-editor/src/_editor.scss:40` gives `.editor-input` only
  `padding: 15px 10px`, and `_editor-overrides.scss:17` sets `.editor-container { max-width: none; }`
  — nothing caps the column width, so prose wraps to the full panel width and the inline-end padding
  is 10px. `_usj-nodes.scss:2502` reserves a **start**-side gutter (`padding-left:
  var(--psc-gutter-width)`, 4em) for paragraph markers; `:2647` mirrors it for RTL. There is no
  end-side gutter of any kind.
- **Decision:** (1) **Reserve space rather than float.** A Simple-scoped
  `padding-inline-end: var(--psc-character-marker-bar-width)` in
  `extensions/src/platform-scripture-editor/src/_simple-mode.scss` takes the bar's column out of the
  text's box, so "never overlaps text" is a property of the construction. The overlay's positioned
  container takes its `width` from the **same** custom property, so the bar cannot grow inline-start
  over text when a localized label is wider than English. (2) **5em (80px), not OQ-8's ~7.4em**
  (the prototype bar's `min-w-[118px]`). The narrower reservation is sufficient only because the bar
  uses a **compact trigger**: it omits `currentMarkerLabel`, so the button reads `bd` rather than
  `bd - Bold` (the full localized name still appears in the popover list). Less column narrowing for
  the same guarantee, with **zero** changes to the shipped control. (3) Only the vertical axis is
  computed in JS; horizontal placement is `inset-inline-end: 0`, which mirrors for RTL and is immune
  to paragraph indentation (`\q`, `\q2`) with no math.
- **Alternatives:** **Float the bar over the right margin** (OQ-8's lighter-weight option) —
  **rejected on evidence**: per the two rules cited above there is no margin to float into, so a
  floating bar would sit on top of project text on most wrapped lines, making the overlap the normal
  case rather than an edge case. **Raise `--psc-gutter-width` or put the reservation on a bare
  `.usfm` selector** — rejected: `_usj-nodes.scss` styles the editor in **both** interface modes, so
  either would narrow every Power user's text column for a control they cannot see, and the React
  `!isPowerMode` gate cannot cover a CSS selector. **Reserve only while the bar is drawn** —
  rejected: the whole text column would reflow as the caret entered and left the editor. **Draw the
  bar over the adjacent dock panel** (so no column narrowing at all) — deferred: it needs the
  renderer's overlay service to accept component content, which it does not today.
- **Consequences:** Power mode is pixel-identical (verified in the running app: computed
  `padding-inline-end` is 80px in Simple, 10px in Power). Changing the reserved width is a one-line
  change to a single custom property — and because that property is declared once on
  `.editor-container-simple` and consumed by both the reservation and the bar's width, the two cannot
  drift. The reservation is **double-gated**: it matches only `.editor-container-simple`
  **and** `.psc-gutter-markers`, and the second class is present throughout Simple mode only because
  `getViewOptionsForType` hardcodes the paragraph-structure preset (which sets
  `hasGutterParaMarkers`) for both Simple view types. **Revisit** if Simple ever gains a
  view-option chooser: turning gutter paragraph markers off would remove the reservation while the
  bar keeps rendering — exactly the overlap this decision exists to prevent — so the reservation must
  then move onto a selector the bar's own condition guarantees. Also **revisit** the width on UX
  sign-off, and the overlap-the-dock-panel option if the overlay service grows component content.
- **Source:** `PT-XXX-D2` character-marker gutter placement (design spec
  `docs/superpowers/specs/2026-08-04-character-marker-gutter-placement-design.md`, §2.1 and §3.1).
