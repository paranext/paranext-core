...
- **Consequences:** every window-scoped service now has a scoped identity and (via the router) a
  ...
  from them) but are no longer how anything FINDS a window's implementation — see ADR-0029.
...
- **Amended 2026-08-07 (ADR-0032):** the original decision also included
  ...
  needed a name-keeping mechanism, and that is what ADR-0032 removes.
...
- **Why the entry is not here:** the approach it recorded is not available in this repo, and an
  ...
  keeps the text.
- **What covers this ground instead:** ADR-0030 (the scroll group service) and ADR-0031 (the theme
  service).
