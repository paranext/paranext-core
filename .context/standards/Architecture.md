...
- **Platform code in the renderer registers zero globally-unique names, and no request or command
  ...
  in main — `src/main/services/scroll-group.service-host.ts` (ADR-0020) and
  `src/main/services/theme.service-host.ts` (ADR-0021). Main outlives every window, so a
  ...
  window's shard as a method call (ADR-0022). There is nothing left to keep a per-window name list
  in step with.
