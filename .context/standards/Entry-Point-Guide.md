...
For each entry point (EP-XXX), add to `extensions/src/{ext}/contributions/menus.json`:
...
| EP ID | Menu Path | Command | Label Key | menus.json | main.ts | types.d.ts | localized | Status |
|-------|-----------|---------|-----------|------------|---------|------------|-----------|--------|
| EP-001 | Platform > New Project | {ext}.openNewProject | %mainMenu_newProject% | | | | | |
| EP-002 | Platform > Restore Project | {ext}.openRestoreProject | %mainMenu_restoreProject% | | | | | |
...
git commit -m "[P3][ui] {feature}: Register menu entry points

Entry points: EP-001 through EP-{N}
