...
- **Zero callers = aspirational scaffolding.** A pattern nobody reads is a guess about a future use case, not a proven approach. Copying it propagates an untested idea. (Example: an intermediate ai-porting draft of the greek-esther template picker carried a `useWebViewState('pickerResult')` round-trip that no consumer ever read — the component as shipped in this repo is props/callback-driven, and the working production pattern for committing a dialog result was the manage-books direct-commit, which is structurally different.)
