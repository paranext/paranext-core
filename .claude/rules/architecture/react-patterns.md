...
- **Detect-and-reuse with the two-call pattern**: `papi.webViews.openWebView(type, layout, { ...options, existingId: '?', createNewIfNotFound: false })` → if the returned id is truthy, `papi.webViews.reloadWebView(type, id, options)` and return that id; otherwise call `openWebView` again without `existingId`.
  ...
  - **`reloadWebView` REMOUNTS the web view for HTML and React web views — do not assume otherwise for those.** It re-runs the provider's `getWebView`, and `srcNonce = newNonce()` is regenerated per call and interpolated into the generated `content` (`src/renderer/services/web-view.service-shard.ts`), so `content` differs every time and the `srcDoc` bound in `web-view.component.tsx` changes, reloading the iframe and destroying the React root. Two consequences: fresh options land in ordinary mount-time initializers with no re-apply machinery (no launch token, no consume-once protocol), and every reload **discards the web view's transient UI state** — scroll position, filter text, attached files, in-progress form input. Weigh that cost before reloading a web view the user may be mid-task in. There is a standing TODO on that nonce asking whether it should be stable per web view; if it ever becomes stable, mount-only initializers stop seeing new options. (`getWebViewNonce(id)` IS stable per id — it just is not the nonce that reaches `content`.)
  - **A `WEB_VIEW_CONTENT_TYPE.URL` web view does NOT remount.** Its `webViewContent = webView.content` verbatim — no nonce is interpolated — and `web-view.component.tsx` binds it as `src`, not `srcDoc`, so the same URL in means an unchanged `src`, no navigation and no remount. Mount-time initializers therefore never see the new options: a launch parameter added to a URL web view by following the rule above would silently never arrive. A URL web view that must react to fresh options needs a re-apply path inside the iframe (or a deterministically changing URL), not a mount-only initializer.
  - **Check `reloadWebView`'s return value.** It resolves `undefined` when the target web view is gone (closed between the probe and the reload); treating that as success returns an id for a tab that does not exist.
  - Avoid: serializing a slow lookup ahead of `openWebView` so a click produces no window for seconds; leaving the probe→open gap unserialized, which lets a double-click open two instances of a single-instance web view.
