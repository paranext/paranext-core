...
Host a DataProvider engine in the process where its **dominant data source** lives, so subscribers don't fan out cross-process. A renderer hosts what is genuinely that ONE window's — focus, `WebViewDefinition` reads, its own dock layout (`src/renderer/services/*.service-shard.ts`). The extension host hosts engines whose data is genuinely extension-host-local (extension lifecycle, cross-extension coordination). Anything app-global belongs in main, which outlives every window: per-machine facts (OS dark mode via `nativeTheme`), app-wide `localStorage`, and any state two windows must agree on (theme, scroll groups — see `adr-scroll-group-hosted-in-main` and `adr-theme-hosted-in-main`).

- **Choice:** put the engine in the process that owns its primary data source. e.g. `window.service-shard.ts` (emits `Focus` / `AppFocus` from renderer-local events) is per-window and lives in the renderer, while `src/main/services/theme.service-host.ts` reads the machine's color-scheme preference and the app-wide store and lives in main.
- **Avoid:** hosting an engine in the extension host when ALL its data sources are renderer-local — every event would then JSON-RPC across the boundary for each subscriber. And do not host app-global state in a renderer: a window is closable, so every other window's access to that state is only as durable as the one window the user happens to close first.
