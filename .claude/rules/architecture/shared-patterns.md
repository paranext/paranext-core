...
- **Avoid:** hosting an engine in the extension host when ALL its data sources are renderer-local — every event would then JSON-RPC across the boundary for each subscriber. And do not host app-global state in a renderer: a window is closable, so the engine has to be re-elected and re-subscribed to every time the user closes the wrong one.
