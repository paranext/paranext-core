...
- **Choice:** put the engine in the process that owns its primary data source. e.g. `window.service-shard.ts` (emits `Focus` from renderer-local events) is per-window and lives in the renderer, while `src/main/services/theme.service-host.ts` reads the machine's color-scheme preference and the app-wide store and lives in main.
