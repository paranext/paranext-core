...
// Resolve playwright from paranext-core's node_modules. ESM resolution may not
// find playwright if the script is not at the repo root. We use createRequire
// anchored at paranext-core's package.json.
...
function buildLocator(page, params) {
  ...
  if (selector) {
    ...
    if (hasText) return loc.filter({ hasText: new RegExp(hasText, 'i') });
    return loc;
  }
  ...
  for await (const line of rl) {
    ...
    try {
      switch (cmd) {
        ...
        case 'frame': {
          const frameLocator = page.frameLocator(params.selector);
          // frameLocator doesn't have a direct "page" equivalent, so we switch
          // to using frame from page.frames()
          ...
          if (!found && params.selector) {
            // Use frameLocator approach — get the frame element's src and match
            try {
              const frameSrc = await page.locator(params.selector).getAttribute('src', { timeout });
              if (frameSrc) {
              ...
              if (!found) {
                // src is null (srcdoc iframe) or no URL match — fall through to index-based
              ...
              // If selector fails, try index-based
              const idx = params.index || 1;
              const nonMainFrames = frames.filter((f) => f !== page.mainFrame());
              if (idx <= nonMainFrames.length) {
                currentFrame = nonMainFrames[idx - 1];
