...
run_all_ai_checks() {
  ...
  _log() { echo "$(date -u +"%Y-%m-%dT%H:%M:%SZ")|ai-hooks|$1" >> "$AUDIT_LOG" 2>/dev/null || true; }
  ...
  if has_ts_changes; then
    ...
    run_typecheck || { rc=$?; _log "typecheck|FAILED|exit=$rc"; exit $rc; }
  ...
  if has_csharp_changes; then
    ...
    run_csharp_build_check || { rc=$?; _log "csharp_build|FAILED|exit=$rc"; exit $rc; }
