 ...
 *
 * RED phase — all tests use test.fixme() until implementation activates them. Tests verify tab
 * container, Dictionary/Encyclopedia/Media/Maps tabs, content loading, expand/collapse, rendering
 * status icons, and semantic domain links.
 *
...
test.describe('UI-PKG-006: ResearchTabs — Render', () => {
  test.fixme(
    'RND-001: Four tab icons render (Dictionary, Encyclopedia, Media, Maps)',
    async ({ mainPage }) => {
      // @scenario BHV-401
  ...
  test.fixme('RND-002: Dictionary tab is selected by default', async ({ mainPage }) => {
    // @scenario BHV-401
    const frame = await openERWebView(mainPage);
  ...
  test.fixme(
    'RND-003: Dictionary tab shows lexicon entries with lemma, POS, glosses',
    async ({ mainPage }) => {
  ...
  test.fixme(
    'RND-004: Tab icons show enabled/disabled based on content count',
    async ({ mainPage }) => {
      // @scenario BHV-401
      const frame = await openERWebView(mainPage);
...
test.describe('UI-PKG-006: ResearchTabs — Interaction', () => {
  test.fixme('INT-001: Tab switching loads different content', async ({ mainPage }) => {
    // @scenario BHV-401
    const frame = await openERWebView(mainPage);
    const tabs = frame.locator('[data-testid="research-tabs"] [role="tab"]');
    await expect(tabs.first()).toBeVisible({ timeout: 10_000 });
  ...
  test.fixme('INT-002: Dictionary entry expand/collapse row detail', async ({ mainPage }) => {
    // @scenario BHV-406
    const frame = await openERWebView(mainPage);
    const dictContent = frame.locator('[data-testid="dictionary-tab-content"]');
    await expect(dictContent).toBeVisible({ timeout: 15_000 });
  ...
  test.fixme('INT-003: Expand all / Collapse all functionality', async ({ mainPage }) => {
    // @scenario BHV-407
    const frame = await openERWebView(mainPage);
    const dictContent = frame.locator('[data-testid="dictionary-tab-content"]');
    await expect(dictContent).toBeVisible({ timeout: 15_000 });
  ...
  test.fixme(
    'INT-004: Encyclopedia row click opens ArticleViewer overlay',
    async ({ mainPage }) => {
      // @scenario BHV-406
      const frame = await openERWebView(mainPage);
  ...
  test.fixme(
    'INT-005: Media tab thumbnail click opens MediaViewer overlay',
    async ({ mainPage }) => {
      // @scenario BHV-406
      const frame = await openERWebView(mainPage);
      // Switch to Media tab
...
test.describe('UI-PKG-006: ResearchTabs — Data Wiring', () => {
  test.fixme('DW-001: Content reloads on scope filter change', async ({ mainPage }) => {
    // @scenario BHV-402
    const frame = await openERWebView(mainPage);
    const dictContent = frame.locator('[data-testid="dictionary-tab-content"]');
    await expect(dictContent).toBeVisible({ timeout: 15_000 });
  ...
  test.fixme(
    'DW-002: Content reloads on linked word click (filter change)',
    async ({ mainPage }) => {
