...
test.describe('UI-PKG-004: ERToolbar — Render', () => {
  test('RND-001: Toolbar renders with Research Terms toggle, project dropdown, and highlight buttons', async ({
    mainPage,
  }) => {
  ...
  test('RND-002: Found and Missing buttons disabled when no tracked project', async ({
    mainPage,
  }) => {
    // @scenario BHV-414
    const frame = await openERWebView(mainPage);
  ...
  test('RND-003: Scope filter dropdown defaults to "Current Verse"', async ({ mainPage }) => {
    // @scenario BHV-402
    const frame = await openERWebView(mainPage);
...
test.describe('UI-PKG-004: ERToolbar — Interaction', () => {
  test('INT-001: Research Terms toggle controls blue highlight CSS class', async ({ mainPage }) => {
    // @scenario BHV-414
    const frame = await openERWebView(mainPage);
  ...
  test('INT-002: Scope filter changes reload research tab content', async ({ mainPage }) => {
    // @scenario BHV-402
    const frame = await openERWebView(mainPage);
    const scopeFilter = frame.locator('[data-testid="scope-filter"]');
    await expect(scopeFilter).toBeVisible({ timeout: 10_000 });
  ...
  test('INT-003: Info icon click toggles guide panel', async ({ mainPage }) => {
    // @scenario BHV-604
    const frame = await openERWebView(mainPage);
  ...
  test('INT-004: Clear filter button clears active word filter', async ({ mainPage }) => {
    // @scenario BHV-403
    const frame = await openERWebView(mainPage);
  ...
  test('INT-005: Tracked project selection enables Found/Missing toggles', async ({ mainPage }) => {
    // @scenario BHV-414
    const frame = await openERWebView(mainPage);
...
test.describe('UI-PKG-004: ERToolbar — Data Wiring', () => {
  test('DW-001: Filter box shows green when results found, orange when zero', async ({
    mainPage,
  }) => {
    // @scenario BHV-403
    const frame = await openERWebView(mainPage);
