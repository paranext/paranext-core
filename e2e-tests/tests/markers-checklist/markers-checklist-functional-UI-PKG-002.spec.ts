...
test.describe('markers-checklist UI-PKG-002: Checklists Tool', () => {
  ...
  // @scenario TS-040
  // @behavior BHV-312
  test('Settings… item is present in the three-dot tab menu under the export group', async ({
    mainPage,
  }) => {
    await openMarkersChecklistViaToolsMenu(mainPage);
  ...
  test('Hide Matches + verse range survive close and reopen via useWebViewState slots', async ({
    mainPage,
  }) => {
    await openMarkersChecklistViaToolsMenu(mainPage);
    const frame = checklistFrame(mainPage);
  ...
  test('renders the "Comparative texts have identical markers." message when all markers match', async ({
    mainPage,
  }) => {
    await openMarkersChecklistViaToolsMenu(mainPage);
    const frame = checklistFrame(mainPage);
  ...
  test('when buildChecklistData fails, renders a destructive Alert with a Retry button that re-invokes the call', async ({
    mainPage,
  }) => {
    await openMarkersChecklistViaToolsMenu(mainPage);
    const frame = checklistFrame(mainPage);
    ...
    // state. If the dialog isn't yet wired, this test will stay at test() until UI-PKG-003
    // activates.
    const tab = mainPage.locator('.dock-tab').filter({ hasText: WEBVIEW_IFRAME_TITLE_RE });
    await tab
