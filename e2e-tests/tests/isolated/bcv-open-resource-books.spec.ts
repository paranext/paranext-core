...
test.describe('book/chapter/verse control reaches books in an open resource', () => {
  ...
  test('offers a book from another open project, greyed, and navigates to it', async ({
    ...
    const showAllBooks = mainPage.getByRole('button', { name: 'Show all books' });
    ...
    await expect(showAllBooks).toHaveCount(0);
    ...
    await expect(showAllBooks).toBeVisible({ timeout: 60_000 });
    ...
    await expect(revelationItem).toHaveAttribute('aria-label', /not in this project/);
