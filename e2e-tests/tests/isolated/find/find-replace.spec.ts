 ...
 *
 * The exclusions are load-bearing: the Find panel restores the project's last search term into an
 * empty search box when it mounts, so selecting a word that happens to equal a term an earlier test
 * already searched would let this test pass without the selection ever reaching Find.
 */
async function selectDistinctiveWordInEditor(
  ...
  return editorFrame.evaluate((excluded: string[]) => {
    ...
    throw new Error(
      'No distinctive word (6+ letters, not an excluded search term) found in the editor',
    );
  }, excludedWords);
}
