...
test.describe('Editor selection to Find', () => {
  ...
  test('pre-fills and searches the editor selection when Find is opened from the tab menu', async ({
    ...
    await closeFindPanel(mainPage);
    // Leave no selection behind. `closeFindPanel` clicks the dock tab on the main page, which never
    // reaches the editor iframe, so without this the selection — and the pointer-press snapshot
    // built from it — would pre-fill every later `openFindPanel` call in this file and kick off an
